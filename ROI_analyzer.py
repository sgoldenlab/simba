...
from simba.enums import Paths, ReadConfig, Keys, Dtypes
...
class ROIAnalyzer(object):
    ...
    def __init__(self,
        ...
        self.project_path, self.file_type = read_project_path_and_file_type(config=config)
        self.no_animals = read_config_entry(config, ReadConfig.GENERAL_SETTINGS.value, ReadConfig.ANIMAL_CNT.value, Dtypes.INT.value)
        self.logs_path = os.path.join(self.project_path, 'logs')
        self.detailed_ROI_data_path = os.path.join(self.project_path, Paths.DETAILED_ROI_DATA_DIR.value)
        self.vid_info_df = read_video_info_csv(os.path.join(self.logs_path, ReadConfig.VIDEO_INFO_CSV.value))
        ...
        self.p_thresh = read_config_entry(config, ReadConfig.ROI_SETTINGS.value, ReadConfig.PROBABILITY_THRESHOLD.value, Dtypes.FLOAT.value, 0.00)
        if data_path != None:
            ...
            if len(self.files_found) == 0:
                ...
                raise ValueError('SIMBA ERROR: No data files found.')
