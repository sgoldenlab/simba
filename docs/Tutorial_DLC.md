...
**Note**: This part of SimBA was written early on - when [DeepLabCut](http://www.mousemotorlab.org/deeplabcut) was accessed exclusively by the command line and did not come with the very nice graphical user interface it comes with today. At that early point we added this part of SimBA to make the most important DeepLabCut functions accessible for all members of the lab, regardless of previous programming experience. However - if SimBA users prefer to work in the DeepLabCut interface - to build and access DeepLabCut pose estimation models, then the tracking data can instead be generated in the DeepLabCut interface and this SimBA step can be skipped. The generated pose estimation data can instead be imported directly in csv file format when creating the [behavioral classifier project](https://github.com/sgoldenlab/simba/blob/master/docs/tutorial.md).     
...
7. If you wish to use the settings used by the Golden lab (This setting is used to track two mice, and eight body parts on each of the two mice. Fpr more information, click [here](https://github.com/sgoldenlab/simba/blob/master/docs/tutorial.md#step-1-generate-project-config). If you wish to generate your on DeepLabCut tracking config, please leave this box un-ticked.
