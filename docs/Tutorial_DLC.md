...
*Note*: This part of SimBA was written early on - when [DeepLabCut](http://www.mousemotorlab.org/deeplabcut) was accessed exclusively by the command line and did not come with the very nice GUI it comes with today. At that early point we added this part of SimBA to make DeepLabCut accessable for all members of the Sam Golden lab. If users prefer to work in the DeepLabCut interface to create and access pose estimation models, then the tracking data can instead be generated in the DLC GUI and this step can be skipped. The generated pose estimation data can instead be imported directly in csv file format when creating the [behavioral classifier project](https://github.com/sgoldenlab/simba/blob/master/docs/tutorial.md).     
