...
Once complete, you will see the following message: `All SHAP data saved in project_folder/models/evaluations` directory. Navigate to the directory to access your SHAP values. In this folder you will see four newly created files. For example, if you performed SHAP calculations for a classifier called `copulation`, you will find the files `RAW_SHAP_feature_values_copulation_prediction.csv` and `SHAP_values_copulation_prediction.csv` which contain **detailed** SHAP data. Morever, in the same `project_folder/models/evaluations` directory, you will find three files with SHAP summary statistics named something like `SHAP_summary_copulation_PRSESENT_20210507160801.csv`, `SHAP_summary_copulation_ABSENT_20210507160801.csv` and `SHAP_summary_line_graph_copulation_20210507160801.png`. Below we will go through how the data in these two files can be interpreted. 
