...
>*Note*: The SimBA interpolation tool will **only** interpolate body-part positions when entire animals are missing from the video frames. The SimBA interpolation tool will **not** interpolate coordinates of animals that are partly missing from the frame. When the animals are partly missing from the frame, then the missing body-parts will be located at coordinate (0,0 - i.e., top left of the frame).  
