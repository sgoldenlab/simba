...
In these Scenario you have the above pilot data, the experimental videos, and SimBA. All videos have been pre-processed and DLC tracked using the "16 body-part, 2 mice" configuration. You now would like to generate a new predictive classifier for "Behavior that Will Get a Nature Paper (Behavior BtWGaNP)". In this first Scenario, we will **use the pilot data to generate the classifier for behavior BtWGaNP.** 
