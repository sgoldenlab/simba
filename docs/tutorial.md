...
6. `Animal Settings` is the number of animals and body parts that the user wished to track. The default for **SimBA** is 2 animals and 16 body parts ( `2 animals, 16bp`). There are a few options in the dropdown menu.
...
3. If you have existing frames of the videos in the project folder, you can import the folder containing the frames into the project. Under **Import frame folders**, click `Browse Folder` to choose the frame folder and click `Import frames`. The folder will be imported to `project_folder/frames/input`
