...
| **TRANSPOSE OUTPUT CSV** (SETTINGS) | If checked, transposes the output so each video is one row and animal–body-part–measurement combinations are columns. Default: each row is a video–animal–body-part–measurement combination. |
| **INCLUDE VIDEO FRAME COUNT** (SETTINGS) | If checked, SimBA adds an extra row per video with measurement **VIDEO FRAME COUNT** and the number of frames in that video’s outlier-corrected pose file. Default: off. |
| **INCLUDE VIDEO LENGTH (HH:MM:SS)** (SETTINGS) | If checked, SimBA adds an extra row per video with measurement **VIDEO LENGTH (HH:MM:SS)** and the duration from frame count ÷ fps (same `HH:MM:SS` format as elsewhere in SimBA). Default: off. |
