...
| **INCLUDE VIDEO LENGTH (HH:MM:SS)** (SETTINGS) | If checked, SimBA adds an extra row per video with measurement **VIDEO LENGTH (HH:MM:SS)** and the duration from frame count ÷ fps (`HH:MM:SS`). Default: off. |
