...
def read_video_info_csv(file_path=None):
    if not check_if_file_exist(file_path):
        raise FileNotFoundError('The project "project_folder/logs/video_info.csv" file does not exists. Please generate the file by completing the [Video parameters] step')
    if not check_if_file_is_readable(file_path):
        raise ValueError('The project "project_folder/logs/video_info.csv" file does not readable/corrupted. Please re-create it by completing the [Video parameters] step')
    ...
    for c in ['Video', 'fps', 'Resolution_width', 'Resolution_height', 'Distance_in_mm', 'pixels/mm']:
        if c not in info_df.columns:
            raise ValueError('The project "project_folder/logs/video_info.csv" does not not have an anticipated {} header. Please re-create the file and make sure each video has a {} value'.format(str(c), str(c)))
    ...
    for c in ['fps', 'Resolution_width', 'Resolution_height', 'Distance_in_mm', 'pixels/mm']:
        try:
            ...
            raise ValueError('The values in the {} column of the "project_folder/logs/video_info.csv" file could not be interepreted as a number. Please re-create the file and make sure the entries in the {} column are all numeric.'.format(str(c), str(c)))
