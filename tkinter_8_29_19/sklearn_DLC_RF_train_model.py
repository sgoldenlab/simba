...
def RF_trainmodel(configini):
    ...
    max_features = config.getint('RF settings', 'max_features')
    ...
    for i in range(model_nos):
        ...
        sample = int((len(targetFrameRows)) + (len(nonTargetFrameRows)/2))
        ...
        data_train, data_test, target_train, target_test = train_test_split(trainFrame,target, test_size=test_size, random_state=10)

        #RANDOM FORREST
        clf=RandomForestClassifier(n_estimators=n_estimators, max_features=max_features, n_jobs=n_jobs, criterion=criterion, min_samples_leaf=min_samples_leaf, verbose=2)
        ...
        log_fn = str(currTargetName) + '_feat_importances' + '.csv'
        log_fn = os.path.join(log_path, log_fn)
        log_df.to_csv(log_fn)
