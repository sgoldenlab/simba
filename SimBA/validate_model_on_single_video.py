...
def validate_model_one_vid(inifile,csvfile,savfile,dt,sb):
    ...
    inputFile = pd.read_csv(sample_feature_file, index_col=0)
    outputDf = inputFile
    ...
    for index, row in currentDf.iterrows():
        ...
        print('Frame:  ' + str(loop) + '/' + str(rowCount))
        loop += 1
    ...
    #combine gantt and frames
    frames_dir_out_merged = os.path.join(frames_dir_out_validation, 'validation', sample_feature_file_Name, 'merged')
    if not os.path.exists(frames_dir_out_merged):
        os.makedirs(frames_dir_out_merged)
    for i in range(rowCount):
        frameImagePath = os.path.join(frames_dir_out_validation_sklearn, str(loop) + '.bmp')
        ganttImagePath = os.path.join(frames_dir_out_validation_gantt, str(loop) + '.png')
        frameImage = cv2.imread(frameImagePath)
        ganttImage = cv2.imread(ganttImagePath)
        horizontalConcat = np.concatenate((frameImage, ganttImage), axis=1)
        horizontalConcat = imutils.resize(horizontalConcat, height=600)
        horizontalConcatHeight, horizontalConcatWidth = horizontalConcat.shape[:2]
    ...
    command = str('ffmpeg -r ' + str(fps) + str(' -f image2 -s ') + str(horizontalConcatHeight) + 'x' + str(horizontalConcatWidth) + ' -i ' + str(ffmpegFileName) + ' -vcodec libx264 -b ' + str(2400) + 'k ' + str(movie_dir_path_out))
