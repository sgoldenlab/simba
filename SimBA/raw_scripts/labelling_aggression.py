...
class main_interface():
    def __init__(self):
        ...
        self.select = Button(self.button_frame, text="Select Frame", command=lambda:
        ...
        # Save
        save = Button(window, text="Save and Advance", command=lambda: self.save_checkBoxes(window))
        ...
        update = Button(videoPlayer, text = 'Update', command = lambda: updateFrameFromVideo(window, self.fbox))
...
# Video
def playVideo():
    p = Popen('python ' + str(os.path.dirname(os.path.split(os.path.split(os.path.dirname(os.path.dirname(os.getcwd())))[-2])[-2])) +
          r"/play_video.py", stdin=PIPE, stdout=PIPE, shell=True)
    # print(str(os.path.split(os.path.dirname(os.path.dirname(os.getcwd())))[-2] + str(
    #     '/videos/Video' + str(currentVideo) + '.mp4')))
