...
for i in filesFound:
    ...
    for index, row in currentDf.iterrows():
        ...
        # Draw DLC circles
        cv2.circle(im, (row['Ear_left_1_x'], row['Ear_left_1_y']), bodyPartCircleSize, (255, 0, 0), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Ear_right_1_x'], row['Ear_right_1_y']), bodyPartCircleSize, (255, 191, 0), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Nose_1_x'], row['Nose_1_y']), bodyPartCircleSize, (255, 255, 0), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Center_1_x'], row['Center_1_y']), bodyPartCircleSize, (255, 165, 0), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Lat_left_1_x'], row['Lat_left_1_y']), bodyPartCircleSize, (0, 255, 0), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Lat_right_1_x'], row['Lat_right_1_y']), bodyPartCircleSize, (255, 0, 255), thickness=-1, lineType=8,shift=0)
        cv2.circle(im, (row['Tail_base_1_x'], row['Tail_base_1_y']), bodyPartCircleSize, (0, 128, 0), thickness=-1, lineType=8, shift=0)

        cv2.circle(im, (row['Ear_left_2_x'], row['Ear_left_2_y']), bodyPartCircleSize, (255, 20, 147), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Ear_right_2_x'], row['Ear_right_2_y']), bodyPartCircleSize, (139, 0, 139), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Nose_2_x'], row['Nose_2_y']), bodyPartCircleSize, (127, 255, 212), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Center_2_x'], row['Center_2_y']), bodyPartCircleSize, (210, 105, 30), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Lat_left_2_x'], row['Lat_left_2_y']), bodyPartCircleSize, (255, 127, 80), thickness=-1, lineType=8, shift=0)
        cv2.circle(im, (row['Lat_right_2_x'], row['Lat_right_2_y']), bodyPartCircleSize, (64, 224, 208), thickness=-1, lineType=8,shift=0)
        cv2.circle(im, (row['Tail_base_2_x'], row['Tail_base_2_y']), bodyPartCircleSize, (255, 105, 180), thickness=-1, lineType=8, shift=0)
        ...
        #draw angle
        cv2.putText(im, str(angle1), (M1polyglon_array[6][0], M1polyglon_array[6][1]), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(im, str(angle2), (M2polyglon_array[6][0], M2polyglon_array[6][1]), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (255, 255, 255), 2)
        ...
        cv2.putText(im, str('Timers'), (10, ((height-height)+textSpacing)), cv2.FONT_HERSHEY_COMPLEX, MfontSize, (0, 255, 0), 2)
        addSpacer = 2
        for k in range(counters_no):
           cv2.putText(im, (str(target_names[k]) + ' ' + str(target_timers[k]) + str('s')), (10, (height-height)+textSpacing*addSpacer), cv2.FONT_HERSHEY_SIMPLEX, SfontSize, (0, 0, 255), 2)
           addSpacer+=1

        cv2.putText(im, str('ensemble prediction'), (10, (height-height)+textSpacing*addSpacer), cv2.FONT_HERSHEY_SIMPLEX, MfontSize, (0, 255, 0), 2)
        addSpacer += 2
        colors = [(2, 166, 249), (47, 255, 173), (0, 165, 255), (60,20,220), (193, 182, 255), (238,130,238), (144, 128, 112), (32, 165, 218), (0,0,128), (209,206,0)]
        for p in range(counters_no):
            if row[targetColumns[p]] == 1:
                cv2.putText(im, str(target_names[p]), (10, (height - height) + textSpacing * addSpacer), cv2.FONT_HERSHEY_TRIPLEX, LfontSize, colors[p], 2)
                target_counters[p] += 1
                addSpacer += 1
        if "tail_rattle" in target_names:
            if row[targetColumns[tailRattleIndex]] == 1:
                ...
                cv2.putText(im, target_names[tailRattleIndex], ((dotCoordinateX+50), (dotCoordinateY+50)), cv2.FONT_HERSHEY_TRIPLEX, LfontSize, (255, 255, 255), 2)
                firstRattle = False
