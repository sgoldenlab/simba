...
def extract_features_wotarget(inifile):
    ...
    for i in filesFound:
        ...
        csv_df['Mouse_polygon_size_change'] = (csv_df_combined['Mouse_poly_area_shifted'] - csv_df_combined['Mouse_1_poly_area'])
        ...
        for i in range(len(roll_windows_values)):
            currentColName = 'Smallest_euclidean_distance_hull' + str(roll_windows_values[i])
            currentDev_colName = currentColName + '_deviation'
            csv_df[currentDev_colName] = (csv_df[currentColName].mean() - csv_df[currentColName])

        for i in range(len(roll_windows_values)):
            currentColName = 'Largest_euclidean_distance_hull' + str(roll_windows_values[i])
            currentDev_colName = currentColName + '_deviation'
            csv_df[currentDev_colName] = (csv_df[currentColName].mean() - csv_df[currentColName])
        ...
        for i in range(len(roll_windows_values)):
            currentColName = 'Mouse_angle' + str(roll_windows_values[i])
            currentDev_colName = currentColName + '_deviation'
            csv_df[currentDev_colName] = (csv_df[currentColName].mean() - csv_df[currentColName])
        ...
        csv_df['Sum_probabilities'] = (
                    csv_df['Mouse1_left_ear_p'] + csv_df['Mouse1_right_ear_p'] + csv_df['Mouse1_left_hand_p'] + csv_df['Mouse1_right_hand_p'] +
                    csv_df['Mouse1_left_foot_p'] + csv_df['Mouse1_tail_p'] + csv_df['Mouse1_right_foot_p'] + csv_df[
                        'Mouse1_back_p', 'Mouse1_nose_p'])
