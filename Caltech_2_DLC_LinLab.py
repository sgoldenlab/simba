...
#If the videos are not cut, set the 'Initial frames removed' in this csv file to be 0. For an example of the layout of this csv file,go to this link: 
...
predictionBehaviourList = []
for i in behaviourList:
    AppendName = str(i) + '_prediction'
    predictionBehaviourList.append(AppendName)
...
for i in range(len(DLCfilesList)):
    ...
    for k in range(len(behaviourList)):
        currentDLCfile.rename(columns={behaviourList[k]: predictionBehaviourList[k]}, inplace=True)
