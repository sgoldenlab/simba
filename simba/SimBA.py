...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    ...
    def importBoris(self):
        ...
        boris_appender = BorisAppender(
            config_path=self.config_path, boris_folder=ann_folder
        )
    ...
    def importSolomon(self):
        ...
        solomon_importer = SolomonImporter(
            config_path=self.config_path, solomon_dir=ann_folder
        )
    ...
    def import_deepethogram(self):
        ...
        deepethogram_importer = DeepEthogramImporter(
            config_path=self.config_path, deep_ethogram_dir=ann_folder
        )
    ...
    def importMARS(self):
        ...
        bento_appender = BentoAppender(
            config_path=self.config_path, bento_dir=bento_dir
        )
        bento_appender.run()
...
class App(object):
    def __init__(self):
        ...
        video_process_menu.add_command(
            ...
            compound="left",
            image=self.menu_icons["trash"]["img"],
            command=lambda: superimpose_frame_count(file_path=askopenfilename()),
        )
        video_process_menu.add_command(
            label="Convert to grayscale",
            compound="left",
            image=self.menu_icons["grey"]["img"],
            command=lambda: video_to_greyscale(file_path=askopenfilename()),
        )
        video_process_menu.add_command(
