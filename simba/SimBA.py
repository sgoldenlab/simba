...
class loadprojectini:
    def __init__(self,configini):
        ...
        self.new_ROI_frm = LabelFrame(tab6, text='New SimBA ROI interface')
        ...
        button_plotsklearnr2 = Button(label_skv_single, text='Visualize classification results',command=lambda: self.plotsklearnresultsingle(self.projectconfigini,self.videovar2.get(),self.genframevar2.get(),self.video_entry.getChoices()))
    ...
    def plotsklearnresultsingle(self, project_config_path, video_setting, frame_setting, video_path):
        if video_setting == 1:
            video_setting = True
        else:
            video_setting = False
        ...
        simba_plotter = PlotSklearnResults(config_path=project_config_path, video_setting=video_setting, frame_setting=frame_setting, video_file_path=video_path)
    ...
    def create_gannt_plots(self, frame_setting, video_setting):
        ...
        if frame_setting == 1:
            ...
            frame_setting = False
        gannt_creator = GanntCreator(config_path=self.projectconfigini, frame_setting=frame_setting, video_setting=video_setting)
    ...
    def timebinmove(self,var):
        ...
        if var == 'mov':
            ...
            tb_button = Button(tb_labelframe, text='Run', command=lambda:self.run_time_bins_clf(time_bin=int(tb_entry.entry_get)))
        ##organize
    ...
    def run_time_bins_clf(self, time_bin=None):
        time_bins_clf_analyzer = TimeBinsClf(config_path=self.projectconfigini, bin_length=time_bin)
        time_bins_clf_analyzer.analyze_timebins_clf()
    ...
    def plotsklearn_result(self):
        if self.videovar.get() == 1:
            ...
            video_setting = False
        if self.genframevar.get() == 1:
            frame_setting = True
        else:
            frame_setting = False

        simba_plotter = PlotSklearnResults(config_path=self.projectconfigini, video_setting=video_setting, frame_setting=frame_setting)
        simba_plotter.initialize_visualizations()
    ...
    def validatemodelsinglevid(self):
        ...
        model_validator.save_classification_data()
