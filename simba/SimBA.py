...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    def __init__(self,
        ...
        button_extractfeatures = Button(label_extractfeatures,text='Extract Features',command = lambda: threading.Thread(target=self.run_feature_extraction).start())
        ...
        button_importboris = Button(label_thirdpartyann,text='Import BORIS Annotation (select folder with .csv files)', fg='blue', command=self.importBoris)
        button_importsolomon = Button(label_thirdpartyann,text='Import SOLOMON Annotation (select folder with .csv files', fg='blue', command=self.importSolomon)
        ...
        button_importdeepethogram = Button(label_thirdpartyann,text='Import DEEPETHOGRAM Annotation (select folder with .csv files)', fg='blue', command=self.import_deepethogram)
        import_observer_btn = Button(label_thirdpartyann, text='Import NOLDUS OBSERVER Annotation (select folder with .xls/xlsx files)', fg='blue', command=self.import_noldus_observer)
        ...
        button_importethovision.grid(row=3, column=0, sticky=NW)
        button_importdeepethogram.grid(row=4,column=0, sticky=NW)
        import_observer_btn.grid(row=5, column=0, sticky=NW)
