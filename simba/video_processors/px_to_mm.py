...
def get_coordinates_nilsson(filenames, knownmm):
    ...
    def draw_circle(event, x, y, flags, param):
        ...
        if (event == cv2.EVENT_LBUTTONDBLCLK) and len(cordList) < 4:
            ...
            if len(cordList) == 4:
                ...
                cv2.line(
                    overlay,
                    (cordList[0], cordList[1]),
                    (cordList[2], cordList[3]),
                    (144, 0, 255),
                    6,
                )
    ...
    try:
        ...
        print(
            "ERROR: Make sure the video file "
            + str(videoName)
            + " is located in your project_folder/videos directory"
        )
    ...
    while 1:
        ...
        if (cordStatus == True) and (moveStatus == False) and (insertStatus == False):
            if changeLoop == True:
                ...
                cv2.circle(
                    overlay, (cordList[0], cordList[1]), circleScale, (144, 0, 255), -1
                )
                cv2.circle(
                    overlay, (cordList[2], cordList[3]), circleScale, (144, 0, 255), -1
                )
                cv2.line(
                    overlay,
                    (cordList[0], cordList[1]),
        ...
        if (moveStatus == True) and (insertStatus == False):
            ...
            if coordChange[0] == 1:
                cv2.circle(
                    img, (cordList[2], cordList[3]), circleScale, (144, 0, 255), -1
                )
            if coordChange[0] == 2:
                cv2.circle(
                    img, (cordList[0], cordList[1]), circleScale, (144, 0, 255), -1
                )
            cv2.imshow(
                "Select coordinates: double left mouse click at two locations. Press ESC when done",
                img,
    ...
    os.remove(fileName)
    euclidPixelDist = np.sqrt(
        (cordList[0] - cordList[2]) ** 2 + (cordList[1] - cordList[3]) ** 2
    )
    mm_dist = int(knownmm)
    cordStatus = False
    moveStatus = False
