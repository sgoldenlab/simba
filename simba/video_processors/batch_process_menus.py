...
from simba.utils.errors import NoFilesFoundError, FFMPEGCodecGPUError
...
class BatchProcessFrame(PopUpMixin):
    ...
    def change_quality_options_cpu_gpu(self):
        ...
        for video_cnt, video_name in enumerate(self.videos.keys()):
            ...
            if not self.use_gpu_var.get():
                self.videos[video_name]['video_quality_dropdown'] = DropDownMenu(self.videos_frm, '', self.cpu_video_quality, '5')
    ...
    def create_video_rows(self):
        ...
        for w in self.videos_frm.grid_slaves():
            if w.grid_info()["column"] == 14 and w.grid_info()["row"] > 0: w.grid_remove()
        for video_cnt, (name, data) in enumerate(self.videos_in_dir_dict.items()):
            ...
            self.videos[name]['video_quality_dropdown'] = DropDownMenu(self.videos_frm, '', self.cpu_video_quality, '5')
    ...
    def batch_process_crop_function(self, video_name):
        ...
        self.crop_dict[video_name]['height'] = roi_selector.width
