...
class BatchProcessFrame(PopUpMixin):
    ...
    def get_input_files(self):
        for file_path in glob.glob(self.input_dir + '/*'):
            ...
            if lower_str_name.endswith(Options.ALL_VIDEO_FORMAT_OPTIONS.value):
                ...
                self.videos_in_dir_dict[video_name] = get_video_meta_data(video_path=file_path, fps_as_int=False)
    ...
    def create_main_window(self):
        ...
        self.quick_set_quality_dropdown = DropDownMenu(self.quick_set_quality, 'Video Quality % ', list(range(10, 110, 10)), '14')
        self.quick_set_quality_dropdown.setChoices(100)
        self.quick_set_qualitys_empty_row = Label(self.quick_set_quality, text=" ")
        self.quick_set_quality_apply = Button(self.quick_set_quality, text='Apply', command=lambda: self.apply_quality_to_all())

        self.use_gpu_frm = LabelFrame(self.quick_settings_frm, text='GPU', padx=5, pady=10)
        self.use_gpu_var = BooleanVar(value=False)
        use_gpu_cb = Checkbutton(self.use_gpu_frm, text='Use GPU (reduced runtime)', variable=self.use_gpu_var, command=lambda: self.change_quality_options_cpu_gpu())
    ...
    def change_quality_options_cpu_gpu(self):
        ...
        if not self.use_gpu_var.get():
            ...
            self.quick_set_quality_dropdown = DropDownMenu(self.quick_set_quality, 'Video Quality % ', ['Low', 'Medium', 'High'], '14')
        ...
        for video_cnt, video_name in enumerate(self.videos.keys()):
            ...
            if not self.use_gpu_var.get():
                self.videos[video_name]['video_quality_dropdown'] = DropDownMenu(self.videos_frm, '', list(range(10, 110, 10)), '5')
                self.videos[video_name]['video_quality_dropdown'].setChoices(100)
            else:
                self.videos[video_name]['video_quality_dropdown'] = DropDownMenu(self.videos_frm, '', ['Low', 'Medium', 'High'], '5')
                self.videos[video_name]['video_quality_dropdown'].setChoices('Medium')
            self.videos[video_name]['video_quality_dropdown'].grid(row=video_cnt+1, column=14, sticky=W)


    def apply_resolution_to_all(self):
        check_int(value=self.quick_downsample_width_val.get(), min_value=0, name=f'Quick set downsample WIDTH {self.quick_downsample_width_val.get()}')
        check_int(value=self.quick_downsample_height_val.get(), min_value=0, name=f'Quick set downsample HEIGHT {self.quick_downsample_height_val.get()}')
        for video_name in self.videos.keys():
    ...
    def apply_trim_to_all(self):
        check_if_string_value_is_valid_video_timestamp(value=self.quick_clip_start_entry_box.get(), name=f'Quick set clip START time {self.quick_clip_start_entry_box.get()}')
        check_if_string_value_is_valid_video_timestamp(value=self.quick_clip_end_entry_box.get(), name=f'Quick set clip END time {self.quick_clip_start_entry_box.get()}')
        check_that_hhmmss_start_is_before_end(start_time=self.quick_clip_start_entry_box.get(), end_time=self.quick_clip_end_entry_box.get(), name='Quick set START and END time')
        for video_name in self.videos.keys():
    ...
    def apply_fps_to_all(self):
        check_float(value=self.quick_set_fps_val.get(), min_value=0, name=f'Quick set FPS setting {self.quick_set_fps_val.get()}')
        for video_name in self.videos.keys():
    ...
    def create_video_table_headings(self):
        ...
        self.headings['video_quality_head'] = Label(self.videos_frm, text='Quality %', width=15)
    ...
    def create_video_rows(self):
        ...
        for video_cnt, (name, data) in enumerate(self.videos_in_dir_dict.items()):
            ...
            self.videos[name]['video_quality_dropdown'] = DropDownMenu(self.videos_frm, '', list(range(10, 110, 10)), '5')
    ...
    def execute(self):
        ...
        for video_cnt, (name, data) in enumerate(self.videos_in_dir_dict.items()):
            ...
            if not self.use_gpu_var.get():
                out_video_dict['video_data'][name]['output_quality'] = self.percent_to_crf_lookup[int(self.videos[name]['video_quality_dropdown'].getChoices())]
            else:
                out_video_dict['video_data'][name]['output_quality'] = self.video_quality_to_preset_lookup[self.videos[name]['video_quality_dropdown'].getChoices()]
