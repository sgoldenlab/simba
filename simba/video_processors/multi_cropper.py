...
from simba.utils.errors import CountError, InvalidVideoFileError, InvalidFileTypeError
...
class MultiCropper(object):
    ...
    def __init__(self,
        ...
        if f'.{file_type}' not in Options.ALL_VIDEO_FORMAT_OPTIONS.value:
            raise InvalidFileTypeError(msg=f'The filetype .{file_type} is invalid. Options: {Options.ALL_VIDEO_FORMAT_OPTIONS.value}')
        self.files_found = glob.glob(self.input_folder + f'/*.{self.file_type}')
        check_if_filepath_list_is_empty(filepaths=self.files_found, error_msg=f'SIMBA CROP ERROR: The input direct {self.input_folder} contains ZERO videos in {self.file_type} format')
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            cap.release()
            if not ret:
                raise InvalidVideoFileError(msg=f'The first frame of video {video_name} could not be read', source=self.__class__.__name__)
            ...
            frame = self.draw_txt_w_bg(img=frame, text=video_name, pos=(TextOptions.BORDER_BUFFER_X.value, int((height - height) + space_size)), font_scale=font_size, font_thickness=TextOptions.TEXT_THICKNESS.value, text_color=TextOptions.COLOR.value, text_color_bg=(0,0,0))
            frame = self.draw_txt_w_bg(img=frame, text=f"Define the rectangle boundaries of {self.crop_cnt} cropped videos", pos=(TextOptions.BORDER_BUFFER_X.value, int((height - height) + space_size * self.add_spacer)), font_scale=font_size, font_thickness=TextOptions.TEXT_THICKNESS.value, text_color=TextOptions.COLOR.value, text_color_bg=(0,0,0))
            frame = self.draw_txt_w_bg(img=frame, text=f"Press ESC to continue", pos=(TextOptions.BORDER_BUFFER_X.value, int((height - height) + space_size * (self.add_spacer + 2))), font_scale=font_size, font_thickness=TextOptions.TEXT_THICKNESS.value, text_color=TextOptions.COLOR.value, text_color_bg=(0,0,0))
            ...
            for box_cnt in range(self.crop_cnt):
                ...
                frame = self.draw_txt_w_bg(img=frame, text=video_name, pos=(TextOptions.BORDER_BUFFER_X.value, int((height - height) + space_size)), font_scale=font_size, font_thickness=TextOptions.TEXT_THICKNESS.value, text_color=TextOptions.COLOR.value, text_color_bg=(0, 0, 0))
                frame = self.draw_txt_w_bg(img=frame, text=f'Draw crop #{box_cnt+1} boundaries and press ESC', pos=(TextOptions.BORDER_BUFFER_X.value, int((height - height) + space_size * self.add_spacer)), font_scale=font_size, font_thickness=TextOptions.TEXT_THICKNESS.value, text_color=TextOptions.COLOR.value, text_color_bg=(0, 0, 0))
                roi_selector = ROISelector(path=frame, title=f'MULTI-CROP {video_name} into {self.crop_cnt} videos - press ESC when happy with a rectangle')
                roi_selector.run()
                cv2.destroyAllWindows()
                self.crop_df.loc[len(self.crop_df)] = [video_name, roi_selector.height, roi_selector.width, roi_selector.top_left[0], roi_selector.top_left[1]]
        ...
        for video_name in self.crop_df['Video'].unique():
            ...
            in_video_path = os.path.join(self.input_folder, f'{video_name}.{self.file_type}')
            for cnt, (idx, row) in enumerate(video_crops.iterrows()):
                ...
                out_file_fn = os.path.join(self.output_folder, video_name + f'_{cnt+1}.{self.file_type}')
                if self.gpu:
        ...
        stdout_success(msg=f'{str(len(self.crop_df))} new cropped videos created from {str(len(self.files_found))} input videos. Cropped videos are saved in the {self.output_folder} directory', elapsed_time=self.timer.elapsed_time_str,  source=self.__class__.__name__)
