        ...
        for video_name in self.crop_df['Video'].unique():
            ...
            for cnt, (idx, row) in enumerate(video_crops.iterrows()):
                ...
                if self.gpu:
                    ...
                    command = str('ffmpeg -y -i ') + str(in_video_path) + str(' -vf ') + str('"crop=') + str(width) + ':' + str(height) + ':' + str(topLeftX) + ':' + str(topLeftY) + '" ' + str('-c:v libx264 -c:a copy ') + str(out_file_fn + ' -hide_banner -loglevel error')
