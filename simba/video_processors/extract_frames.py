...
def extract_frames(
    video_filename, frames_dir, overwrite=True, start=-1, end=-1, every=1
):
...
def video_to_frames(
    video_filename, frames_dir, overwrite=True, every=1, chunk_size=1000
):
    ...
    frame_chunks[-1][-1] = min(
        frame_chunks[-1][-1], total
    )  # make sure last chunk has correct end frame

    print(
        "Extracting "
        + str(total)
        + " frames from "
        + str(os.path.basename(video_filename))
        + "..."
    )
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        futures = [
            executor.submit(
                extract_frames, video_filename, frames_dir, overwrite, f[0], f[1], every
            )
