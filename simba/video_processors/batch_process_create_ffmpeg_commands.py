...
class FFMPEGCommandCreator(object):
    ...
    def clip_videos(self):
        ...
        for video, video_data in self.videos_to_clip.items():
            ...
            if self.gpu:
                command = 'ffmpeg -hwaccel auto -i "{}" -ss {} -t {} -c:v h264_nvenc -async 1 "{}" -hide_banner -loglevel error'.format(in_path, start_time, time_difference, out_path)
            else:
    ...
    def apply_fps(self):
        ...
        for video, video_data in self.videos_to_change_fps.items():
            ...
            if self.gpu:
                ...
                command = 'ffmpeg -i "{}" -crf 17 -filter:v fps=fps={} "{}" '.format(in_path, fps, out_path)
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
