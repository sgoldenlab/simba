...
class FFMPEGCommandCreator(object):
    ...
    def find_relevant_videos(self, variable=None):
        ...
        for video, video_data in self.video_dict['video_data'].items():
            if video_data[variable]:
                ...
                videos[video_data['video_info']['video_name']]['last_operation'] = video_data['last_operation']
                videos[video_data['video_info']['video_name']]['output_quality'] = video_data['output_quality']
        return videos
    ...
    def downsample_videos(self):
        ...
        for video, video_data in self.videos_to_downsample.items():
            ...
            if self.gpu:
                ...
                command = f'ffmpeg -i "{in_path}" -vf scale={width}:{height} "{out_path}" -crf {self.quality} -hide_banner -loglevel error'
    ...
    def clip_videos(self):
        ...
        for video, video_data in self.videos_to_clip.items():
            ...
            if self.gpu:
                ...
                command = f'ffmpeg -i "{in_path}" -ss {start_time} -t {time_difference} -async 1 -crf {self.quality} -c:a copy "{out_path}" -hide_banner -loglevel error'
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
    ...
    def apply_fps(self):
        ...
        for video, video_data in self.videos_to_change_fps.items():
            ...
            if self.gpu:
                ...
                command = f'ffmpeg -i "{in_path}" -crf {self.quality} -filter:v fps=fps={fps} "{out_path}"'
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
    ...
    def apply_grayscale(self):
        ...
        for video, video_data in self.videos_to_greyscale.items():
            ...
            if self.gpu:
                ...
                command = f'ffmpeg -i "{in_path}" -crf 17 -vf hue=s=0 "{out_path}" -hide_banner -loglevel error'
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
        ...
        for video, video_data in self.videos_to_frm_cnt.items():
            ...
            try:
                if self.gpu:
                    ...
                    command = f'ffmpeg -i "{in_path}" -crf {self.quality} -vf "drawtext=fontfile=Arial.ttf:text=\'%{{frame_num}}\':start_number=0:x=(w-tw)/2:y=h-(2*lh):fontcolor=black:fontsize=20:box=1:boxcolor=white:boxborderw=5" -c:a copy -y "{out_path}" -hide_banner -loglevel error'
                ...
                if self.gpu:
                    ...
                    command = f'ffmpeg -i "{in_path}" -vf "drawtext=fontfile={simba_font_path}:text=\'%{{frame_num}}\':start_number=1:x=(w-tw)/2:y=h-(2*lh):fontcolor=black:fontsize=20:box=1:boxcolor=white:boxborderw=5" -crf {self.quality} -c:a copy -y "{out_path}" -hide_banner -loglevel error'
                subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
    ...
    def apply_clahe(self):
        ...
        for video, video_data in self.videos_to_frm_cnt.items():
            ...
            print(f'Applying CLAHE {video}... (note: process can be slow for long videos)')
            in_path, out_path = video_data['path'], os.path.join(self.process_dir, os.path.basename(video_data['path']))
    ...
    def crop_videos(self):
        ...
        for video, video_data in self.videos_to_crop.items():
            ...
            if self.gpu:
                ...
                command = f'ffmpeg -i "{in_path}" -vf "crop={width}:{height}:{top_left_x}:{top_left_y}" -c:v libx264 -crf {self.quality} -c:a copy "{out_path}" -hide_banner -loglevel error'
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
