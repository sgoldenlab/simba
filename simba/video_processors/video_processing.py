...
def change_single_video_fps(file_path: Union[str, os.PathLike], fps: int) -> None:
    """
...
def change_fps_of_multiple_videos(directory: Union[str, os.PathLike], fps: int) -> None:
    """
    ...
    stdout_success(
        msg=f"SIMBA COMPLETE: FPS of {str(len(video_paths))} videos changed to { str(fps)}"
    )
...
def remove_beginning_of_video(file_path: Union[str, os.PathLike], time: int) -> None:
    """
...
def clip_video_in_range(
    file_path: Union[str, os.PathLike], start_time: str, end_time: str
) -> None:
    """
...
def downsample_video(
    file_path: Union[str, os.PathLike], video_height: int, video_width: int
) -> None:
    """
...
def gif_creator(file_path: str, start_time: int, duration: int, width: int) -> None:
    """
...
def batch_convert_video_format(
    directory: Union[str, os.PathLike], input_format: str, output_format: str
) -> None:
    """
...
def multi_split_video(
    file_path: Union[str, os.PathLike], start_times: List[str], end_times: List[str]
) -> None:
    """
    ...
    for start_time_cnt, start_time in enumerate(start_times):
        if (
            (len(start_time) != 8)
            or (not r.match(start_time))
            or (re.search("[a-zA-Z]", start_time))
        ):
            raise InvalidInputError(
                msg=f"Start time for clip {str(start_time_cnt+1)} is should be in the format XX:XX:XX where X is an integer between 0-9"
            )
    for end_time_cnt, end_time in enumerate(end_times):
        if (
            (len(end_time) != 8)
            or (not r.match(end_time))
            or (re.search("[a-zA-Z]", end_time))
        ):
            raise InvalidInputError(
                msg=f"End time for clip {str(end_time_cnt+1)} is should be in the format XX:XX:XX where X is an integer between 0-9"
            )
    for clip_cnt, (start_time, end_time) in enumerate(zip(start_times, end_times)):
        ...
        if end_in_s < start_in_s:
            raise InvalidInputError(
                f"Clip {str(clip_cnt+1)} has has an end-time which is before the start-time."
            )
        if end_in_s == start_in_s:
            raise InvalidInputError(
                msg=f"Clip {str(clip_cnt+1)} has the same start time and end time."
            )
        if end_in_s > video_meta_data["video_length_s"]:
            raise InvalidInputError(
                f'Clip {str(clip_cnt + 1)} has end time at {str(end_in_s)} seconds into the video, which is greater then the lenth of the video ({str(video_meta_data["video_length_s"])}s).'
            )
        save_path = os.path.join(
...
def crop_multiple_videos(
    directory_path: Union[str, os.PathLike], output_path: Union[str, os.PathLike]
) -> None:
    """
...
def frames_to_movie(
    directory: Union[str, os.PathLike], fps: int, bitrate: int, img_format: str
) -> None:
    """
...
class VideoRotator(ConfigReader):
    ...
    def __init__(
        self, input_path: Union[str, os.PathLike], output_dir: Union[str, os.PathLike]
    ) -> None:
