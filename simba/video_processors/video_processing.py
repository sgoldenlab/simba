...
from simba.utils.enums import Formats, Paths, ConfigKey, OS
...
def superimpose_frame_count(file_path: Union[str, os.PathLike],
    ...
    if not os.path.isfile(simba_font_path):
        ...
        elif (platform.system() == OS.MAC.value) or (platform.system() == OS.LINUX.value):
    ...
    try:
        if gpu:
            ...
            command = (
                        str('ffmpeg -y -i ') + '"' + file_path + '"' + ' -vf "drawtext=fontfile=' + simba_font_path + ': text=\'%{frame_num}\': start_number=0: x=(w-tw)/2: y=h-(2*lh): fontcolor=black: fontsize=20: box=1: boxcolor=white: boxborderw=5" -c:a copy ' + '"' + save_name + '" -y')
...
def crop_single_video(file_path: Union[str, os.PathLike],
    ...
    if gpu:
        ...
        command = f'ffmpeg -y -i "{file_path}" -vf "crop={roi_selector.width}:{roi_selector.height}:{roi_selector.top_left[0]}:{roi_selector.top_left[1]}" -c:v libx264 -crf 21 -c:a copy "{save_path}"'
    subprocess.call(command, shell=True)
    timer.stop_timer()
