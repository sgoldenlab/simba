...
class OutlierCorrecterLocation(object):
    def __init__(self,
        ...
        in the current frame relative to the location of the body-part in the preceding frame. Results are stored
        in the `project_folder/csv/outlier_corrected_movement_location` directory of the SimBA project.
        ...
        self.dir_out = os.path.join(self.project_path, 'csv', 'outlier_corrected_movement_location')
        ...
        for animal_name in self.animal_bp_dict.keys():
            ...
            self.outlier_bp_dict[animal_name]['bp_1'] = read_config_entry(self.config, 'Outlier settings', 'location_bodypart1_{}'.format(animal_name.lower()), 'str')
            self.outlier_bp_dict[animal_name]['bp_2'] = read_config_entry(self.config, 'Outlier settings', 'location_bodypart2_{}'.format(animal_name.lower()), 'str')

    def find_location_outliers(self):
