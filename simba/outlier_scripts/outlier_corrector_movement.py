...
class OutlierCorrecterMovement(object):
    def __init__(self,
        ...
        in the current frame. Results are stored
        in the `project_folder/csv/outlier_corrected_movement` directory of the SimBA project.
        ...
        self.dir_in = os.path.join(self.project_path, 'csv', 'input_csv')
        ...
        self.criterion = read_config_entry(self.config, 'Outlier settings', 'movement_criterion', 'float')
    ...
    def outlier_replacer(self, bp_lst=None, animal_name=None):
    ...
    def correct_movement_outliers(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            self.data_df = insert_column_headers_for_outlier_correction(data_df=self.data_df, new_headers=self.headers, filepath=file_path)
            ...
            for animal_name in self.outlier_bp_dict.keys():
                ...
                self.outlier_replacer(bp_lst=animal_bps, animal_name=animal_name)
...
# test = OutlierCorrecterMovement(config_path='/Users/simon/Desktop/troubleshooting/train_model_project/project_folder/project_config.ini')
