...
class ImportSLEAP(object):
    def __init__(self, project_path = None,
        ...
        self.file_format = get_workflow_file_format(self.config)
    ...
    def save_multi_index_header_df(self, df=None, filetype=None, savepath=None):
        self.save_paths_lst = []
        if filetype == 'csv':
            df.to_csv(savepath)
        if filetype == 'parquet':
    ...
    def initate_import_slp(self):
        ...
        for vdn_cnt, file_path in enumerate(self.files_found):
            ...
            self.sleap_dict = in_h5.visititems(self.h5_to_dict)
            self.video_path = self.get_provenance()
    ...
    def check_that_all_animals_exist_in_frame(self):
    ...
    def create_tracks(self):
        ...
        #for frame_cnt, frame in enumerate(range(0, 500)):
            ...
            if self.animals_no > 1:
                self.check_that_all_animals_exist_in_frame()
        ...
        self.save_multi_index_header_df(df=self.data_df,filetype=self.file_format,savepath=self.save_path)
    ...
    def create_third_side_img(self):
        self.side_img = np.ones((int(self.video_meta_data['height'] / 2), self.video_meta_data['width'], 3))
        cv2.putText(self.side_img, 'Current video: ' + self.video_name, (10, int(self.spacing_scale)), cv2.FONT_HERSHEY_SIMPLEX, self.font_scale, (255, 255, 255), 2)
    ...
    def show_clean_window(self):
        ...
        while not keyboard_choice:
            ...
            if k == ord('x'):
                ...
                self.assign_individuals()
                keyboard_choice = True

        self.create_third_side_img()
        self.concat_img = np.uint8(np.concatenate((self.frame, self.side_img), axis=0))
        cv2.imshow('Define animal IDs', self.concat_img)
        keyboard_choice = False
        while not keyboard_choice:
    ...
    def insert_multiindex_header(self):
        multi_index_columns = []
    ...
    def save_df(self):
        self.insert_multiindex_header()
        self.save_multi_index_header_df(self.data_df,self.file_format,self.save_path)
    ...
    def perform_interpolation(self):
        if self.interpolation_settings != 'None':
            ...
            self.save_multi_index_header_df(df=interpolate_body_parts.new_df, filetype=self.file_format, savepath=self.save_path)
