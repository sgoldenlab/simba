...
def check_if_valid_img(data: np.ndarray,
                       source: Optional[str] = '',
                       raise_error: Optional[bool] = True) -> Union[bool, None]:
...
def check_that_dir_has_list_of_filenames(dir: Union[str, os.PathLike],
...
def check_valid_array(data: np.ndarray,
                      ...
                      accepted_dtypes: Optional[List[str]] = None) -> None:
    ...
    :parameter Optional[List[Tuple[int]]] accepted_ndims: List of tuples representing acceptable dimensions. If provided, checks whether the array's number of dimensions matches any tuple in the list.
    ...
    >>> check_valid_array(data, source="Example", accepted_ndims=[(2, 2)], accepted_sizes=[2], accepted_dtypes=['int'])
    """
