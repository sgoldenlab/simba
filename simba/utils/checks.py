...
def check_valid_array(data: np.ndarray,
                      ...
                      accepted_shapes: Optional[List[Tuple[int]]] = None) -> None:
...
def check_valid_lst(data: list,
    ...
    if min_len is not None:
        if len(data) < min_len:
            if raise_error:
                raise InvalidInputError(msg=f'Invalid length of list. Found {len(data)}, minimum accepted: {min_len}', source=source)
            else:
                return False
    if max_len is not None:
        if len(data) > max_len:
            if raise_error:
                raise InvalidInputError(msg=f'Invalid length of list. Found {len(data)}, maximum accepted: {min_len}', source=source)
            else:
                return False
