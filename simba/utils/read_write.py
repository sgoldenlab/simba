...
def read_video_info_csv(file_path: Union[str, os.PathLike]) -> pd.DataFrame:
    ...
    if info_df["fps"].min() <= 1:
        InvalidValueWarning(
            ...
            source=read_video_info_csv_.__name__,
        )
    return info_df
...
def copy_multiple_videos_to_project(
    ...
    file_type: str,
    symlink: bool = False,
    allowed_video_formats: Tuple[str] = ("avi", "mp4"),
) -> None:
    """
    ...
    for file_cnt, file_path in enumerate(video_path_lst):
        ...
        if os.path.isfile(dest1):
            ...
            if not symlink:
                print(
                    "{} copied to project (Video {}/{}, elapsed timer {}s)...".format(
                        filebasename,
                        str(file_cnt + 1),
                ...
                print(
                    "{} copied to project (SYMLINK) (Video {}/{}, elapsed timer {}s)...".format(
                        filebasename,
                        str(file_cnt + 1),
                        str(len(video_path_lst)),
                        timer.elapsed_time_str,
...
def clean_sleap_csv_filename(filename: str) -> str:
    """
    Clean a SLEAP CSV filename by removing '.analysis' suffix and project name prefix to match orginal video name.
...
def clean_sleap_csv_filenames_in_dir(dir: Union[str, os.PathLike]) -> None:
    """
    Clean up SLEAP CSV filenames in the specified directory by removing a prefix
    and a suffix, and renaming the files to match the names of the original video files.

    .. note::
       Modified from `vtsai881 <https://github.com/vtsai881>`_.

    :param Union[str, os.PathLike] dir: The directory path where the SLEAP CSV files are located.
    ...
    for file_path in glob.glob(dir + f"/*.{Formats.CSV.value}"):
