            ...
            if check_multiindex:
                ...
                if multi_index_headers_to_keep is not None:
                    if multi_index_headers_to_keep not in list(range(0, header_col_cnt)):
                        raise InvalidInputError(msg=f'The selected multi-header index column {multi_index_headers_to_keep} does not exist in the multi-index header columns: {list(range(0, header_col_cnt))}')
                    else:
...
def read_roi_data(roi_path: Union[str, os.PathLike]) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
    ...
    try:
        ...
        raise InvalidFileTypeError(msg=f'{roi_path} is not a valid SimBA ROI definitions file')
