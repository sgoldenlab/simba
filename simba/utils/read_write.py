...
from simba.utils.checks import check_int, check_if_dir_exists, check_that_column_exist
...
def concatenate_videos_in_folder(in_folder: Union[str, os.PathLike],
    ...
    if substring is not None:
        ...
        for file_path in files:
            if substring in get_fn_ext(filepath=file_path)[1]: sliced_paths.append(file_path)
...
def read_frm_of_video(video_path: Union[str, os.PathLike, cv2.VideoCapture],
                      ...
                      greyscale: Optional[bool] = False) -> np.ndarray:
    """
    ...
    if os.path.isfile(destination) and not overwrite:
        ...
        if not symlink:
            ...
            try:
                ...
                raise PermissionError(msg='Symbolic link privilege not held. Try running SimBA in terminal opened in admin mode', source=copy_single_video_to_project.__name__)
        timer.stop_timer()
        if not symlink:
...
def copy_multiple_videos_to_project(config_path: Union[str, os.PathLike],
    ...
    for file_cnt, file_path in enumerate(video_path_lst):
        ...
        if os.path.isfile(dest1):
            ...
            timer.stop_timer()
            if not symlink:
                print(f'{filebasename} copied to project (Video {file_cnt + 1}/{len(video_path_lst)}, elapsed timer {timer.elapsed_time_str}s)...')
            else:
                print(f'{filebasename} copied to project (SYMLINK) (Video {file_cnt + 1}/{len(video_path_lst)}, elapsed timer {timer.elapsed_time_str}s)...')
    stdout_success(msg=f'{len(video_path_lst)} videos copied to project.', source=copy_multiple_videos_to_project.__name__)
...
def find_all_videos_in_project(videos_dir: Union[str, os.PathLike],
    ...
    for file_cnt, file_path in enumerate(file_paths_in_folder):
        try:
            ...
            raise InvalidFilepathError(msg=f'{file_path} is not a valid filepath', source=find_all_videos_in_project.__name__)
...
def copy_files_in_directory(in_dir: Union[str, os.PathLike],
    ...
    if not os.listdir(out_dir): os.makedirs(out_dir)
...
def remove_files(file_paths: List[Union[str, os.PathLike]],
