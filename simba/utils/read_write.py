...
def read_video_info_csv(file_path: Union[str, os.PathLike], raise_error: bool = True) -> pd.DataFrame:
    ...
    :param bool raise_error: If True, raises error if the entries in the file are not of expected format. Default True.
    ...
    for c in EXPECTED_FLOAT_COLS:
        ...
        if not validity and raise_error:
    ...
    if os.path.isdir(data_path):
        ...
        with open(data_path, "rb") as f:
            try:
                ...
                print(e.args)
                raise InvalidFileTypeError(
                    msg=f"Could not decompress file {data_path} - invalid pickle. Was this file created in SimBA using a different version of Python?",
                    source=read_pickle.__name__,
                )
    else:
...
def read_sleap_csv(file_path: Union[str, os.PathLike]) -> Tuple[pd.DataFrame, list, list]:
    ...
    all_track_missing = data_df[REQUIRED_COLUMNS[0]].isna().all()
    if all_track_missing: data_df[REQUIRED_COLUMNS[0]] = 0
    data_df[REQUIRED_COLUMNS[0]] = data_df[REQUIRED_COLUMNS[0]].fillna(0)
    data_df[REQUIRED_COLUMNS[0]] = data_df[REQUIRED_COLUMNS[0]].astype(str).str.replace(r"[^\d.]+", "", regex=True).astype(float).astype(np.int64)
...
def get_audio_duration(audio_path: Union[str, os.PathLike]) -> float:
    ...
    :param Union[str, os.PathLike] audio_path: Path to audio file.
    ...
    try:
        ...
        if result.returncode != 0:
            raise FFMPEGCodecGPUError(msg=f'Failed to get duration from {audio_path}. FFprobe error: {result.stderr}', source='get_audio_duration')
...
def extract_audio_from_video(video_path: Union[str, os.PathLike],
    ...
    cmd = ['ffmpeg', '-i', str(video_path), '-vn', '-acodec', 'libmp3lame', '-ab', bitrate, '-ar', str(sample_rate),
           '-y', str(save_path)]

    try:
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise FFMPEGCodecGPUError(msg=f'Failed to extract audio from {video_path}. FFmpeg error: {result.stderr}',
    ...
    if not os.path.isfile(save_path):
        raise FFMPEGCodecGPUError(msg=f'Audio extraction appeared to succeed but output file not found at {save_path}',
                                  source='extract_audio_from_video')
    timer.stop_timer()
