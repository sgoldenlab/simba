...
class BorisAppender(object):
    ...
    def create_boris_master_file(self):
        ...
        for file_cnt, file_path in enumerate(self.boris_files_found):
            ...
            try:
                ...
                print('SIMBA WARNING: {} is not a valid BORIS file and is skipped..'.format(file_path))
    ...
    def check_non_overlapping_annotations(self):
    ...
    def append_boris(self):
        for file_cnt, file_path in enumerate(self.feature_files_found):
            ...
            vid_annotations = pd.concat([vid_annotation_starts, vid_annotation_stops], axis=0, join='inner', copy=True).sort_index()
                ...
                for clf in self.clf_names:
                    ...
                    if len(clf_annotations_start) != len(clf_annotations_stop):
                        print('SIMBA BORIS ERROR: The BORIS annotations for behavior {} in video {} contains {} start events and {} stop events.'
                              'SimBA requires the stop and start event counts to be equal'.format(clf, self.video_name, str(clf_annotations_start), str(clf_annotations_stop)))
                        raise ValueError()
                    ...
                    results = self.check_non_overlapping_annotations()
                    if len(results) > 0:
                        print('SIMBA BORIS ERROR: The BORIS annotations for behavior {} in video {} contains '
                              'behavior start events that are initated prior '
                              'to preceding behavior event ending.'
                              'SimBA requires a specific behavior event to end before it can start.'.format(clf, self.video_name))
                        raise ValueError()
                    ...
                    if len(idx_difference) > 0:
                        print('SIMBA BORIS WARNING: SimBA found BORIS annotations for behavior {} in video {} that are annotated to '
                              ...
                              '{} different frames that SimBA will **remove** by default. Make sure you imported the same video as you annotated in BORIS into SimBA'.format(clf, self.video_name, str(len(idx_difference))))
                        annotations_idx = [x for x in annotations_idx if x not in idx_difference]
                    self.out_df[clf] = 0
                    self.out_df.loc[annotations_idx, clf] = 1
                    self.save_boris_annotations()

        print('SIMBA COMPLETE: BORIS annotations appended to dataset and saved in project_folder/csv/targets_inserted directory.')

    def save_boris_annotations(self):
...
# test = BorisAppender(config_path='/Users/simon/Desktop/train_model_project/project_folder/project_config.ini', boris_folder=r'/Users/simon/Desktop/train_model_project/boris_import')
