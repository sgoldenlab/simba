...
def plotsklearnresult(configini,videoSetting, frameSetting):
    ...
    for i in filesFound:
        ...
        writer = cv2.VideoWriter(outputFileName.replace('.csv', '.mp4'), fourcc, fps, (videoWidth+videoWidth+600, videoHeight))
        ...
        currRow = 0
        featureColHeaders = list(currentDf.columns)
        featureColHeaders = featureColHeaders[100:400]
        ...
        while (cap.isOpened()):
            ...
            if ret == True:
                ...
                #     cv2.putText(frame, (str(target_names[k]) + ' ' + str(target_timers[k]) + str('s')), (10, (height - height) + spacingScale * addSpacer), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 0, 255), 2)
                #     addSpacer += 1
                #
                ...
                #         target_counters[p] += 1
                #         addSpacer += 1
                sideImage = np.zeros((height, width+600, 3), np.uint8)

                addSpacer = 1
                spacingScale = 5
                counter = 0
                XScale = 5
                for column in featureColHeaders:
                    counter += 1
                    colValue = str(round(currentDf.loc[currRow, column], 2))
                    PrintString = str(column + ' ' + colValue)
                    cv2.putText(sideImage, PrintString, (XScale, spacingScale + addSpacer), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    addSpacer += 20
                    if counter == 90:
                        XScale = 700
                        spacingScale = 5
                    ...
                    if counter == 180:
                        ...
                        spacingScale = 5
                        addSpacer = 1
                imageConcat = np.concatenate((frame, sideImage), axis=1)
                if videoSetting == 1:
