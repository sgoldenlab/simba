...
class ROIFeatureCreator(object):
    def __init__(self,
        ...
        Class for computing features based on the relationships between the location of the animals and user-defined
        ROIs.
    ...
    def analyze_ROI_data(self):
        ...
        for file_cnt, file_path in enumerate(self.features_files):
            ...
            self.process_within_rois()
            self.distance_to_roi_centroids()
            if self.roi_directing_viable:
                self.process_directionality()
            self.new_data_dict[self.video_name] = self.out_df

    def process_within_rois(self):
    ...
    def distance_to_roi_centroids(self):
    ...
    def save_new_features_files(self):
        for video_name, video_data in self.new_data_dict.items():
...
# test = ROIFeatureCreator(config_path='/Users/simon/Desktop/train_model_project/project_folder/project_config.ini')
# test.analyze_ROI_data()
