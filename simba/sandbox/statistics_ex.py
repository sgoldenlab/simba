...
# ... or one way ANOVA
critical_values = pickle.load(open("/Users/simon/Desktop/envs/simba/simba/simba/assets/lookups/critical_values_05.pickle", "rb"))['f']['one_tail'].values.astype(np.float32)
results = Statistics.one_way_anova(sample_1=animal_1_data, sample_2=animal_2_data, critical_values=critical_values)
...
critical_values = pickle.load(open("/Users/simon/Desktop/envs/simba/simba/simba/assets/lookups/critical_values_05.pickle", "rb"))['independent_t_test']['one_tail'].values.astype(np.float32)
results = Statistics.sliding_independent_samples_t(data=animal_data, time_window=0.5, slide_time=1.0, critical_values=critical_values, fps=fps)
...
autocor_white_animal = Statistics.sliding_autocorrelation(data=white_animal, max_lag=0.5, time_window=1.0, fps=fps)
autocor_black_animal = Statistics.sliding_autocorrelation(data=black_animal, max_lag=0.5, time_window=1.0, fps=fps)

# Calculate sliding Z-scores for a given data array over specified time windows.
z_scores = Statistics.sliding_z_scores(data=white_animal, time_windows=np.array([0.5, 1.5]), fps=fps)
...
# ... or yule coefficient
yule = Statistics.yule_coef(x=lateral_threat.astype(np.int8), y=freezing.astype(np.int8))
print(yule)
...
sokal_sneath = Statistics.sokal_sneath(x=lateral_threat.astype(np.int8), y=freezing.astype(np.int8), w=p_vals)
