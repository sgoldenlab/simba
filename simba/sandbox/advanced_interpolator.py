...
class AdvancedInterpolator(ConfigReader):
    ...
    :parameter bool multi_index_data: If True, the incoming data is multi-index columns dataframes. Use of input data is the ``project_folder/csv/input_csv`` directory. Default: False.
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.file_paths):
            ...
            if self.verbose: print(f"Interpolating data in video {video_name} ({file_cnt+1}/{len(self.file_paths)}) ...")
            ...
            print(f'Video {video_name} complete ({file_cnt+1}/{len(self.file_paths)}). Elapsed time {video_timer.elapsed_time_str}s')
