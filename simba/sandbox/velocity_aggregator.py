...
def velocity_aggregator(
    ...
    if ts_plot:
        ...
        plt.savefig(
            os.path.join(config.logs_path, f"rolling_velocities_{config.datetime}.png")
        )
    timer.stop_timer()
    stdout_success(
        msg=f"Velocity aggregator for {len(file_paths)} files complete. Data saved in {config.logs_path} directory ",
        source=velocity_aggregator.__name__,
    )
...
# RUN THE ANALYSIS
velocity_aggregator(
    config_path=config_path, data_dir=data_dir, body_part=body_part, ts_plot=True
)
