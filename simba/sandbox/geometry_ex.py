...
def pad_img_stack(image_dict: Dict[int, np.ndarray], pad_value: Optional[int] = 0) -> Dict[int, np.ndarray]:
    ...
    check_instance(source=pad_img_stack.__name__, instance=image_dict, accepted_types=(dict,))
    check_int(name=f'{pad_img_stack.__name__} pad_value', value=pad_value, max_value=255, min_value=0)
...
def img_stack_to_video(imgs: Dict[int, np.ndarray],
    ...
    check_instance(source=img_stack_to_video.__name__, instance=imgs, accepted_types=(dict,))
    img_sizes = set()
    for k, v in imgs.items(): img_sizes.add(v.shape)
    if len(list(img_sizes)) > 1: imgs = pad_img_stack(imgs)
...
#Entire_video
GeometryPlotter(config_path=CONFIG_PATH, geometries=[white_animal_polygons, black_animal_polygons], video_name=VIDEO_NAME, core_cnt=-1, save_path='/Users/simon/Desktop/envs/simba/troubleshooting/RI/project_folder/frames/output/stack/test_2.mp4').run()
