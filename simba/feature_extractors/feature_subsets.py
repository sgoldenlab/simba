...
class FeatureSubsetsCalculator(ConfigReader,
    ...
    def __init__(self,
        ...
        self.append_to_features, self.append_to_targets, self.checks_files = append_to_features_extracted, append_to_targets_inserted, include_file_checks
        self.create_temp_folders()

    def create_temp_folders(self):
        self.general_temp_folder = os.path.join(self.project_path, 'csv', f'temp_data_{self.datetime}')
        if not os.path.isdir(self.general_temp_folder): os.makedirs(self.general_temp_folder)
        ...
        if self.append_to_features or self.append_to_targets:
            if self.append_to_features:
                self.features_extracted_temp_path = os.path.join(self.features_dir, f'temp_data_{self.datetime}')
                if not os.path.isdir(self.features_extracted_temp_path): os.makedirs(self.features_extracted_temp_path)
            if self.append_to_targets:
                self.targets_inserted_temp_path = os.path.join(self.targets_folder, f'temp_data_{self.datetime}')
                if not os.path.isdir(self.targets_inserted_temp_path): os.makedirs(self.targets_inserted_temp_path)
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            for family_cnt, feature_family in enumerate(self.feature_families):
                ...
                if feature_family == "Two-point body-part distances (mm)":
                    self.calc_distances()
                elif (feature_family== "Within-animal three-point body-part angles (degrees)"):
                    self.calc_angles()
                elif (feature_family== "Within-animal three-point convex hull perimeters (mm)"):
                    self.calc_three_point_hulls()
                elif (feature_family== "Within-animal four-point convex hull perimeters (mm)"):
                    ...
                    self.calc_movements()
                elif (feature_family == "Frame-by-frame body-part distances to ROI centers (mm)"):
                    self.calc_roi_center_distances()
                elif (feature_family == "Frame-by-frame body-parts inside ROIs (Boolean)"):
                    self.calc_inside_roi()
            self.__save_to_temp()
            self.video_timer.stop_timer()
            print(f"Additional features computed for {self.video_name} complete (elapsed time {self.video_timer.elapsed_time_str}s)...")
        if (self.append_to_features or self.append_to_targets):
            print('Appending data to existing files...')
            self.append_to_data()
        self.replace_files_in_folders()
        self.perform_clean_up(directories=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
    ...
    def __save_to_temp(self):
        save_path = os.path.join(self.general_temp_folder, self.video_name + f'.{self.file_type}')
    ...
    def append_to_data(self):
        for file_path in glob.glob(self.general_temp_folder + f'/*.{self.file_type}'):
            ...
            if self.append_to_features:
                ...
                data_save_path = os.path.join(self.features_extracted_temp_path, video_name + f'.{self.file_type}')
                features_path = os.path.join(self.features_dir, f"{video_name}.{self.file_type}")
                ...
                if not check_same_number_of_rows_in_dfs(dfs=[data_df, new_df]):
                    self.remove_multiple_folders(folders=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise CountError(f'Cannot append feature subsets to features file of {video_name}. The files contain an unequal number of rows ({len(new_df)} vs {len(data_df)})')
                if len(check_if_headers_in_dfs_are_unique(dfs=[data_df, new_df])) > 0:
                    self.remove_multiple_folders(folders=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise DataHeaderError(msg=f'Attempted to append new feature data to features extracted file {features_path}. At least one of the new feature names aleady exist in the file.')
            ...
            if self.append_to_targets:
                ...
                data_save_path = os.path.join(self.targets_inserted_temp_path, video_name + f'.{self.file_type}')
                target_path = os.path.join(self.targets_folder, f"{video_name}.{self.file_type}")
                check_file_exist_and_readable(file_path=file_path)
                data_df = read_df(file_path=target_path, file_type=self.file_type)
                targets_clf_df = pd.DataFrame()
                if not check_same_number_of_rows_in_dfs(dfs=[data_df, new_df]):
                    self.perform_clean_up(directories=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise CountError(f'Cannot append feature subsets to targets file of {video_name}. The files contain an unequal number of rows ({len(new_df)} vs {len(data_df)})')
                if len(check_if_headers_in_dfs_are_unique(dfs=[data_df, new_df])) > 0:
                    self.perform_clean_up(directories=[self.targets_inserted_temp_path, self.features_extracted_temp_path, self.general_temp_folder])
                    raise DataHeaderError(msg=f'Attempted to append new feature data to targets file of {target_path}. At least one of the nes feature names aleady exist in the file.')
        ...
        if self.checks_files:
            if self.append_to_features:
                ...
                file_paths = glob.glob(self.features_extracted_temp_path + f'/*.{self.file_type}')
                self.data_df = self.read_all_files_in_folder_mp_futures(file_paths=file_paths, file_type=self.file_type)
            ...
            if self.append_to_targets:
                ...
                file_paths = glob.glob(self.targets_inserted_temp_path + f'/*.{self.file_type}')
                self.data_df = self.read_all_files_in_folder_mp_futures(file_paths=file_paths, file_type=self.file_type)
                self.check_raw_dataset_integrity(df=self.data_df, logs_path=self.logs_path)
                del self.data_df
    ...
    def replace_files_in_folders(self):
        if self.append_to_features:
            prior_to_features_append_dir = os.path.join(self.features_dir, f'Prior_to_feature_subset_append_{self.datetime}')
            print(f'Replacing data in "features_extracted" directory with additional data (For safety, previous files are copied to {prior_to_features_append_dir})...')
            if not os.path.isdir(prior_to_features_append_dir): os.makedirs(prior_to_features_append_dir)
            for file_path in glob.glob(self.features_dir + f'/*.{self.file_type}'):
                shutil.move(file_path, os.path.join(prior_to_features_append_dir, os.path.basename(file_path)))
            for file_path in glob.glob(self.features_extracted_temp_path + f'/*.{self.file_type}'):
                shutil.move(file_path, os.path.join(self.features_dir, os.path.basename(file_path)))

        if self.append_to_targets:
            prior_to_targets_append_dir = os.path.join(self.targets_folder, f'Prior_to_feature_subset_append_{self.datetime}')
            print(f'Replacing data in "targets_inserted" directory with additional data (For safety, previous files are copied to {prior_to_targets_append_dir})...')
            if not os.path.isdir(prior_to_targets_append_dir): os.makedirs(prior_to_targets_append_dir)
            for file_path in glob.glob(self.targets_folder + f'/*.{self.file_type}'):
                shutil.move(file_path, os.path.join(prior_to_targets_append_dir, os.path.basename(file_path)))
            for file_path in glob.glob(self.targets_inserted_temp_path + f'/*.{self.file_type}'):
                shutil.move(file_path, os.path.join(self.targets_folder, os.path.basename(file_path)))

        if self.save_dir != None:
            print(f'Storing new features in {self.save_dir}...')
            for file_path in glob.glob(self.general_temp_folder + f'/*.{self.file_type}'):
                shutil.move(file_path, os.path.join(self.save_dir, os.path.basename(file_path)))
