...
class FeatureSubsetsCalculator(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        for file_path in self.outlier_corrected_paths:
            ...
            if self.feature_family == 'Two-point body-part distances (mm)':
                ...
                self.calc_angles()
            elif self.feature_family == 'Within-animal three-point convex hulls (mm2)':
                self.calc_three_point_hulls()
            elif self.feature_family == 'Within-animal four-point convex hulls (mm2)':
                self.calc_four_point_hulls()
            elif self.feature_family == 'Entire animal convex hulls (mm2)':
    ...
    def calc_three_point_hulls(self):
        for animal, points in self.within_animal_three_point_combs.items():
            for point in points:
                ...
                self.results[f'{animal} three-point convex hull (area mm) {point[0]}-{point[1]}-{point[2]}'] = Parallel(n_jobs=-1, verbose=0, backend="threading")(delayed(self.convex_hull_calculator_mp)(x, self.pixel_per_mm) for x in three_point_arr)
    ...
    def calc_four_point_hulls(self):
        for animal, points in self.within_animal_four_point_combs.items():
            for point in points:
                ...
                self.results[f'{animal} four-point convex hull (area mm) {point[0]}-{point[1]}-{point[2]}-{point[3]}'] = Parallel(n_jobs=-1, verbose=0, backend="threading")(delayed(self.convex_hull_calculator_mp)(x, self.pixel_per_mm) for x in four_point_arr)
    ...
    def animal_convex_hulls(self):
        for animal, point in self.animal_bps.items():
            ...
            self.results[f'{animal} convex hull (area mm)'] = Parallel(n_jobs=-1, verbose=0, backend="threading")(delayed(self.convex_hull_calculator_mp)(x, self.pixel_per_mm) for x in animal_point_arr)
