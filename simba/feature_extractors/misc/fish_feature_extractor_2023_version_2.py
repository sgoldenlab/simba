...
class FishFeatureExtractor:
    def __init__(self, config_path: str):
        ...
        self.roll_windows_values = check_minimum_roll_windows(
            [20, 15, 10, 4, 2], self.video_info_df["fps"].min()
        )
    ...
    def calc_velocity(self):
        for bp in self.bp_names:
            self.csv_df_combined[bp + "_velocity"] = (
            ...
            for window in self.roll_windows_values:
                ...
                try:
                    self.csv_df_combined[
                        f"Kurtosis_bp_distance_to_{side}_border_{window}"
                    ] = (
                        self.csv_df_combined[f"Mean_bp_distance_to_{side}_border"]
    ...
    def calc_convex_hulls(self):
        ...
        for window in self.roll_windows_values:
            self.csv_df_combined[f"Convex_hull_mean_{window}"] = (
                self.csv_df_combined["Convex_hull"]
                .rolling(window, min_periods=1)
                .mean()
            )
            self.csv_df_combined[f"Convex_hull_std_{window}"] = (
