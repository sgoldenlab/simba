...
class NoldusObserverImporter(ConfigReader):
    ...
    def __read_data(self):
        ...
        for file_path in self.observer_files_found:
            df = pd.read_excel(file_path, sheet_name=None).popitem(last=False)[1]
            for video_name in df[VIDEO_NAME_FIELD].unique():
                ...
                video_df = video_df[EXPECTED_FIELDS]
                video_df = video_df[video_df[EVENT_TYPE_FIELD] != POINT_EVENT]
                m = video_df[TIME_FIELD].astype(str).str.len().max()
                video_df[TIME_FIELD] = video_df[TIME_FIELD].astype(str).str.ljust(m, '0')
                ...
                if video_name in list(self.annotation.keys()):
                    self.annotation[video_name] = pd.concat([self.annotation[video_name], video_df], axis=1).reset_index(drop=True)
                else:
...
# test = NoldusObserverImporter(config_path='/Users/simon/Desktop/envs/troubleshooting/two_animals_16bp_032023/project_folder/project_config.ini',
#                               data_dir='/Users/simon/Downloads/observer-files-4-Simon')
# test.run()
