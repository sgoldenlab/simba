...
class AdvancedLabellingInterface(ConfigReader):
    ...
    def bind_shortcut_keys(self):
        self.main_window.bind(self.img_kbd_bindings['save']['kbd'], lambda x: self.save_results())
        self.main_window.bind(self.img_kbd_bindings['frame+1_keep_choices']['kbd'], lambda x: self.advance_frame(new_frm_number=int(self.current_frm_n.get() + 1), keep_prior_img_cb_status=True))
        self.main_window.bind(self.img_kbd_bindings['print_annotation_statistic']['kbd'], lambda x: self.__print_annotation_statistics())
