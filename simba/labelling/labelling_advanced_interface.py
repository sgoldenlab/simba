...
class AdvancedLabellingInterface(ConfigReader):
    ...
    .. image:: _static/img/adv_annotator.png.png
       :width: 800
       :align: center
    ...
    def __init__(self,
        ...
        self.play_video_script_path = os.path.join(os.path.dirname(simba.__file__), 'labelling', 'play_annotation_video.py')
        ...
        self.next_absent_forward_btn = Button(self.buttons_frm, text="NEXT ABSENT", fg='red', command=lambda: self.find_last_next_annotation(forwards=True, present=False))
        self.next_present_forward_btn = Button(self.buttons_frm, text="NEXT PRESENT", fg='blue', command=lambda: self.find_last_next_annotation(forwards=True, present=True))
        self.last_absent_forward_btn = Button(self.buttons_frm, text="PREVIOUS ABSENT", fg='red', command=lambda: self.find_last_next_annotation(forwards=False, present=False))
        self.last_present_forward_btn = Button(self.buttons_frm, text="PREVIOUS PRESENT", fg='blue', command=lambda: self.find_last_next_annotation(forwards=False, present=True))
    ...
    def save_behavior_in_range(self):
        ...
        if not self.range_on.get():
            ...
            raise FrameRangeError(msg="FRAME RANGE ERROR: END FRAME {} IS MORE THAN THE MAX VIDEO FRAME ({}) AND CANNOT BE SHOWN".format(str(end_frm), str(self.max_frm_no)))
        elif start_frm == end_frm:
            raise FrameRangeError(msg="FRAME RANGE ERROR: START FRAME AND END FRAME IS SET TO THE SAME VALUE ({}) AND DOES NOT REPRESENT A RANGE".format(str(end_frm)))
        elif start_frm > end_frm:
            raise FrameRangeError(msg="FRAME RANGE ERROR: START FRAME ({}) IS LARGER THAB THE END FRAME ({}). PLEASE SPECIFY A RANGE OF FRAMES WHERE THE START FRAME PRECEDE THE END FRAME".format(str(start_frm), str(end_frm)))
        else:
    ...
    def check_integrity_of_multiple_classifiers(self):
        ...
        if (len(none_target_lst) > 0) & (len(labelled_target_lst) > 0):
            raise AdvancedLabellingError(frame=str(self.current_frm_n.get()), lbl_lst=labelled_target_lst, unlabel_lst=none_target_lst)
