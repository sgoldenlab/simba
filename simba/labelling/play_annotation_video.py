...
def annotation_video_player():
    ...
    def print_video_txt(frame_number: int, video_info: dict) -> None:
        ...
        cv2.putText(frame, f'F~ {frame_number}', (TextOptions.BORDER_BUFFER_X.value, int((video_info['height'] - spacing_scale))), TextOptions.FONT.value, font_size, TextOptions.COLOR.value, TextOptions.TEXT_THICKNESS.value + 1)
        cv2.putText(frame, f'T~ {current_time}', (TextOptions.BORDER_BUFFER_X.value, int((video_info['height'] - spacing_scale*2))), TextOptions.FONT.value, font_size, TextOptions.COLOR.value, TextOptions.TEXT_THICKNESS.value + 1)
            ...
            while True:
                ...
                if second_key == ord('t'): ## BACK UP TWO FRAME
                    if (current_video_position - 2) < 0:
                        FrameRangeWarning(msg=f'FRAME {current_video_position - 2} CANNOT BE SHOWN', source=annotation_video_player.__name__)
                    else:
