...
class LabellingInterface(object):
    ...
    def __init__(self,
        ...
        for target_cnt, target in enumerate(self.target_lst):
            ...
            self.checkbox[target] = Checkbutton(self.check_frame, text=target, variable=self.checkVar[target], command=lambda: self.__save_behavior_in_frm(frame_number=int(self.current_frm_n.get()), target=target))
    ...
    def __advance_frame(self, new_frm_number: int=None):
    ...
    def __save_behavior_in_frm(self, frame_number=None, target=None):
    ...
    def save_checkboxes(self):
        if self.range_on.get():
            ...
            if start_frm < 0:
                ...
                self.__advance_frame(new_frm_number=end_frm)
        else:
            for target in self.target_lst:
