...
class LabellingInterface(object):
    def __init__(self,
        ...
        '''
        Class for stardard and pseudo-labelling (annotation) interface in SimBA.
        ...
        threshold_dict: dict
            If setting == pseudo, threshold_dict dict contains the machine probability thresholds, with the classifier
            names as keys and the classification probabilities as values.
        '''
        ...
        self.checkVar, self.checkbox = {}, {}
        for target_cnt, target in enumerate(self.target_lst):
            self.checkVar[target] = IntVar()
            self.checkbox[target] = Checkbutton(self.check_frame, text=target, variable=self.checkVar[target], command=lambda: self.save_behavior_in_frm(frame_number=int(self.current_frm_n.get()), target=target))
            self.checkbox[target].grid(row=target_cnt + 1, sticky=W)
            self.checkVar[target].set(self.data_df_targets[target].iloc[self.current_frm_n.get()])
        ...
        save = Button(self.main_window, text="Save Range", command=lambda: self.save_behavior_in_range(self.first_frame.get(),self.last_frame.get()))
    ...
    def advance_frame(self, new_frm_number: int=None):
        print(new_frm_number, self.current_frm_n.get())
        if new_frm_number > self.max_frm_no:
            ...
            self.change_frm_box.insert(0, self.current_frm_n.get())
            for target in self.target_lst:
                self.checkVar[target].set(self.data_df_targets[target].loc[int(self.current_frm_n.get())])
            self.read_frm(frm_number=int(self.current_frm_n.get()))

    def save_behavior_in_range(self, start_frm=None, end_frm=None):
        if not self.range_on.get():
            ...
            for frm_no in range(int(start_frm), int(end_frm) + 1):
                for target in self.target_lst:
                    self.data_df_targets[target].loc[frm_no] = self.checkVar[target].get()
    ...
    def save_behavior_in_frm(self, frame_number=None, target=None):
        self.data_df_targets[target].loc[int(self.current_frm_n.get())] = self.checkVar[target].get()

    def save_checkboxes(self):
        if self.range_on.get():
            ...
            if start_frm < 0:
                ...
                for frm_cnt in range(start_frm, end_frm + 1):
                    for target in self.target_lst:
                        self.data_df_targets.at[frm_cnt, target] = self.checkVar[target].get()
                self.advance_frame(new_frm_number=end_frm)
        else:
            for target in self.target_lst:
                self.data_df_targets.at[int(self.current_frm_n.get()), target] = self.checkVar[target].get()
            self.current_frm_n = IntVar(value=int(self.current_frm_n.get()) + 1)
            self.change_frm_box.delete(0, END)
            self.change_frm_box.insert(0, int(self.current_frm_n.get()))
    ...
    def create_print_statements(self, frame_range: bool=None, start_frame: int=None, end_frame: int=None):
        ...
        if not frame_range:
            for target in self.target_lst:
                target_present_choice = self.checkVar[target].get()
