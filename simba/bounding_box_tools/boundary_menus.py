...
class BoundaryMenus(ConfigReader, PopUpMixin):
    ...
    def __launch_find_boundaries_pop_up(self):
        ...
        self.select_shape_type_frm = LabelFrame(self.find_boundaries_frm, text='SELECT SHAPE TYPE', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
    ...
    def __populate_find_boundaries_menu(self):
        ...
        self.boundary_settings = LabelFrame(self.find_boundaries_frm, text='BOUNDARY SETTINGS', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
    ...
    def __launch_visualize_boundaries(self):
        ...
        self.video_settings_frm = LabelFrame(self.viz_boundaries_frm, text='SETTINGS', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
        self.roi_attr_frm = LabelFrame(self.viz_boundaries_frm, text='ROI ATTRIBUTES', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
        ...
        self.run_visualize_roi = Button(self.viz_boundaries_frm, text='RUN', font=Formats.LABELFRAME_HEADER_FORMAT.value, fg='blue', command=lambda: self.__run_boundary_visualization())
        ...
        Label(self.roi_attr_frm, text='ANIMAL', font=Formats.LABELFRAME_HEADER_FORMAT.value, width=self.max_animal_name_char + 10).grid(row=0, column=0, sticky=N)
        Label(self.roi_attr_frm, text='ROI COLOR', font=Formats.LABELFRAME_HEADER_FORMAT.value, width=self.max_animal_name_char + 10).grid(row=0, column=1, sticky=N)
    ...
    def __launch_boundary_statistics(self):
        ...
        self.settings_frm = LabelFrame(self.statistics_frm, text='SETTINGS', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
        self.file_type_frm = LabelFrame(self.statistics_frm, text='OUTPUT FILE TYPE', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=15)
    ...
    def __launch_agg_boundary_statistics(self):
        ...
        if not os.path.isdir(self.data_path):
            raise NoFilesFoundError(msg='SIMBA ERROR: No anchored ROI statistics found in {}.'.format(self.anchored_roi_path))
