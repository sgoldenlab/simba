...
from simba.misc_tools import check_multi_animal_status, get_fn_ext, find_video_of_file
import os, glob
import pickle
from tkinter import *
...
class BoundaryMenus(object):
    ...
    def __launch_visualize_boundaries(self):
        ...
        self.viz_boundaries_frm.minsize(400, 150)
    ...
    def __launch_agg_boundary_statistics(self):
        ...
        self.minimum_bout_entry_box.grid(row=4, column=0, sticky=NW)
        self.run_btn.grid(row=1, column=0, sticky=NW)

    def _run_agg_stats(self):
        measures = []
        for cb, name in zip([self.interaction_time, self.interaction_bout_cnt, self.interaction_bout_mean, self.interaction_bout_median], ['INTERACTION TIME (s)', 'INTERACTION BOUT COUNT', 'INTERACTION BOUT TIME MEAN (s)', 'INTERACTION BOUT TIME MEDIAN (s)']):
            if cb.get():
        ...
        agg_stats_calculator = AggBoundaryStatisticsCalculator(config_path=self.config_path,
                                                               measures=measures,
                                                               shortest_allowed_interaction=min_bout)
