...
class AnimalBoundaryFinder(ConfigReader, FeatureExtractionMixin):
    ...
    def _save_results(self):
        ...
        stdout_success(msg=f'Animal shapes for {len(self.outlier_corrected_paths)} videos saved at {self.save_path}')
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            for animal_cnt, animal in enumerate(self.animal_bp_dict.keys()):
                print(f'Analyzing shapes in video {self.video_name} ({file_cnt+1}/{len(self.outlier_corrected_paths)}), animal {animal} ({animal_cnt+1}/{len(list(self.animal_bp_dict.keys()))})...')
                if self.roi_type == 'ENTIRE ANIMAL':
