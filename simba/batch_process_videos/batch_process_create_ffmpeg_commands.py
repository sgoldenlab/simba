...
class FFMPEGCommandCreator(object):
    ...
    '''
    Class for creating and executing FFmpeg commands from instructions stored in json format.
    ...
    def clip_videos(self):
        ...
        for video, video_data in self.videos_to_clip.items():
            ...
            command = 'ffmpeg -i {} -ss {} -t {} -async 1 -c:a copy {} -hide_banner -loglevel error'.format(in_path, start_time, time_difference, out_path)
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
