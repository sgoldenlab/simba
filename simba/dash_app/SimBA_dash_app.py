        ...
        for category in g_categories:
            if category == "VideoData":
                ...
                dict_features[b][category] = {
                    column: get_probability_data(column, videos, path)
                ...
                for sub_c in h5file.get(category).keys():
                    ...
                    if b == "Attack":
                        dict_features[b][sub_c] = {
                            column: video[column] for column in video.columns
                        }
                    else:
                        dict_features[b][sub_c] = {
                            column: video[column]
...
all_data_behaviors = get_feature_data(
    H5list_videos, list_behaviors, general_categories, filePath, hf
)
...
def create_color_pickers():
    ...
    for g in group_list:
        new_picker = [
            html.Label(g),
            dcp.ColorPicker(id="ColorPicker" + str(g), color="#609BE0"),
        ]
        output += new_picker
    ...
    children=[
                            ...
                            children=[
                                html.Div(
                                    ...
                                    children=[
                                        ...
                                        html.Label("Feature:"),
                                        dcc.Dropdown(
                                            id="feature_selector", clearable=False
                                        ),
                                        html.Div(
                                            id="multi_group_data",
                                            children=[
        ...
        html.Div(
            ...
            children=[
                ...
                dcc.Graph(
                    id="plot_ind_group", config=config, style={"display": "block"}
                ),
            ],
        ),
        html.Div(
...
def update_config(submit, height, width, filename, extension):
    if submit is not None:
        ...
        return dcc.Graph(id="plot_multi_groups", config=new_config), dcc.Graph(
            id="plot_ind_group", config=new_config
        )
    else:
        return dcc.Graph(id="plot_multi_groups", config=config), dcc.Graph(
            id="plot_ind_group", config=config
        )
...
@app.callback(
    ...
    [Input("category_selector", "value"), Input("behaviors", "value")],
)
def set_features_options(selected_category, behavior):
    return [
        {"label": f, "value": f}
        for f in list(all_data_behaviors[behavior][selected_category].keys())
    ]
...
def set_group_options(
    selected_group, selected_category, selected_behavior, selected_feature
):
    try:
        ...
        if "VideoData" in selected_category:
            ...
            try:
                list_vid = list(
                    all_data_behaviors[selected_behavior][selected_category][
                        selected_feature
                    ].index
                )
...
@app.callback(
    Output("feature_selector", "value"), [Input("feature_selector", "options")]
)
...
@app.callback(Output("category_selector", "options"), [Input("behaviors", "value")])
def set_video_option(behavior):
    if behavior == "Attack":
        return [
            {"label": c, "value": c} for c in list_categories if "Severity" not in c
        ]
    else:
...
@app.callback(
    Output("category_selector", "value"), [Input("category_selector", "options")]
)
...
@app.callback(Output("video_numbers", "value"), [Input("video_numbers", "options")])
...
@app.callback(Output("group_selector", "value"), [Input("group_selector", "options")])
def set_video_option(options):
    try:
        return options[0]["value"]
    except IndexError:
...
def set_input_xrange(
    ...
    if "VideoData" not in category:
        ...
        start_i_sec, end_i_sec = round(current_indx[0] / fps, 3), round(
            current_indx[1] / fps, 3
        )
        start_g_sec, end_g_sec = round(current_multix[0] / fps, 3), round(
            current_multix[1] / fps, 3
        )
...
def update_layout(graph, plot_title, x_axis, y_axis, show_grid, font, size, show_bg):
    ...
    graph["layout"] = go.Layout(
        ...
        xaxis={"showgrid": grid},
        yaxis={"showgrid": grid},
        font=dict(family=font, size=size),
        plot_bgcolor=bg,
    )
    return graph
...
def update_ind_graph(
    ...
    if "Probability" in feature_name:
        ...
        try:
            ...
            if len(show_mean) > 0:
                ...
                x_axis = list(
                    range(
                        all_data_behaviors[behavior][category][feature_name]
                        .eval(mean_group_name)
                        .count()
                    )
                )
                group_mean = all_data_behaviors[behavior][category][feature_name][
                    select_vids
                ].mean(axis=1)
    ...
    # Bar Graph
        ...
        try:
            for v in select_vids:
                try:
                    one_bar = [
                        all_data_behaviors[behavior][category][feature_name].loc[v]
                    ]
                except KeyError:
                    one_bar = [0]
                x_axis = [v]
    ...
    fig = update_layout(
        fig, graph_title, x_title, y_title, showgrid, font, fontsize, showbg
    )
    return fig
...
def update_multi_graph(
    ...
    if "Probability" in feature_name:
        ...
        for group in list_groups:
            ...
            one_mean_line = all_data_behaviors[behavior][category][feature_name][
                group + "_Mean"
            ]
            start_x, end_x = set_graph_viewing_range(startx, endx, x_axis)
        ...
        if len(show_group_mean) > 0:
            ...
            total_mean_line = all_data_behaviors[behavior][category][feature_name][
                "Total_Mean"
            ]
            start_x, end_x = set_graph_viewing_range(startx, endx, x_axis)
        ...
        try:
            for group in list_groups:
                try:
                    group_values = (
                        all_data_behaviors[behavior][category][feature_name]
                        .loc[groups_dict[group]]
                        .tolist()
                    )
                    mean_values, std_error_values = calculate_group_statistics(
                        group_values
                    )
                except KeyError:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        hovertext=std_error_values,
                        name=group,
                        marker_color=color_name[group],
                    )
                )
        except KeyError:
...
@app.callback(
    ...
    [Input("download_data", "n_clicks"), Input("download_group_data", "n_clicks")],
    [
