import pandas as pd
from simba.misc_tools import detect_bouts, find_core_cnt, remove_a_folder, determine_chunksize_for_imap
...
class GanttCreator(object):
    ...
    def create_gannt(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            chunksize = determine_chunksize_for_imap(data_df_len=len(self.data_df))
            print('Creating gantt images, multiprocessing (determined chunksize: {}, cores used: {})...'.format(str(chunksize), str(self.cpu_to_use)))
            with multiprocessing.pool.Pool(self.cpu_to_use) as pool:
                ...
                for cnt, result in enumerate(pool.imap(constants, frame_rng, chunksize=5)):
                    if self.video_setting:
...
# test = GanttCreator(config_path='/Users/simon/Desktop/troubleshooting/B1-MS_US/project_folder/project_config.ini',
#                     frame_setting=False,
#                     video_setting=True)
# test.create_gannt()
