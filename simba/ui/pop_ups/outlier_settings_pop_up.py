...
class OutlierSettingsPopUp(PopUpMixin, ConfigReader):
    ...
    def run(self):
        ...
        for animal_cnt, animal_name in enumerate(self.animal_bp_dict.keys()):
            self.config.set(ConfigKey.OUTLIER_SETTINGS.value, 'movement_bodyPart1_{}'.format(animal_name), self.criterion_dropdowns[animal_name]['movement_bp_1'].getChoices())
            self.config.set(ConfigKey.OUTLIER_SETTINGS.value, 'movement_bodyPart2_{}'.format(animal_name), self.criterion_dropdowns[animal_name]['movement_bp_2'].getChoices())
            self.config.set(ConfigKey.OUTLIER_SETTINGS.value, 'location_bodyPart1_{}'.format(animal_name), self.criterion_dropdowns[animal_name]['location_bp_1'].getChoices())
            self.config.set(ConfigKey.OUTLIER_SETTINGS.value, 'location_bodyPart2_{}'.format(animal_name), self.criterion_dropdowns[animal_name]['location_bp_2'].getChoices())
        with open(self.config_path, 'w') as f:
            self.config.write(f)

        stdout_success(msg='Outlier correction settings updated in the project_config.ini', source=self.__class__.__name__)
        self.root.destroy()
