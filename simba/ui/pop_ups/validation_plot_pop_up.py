...
class ValidationVideoPopUp(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        if not check_float(name=f'{self.__class__.__name__} discrimination_threshold', min_value=0.0, max_value=1.0, value=discrimination_threshold, raise_error=False)[0]:
        ...
        if not check_int(name=f'{self.__class__.__name__} shortest_bout', value=shortest_bout, min_value=0, raise_error=False)[0]:
