...
from simba.utils.checks import check_valid_lst, check_if_dir_exists, check_int
from simba.utils.errors import NoROIDataError, NoFilesFoundError
from simba.mixins.pop_up_mixin import PopUpMixin
from simba.ui.tkinter_functions import CreateLabelFrameWithIcon, SimBADropDown, Entry_Box
from simba.utils.read_write import find_files_of_filetypes_in_directory, str_2_bool
from simba.utils.enums import Links

from simba.data_processors.cue_light_movement_statistics import CueLightMovementAnalyzer
...
class CueLightMovementAnalyzerPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        if file_cnt == 0:
            raise NoFilesFoundError(msg=f'No data files exist in the {self.data_dir} directory: ANALYZE CUE LIGHT DATA before performing cue light movement statistics analysis.', source=self.__class__.__name__)
        ...
        self.bp_dropdown.grid(row=0, column=0, sticky=NW)
        self.pre_window_eb.grid(row=1, column=0, sticky=NW)
        self.post_window_eb.grid(row=2, column=0, sticky=NW)

        self.create_run_frm(run_function=self.run)

    def run(self):
        bp_name = self.bp_dropdown.get_value()
        ...
        analyzer = CueLightMovementAnalyzer(config_path=self.config_path, cue_light_names=self.cue_light_names, bp_name=bp_name, pre_window=int(pre_window), post_window=int(post_window))
        analyzer.run()
        analyzer.save()
