...
class InterpolatePopUp(PopUpMixin, ConfigReader):
    ...
    def __init__(self,
        ...
        self.type_dropdown = SimBADropDown(parent=self.settings_frm, dropdown_options=['MISSING BODY-PARTS', 'MISSING ANIMALS'], label="INTERPOLATION TYPE:", label_width=35, value='MISSING BODY-PARTS', dropdown_width=35, img='file_type', tooltip_key='INTERPOLATE_TYPE')
        self.method_dropdown = SimBADropDown(parent=self.settings_frm, dropdown_options=['NEAREST', 'LINEAR', 'QUADRATIC'], label="INTERPOLATION METHOD:", label_width=35, value='NEAREST', dropdown_width=35, img='equation_small', tooltip_key='INTERPOLATE_METHOD')
        self.save_originals_dropdown = SimBADropDown(parent=self.settings_frm, dropdown_options=['TRUE', 'FALSE'], label="SAVE ORIGINALS:", label_width=35, value='TRUE', dropdown_width=35, img='save_blue', tooltip_key='INTERPOLATE_SAVE_ORIGINALS')
        ...
        self.selected_file = FileSelect(self.single_file_frm, "DATA PATH:", lblwidth=35, file_types=[("VIDEO FILE", ".csv .parquet")], initialdir=self.project_path, lbl_icon='file', tooltip_key='INTERPOLATE_DATA_PATH')
        ...
        self.selected_dir = FolderSelect(self.multiple_file_frm, "SELECT DIRECTORY OF DATA FILES:", lblwidth=35, initialdir=self.project_path, lbl_icon='folder', tooltip_key='INTERPOLATE_DATA_DIR')
...
#InterpolatePopUp(config_path=r"E:\troubleshooting\mitra_emergence_hour\project_folder\project_config.ini")