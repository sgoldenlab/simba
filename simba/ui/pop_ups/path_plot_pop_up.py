...
class PathPlotPopUp(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        self.video_slicing_frm = LabelFrame(self.main_frm, text='SEGMENTS', font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=5)
        ...
        self.slice_cb = Checkbutton(self.video_slicing_frm, text='Plot only defined time-segment', variable=self.slice_var, command=lambda: self.enable_entrybox_from_checkbox(check_box_var=self.slice_var, entry_boxes=[self.video_start_time_entry, self.video_end_time_entry]))
        ...
        self.include_animal_names_cb = Checkbutton(self.settings_frm, text='Include animal names', variable=self.include_animal_names_var)
    ...
    def __create_path_plots(self,
        ...
        if self.autocompute_var.get():
            ...
            if self.bg_clr_dropdown.getChoices() == 'Video - static frame':
                ...
                bg_clr = {'type': 'static', 'opacity': int(''.join(c for c in self.bg_opacity_dropdown.getChoices() if c.isdigit())), 'frame_index': int(self.static_frm_index_eb.entry_get)}
            elif self.bg_clr_dropdown.getChoices() == 'Video - moving frames':
                bg_clr = {'type': 'moving', 'opacity': int(''.join(c for c in self.bg_opacity_dropdown.getChoices() if c.isdigit()))}
            else:
