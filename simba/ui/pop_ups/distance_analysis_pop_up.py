...
class DistanceAnalysisPopUp(ConfigReader, PopUpMixin):
    def __init__(self,
        ...
        self.distance_cnt_dropdown = SimBADropDown(parent=self.distance_cnt_frm, label="# OF DISTANCES", label_width=20, dropdown_width=20, value=1, dropdown_options=list(range(1, 11)), command=self.create_settings_frm, img='abacus', tooltip_key='DISTANCE_ANALYSIS_NUMBER_OF_PAIRS')
        ...
        variance_distance_cb, self.variance_distance_var = SimbaCheckbox(parent=self.measurments_frm, txt='VARIANCE DISTANCE (CM)', txt_img='spread', val=False, tooltip_key='DISTANCE_ANALYSIS_METRICS')
        ...
        variance_distance_cb.grid(row=2, column=0, sticky=NW)
        ...
        transpose_cb, self.transpose_var = SimbaCheckbox(parent=self.transpose_frm, txt='TRANSPOSE OUTPUT CSV', txt_img='rotate', val=False, tooltip_key='DISTANCE_ANALYSIS_TRANSPOSE')
    ...
    def run(self):
        ...
        mean_distance, median_distance, var_distance = self.mean_distance_var.get(), self.median_distance_var.get(), self.variance_distance_var.get()
        if not mean_distance and not median_distance and not var_distance:
