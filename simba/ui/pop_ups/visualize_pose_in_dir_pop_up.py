...
from simba.ui.tkinter_functions import (CreateLabelFrameWithIcon, DropDownMenu, FolderSelect, FileSelect)
...
class VisualizePoseInFolderPopUp(PopUpMixin):
    ...
    def __populate_menu(self, x):
        ...
        self.settings_menu = LabelFrame(self.main_frm, text="SETTINGS", font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=5, fg="black")
        ...
        multiprocess_cb = Checkbutton(self.settings_menu, text="Multi-process (faster)", variable=self.multiprocess_var, command=lambda: self.enable_dropdown_from_checkbox(check_box_var=self.multiprocess_var, dropdown_menus=[self.multiprocess_dropdown]))
        ...
        self.single_video_frm = LabelFrame(self.main_frm, text="VISUALIZE DATA FILE", font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=5, fg="black")
        ...
        self.video_dir_frm = LabelFrame(self.main_frm, text="VISUALIZE DATA DIRECTORY", font=Formats.LABELFRAME_HEADER_FORMAT.value, pady=5, padx=5, fg="black")
        ...
        self.run_folder_btn = Button(self.video_dir_frm, text="RUN", command=lambda: self.run(dir=True), fg='blue')
