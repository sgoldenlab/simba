...
from simba.ui.tkinter_functions import CreateLabelFrameWithIcon, DropDownMenu, Checkbutton
...
class ROIAnalysisTimeBinsPopUp(ConfigReader, PopUpMixin):
    ...
    def create_settings_frm(self):
        ...
        calc_distances_frm = LabelFrame(self.main_frm, text="CALCULATE DISTANCES", font=Formats.LABELFRAME_HEADER_FORMAT.value)
        self.distances_var = BooleanVar()
        self.distances_cb = Checkbutton(calc_distances_frm, text="Compute distances moved within ROIs in each time-bin", variable=self.distances_var)
    ...
    def run(self):
        check_float(name="Time bin", value=self.time_bin_entrybox.entry_get, min_value=10e-6)
        check_float(
