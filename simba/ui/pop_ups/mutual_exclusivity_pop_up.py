        ...
        for rule_cnt, rule_data in self.rules_dict.items():
            ...
            if rule_data['highest_var'].get():
                ...
                if len(rules[rule_cnt]['subordinates']) < 2:
                    raise InvalidInputError(msg=f'Less than two classifiers ticked for rule #{rule_cnt}', source=self.__class__.__name__)
                if rules[rule_cnt]['tie_breaker'] not in rules[rule_cnt]['subordinates'] and rules[rule_cnt]['tie_breaker'] is not None:
                    raise InvalidInputError(msg=f'For rule {rule_cnt}, the tie-breaker ({rules[rule_cnt]["tie_breaker"]}) is not one of the selected classifiers.', source=self.__class__.__name__)
                ...
                if rules[rule_cnt]['winner'] not in rules[rule_cnt]['subordinates']:
                    raise InvalidInputError(msg=f'For rule {rule_cnt}, the winner ({rules[rule_cnt]["winner"]}) is not one of the selected classifiers.', source=self.__class__.__name__)
