...
class CreateUserDefinedPoseConfigurationPopUp(PopUpMixin):
    def __init__(self,
        ...
        PopUpMixin.__init__(self, title="USER-DEFINED POSE CONFIGURATION", size=(700, 700), main_scrollbar=False)
    ...
    def create_bodypart_table(self):
        ...
        if self.selected_animal_cnt > 1:
            self.table_frame = LabelFrame(self.main_frm, text='Bodypart name                            Animal ID number', height=500, width=700)
        else:
    ...
    def validate_unique_entries(self,
        ...
        if duplicates:
            print(duplicates)
            raise DuplicationError(msg='SIMBA ERROR: SimBA found duplicate body-part names (see above). Please enter unique body-part (and animal ID) names.', source=self.__class__.__name__)
        else:
            pass
