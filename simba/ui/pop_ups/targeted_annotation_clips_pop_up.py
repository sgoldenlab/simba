...
class TargetedAnnotationsWClipsPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.selected_video = FileSelect(settings_frm, "Data path", title='Select a data file', lblwidth=10, file_types=[('VIDEO FILE', Options.WORKFLOW_FILE_TYPE_OPTIONS.value)])
    ...
    def show_start_stop(self):
        ...
        if hasattr(self, 'table'): self.table.destroy()
        ...
        run_button = Button(self.table, text='RUN', command=lambda: self.run(), fg='navy', font=Formats.LABELFRAME_HEADER_FORMAT.value)
    ...
    def run(self):
        ...
        machine_results_path = os.path.join(self.machine_results_dir, f'{file_name}.{self.file_type}')
        check_file_exist_and_readable(file_path=machine_results_path)
        _, _, fps = read_video_info(vid_info_df=self.video_info_df, video_name=file_name)
        video_path = find_video_of_file(video_dir=self.video_dir, filename=file_name, raise_error=True)
        start_times, end_times = [], []
        check_file_exist_and_readable(self.selected_video.file_path)
        for cnt, (start_time, end_time) in enumerate(zip(self.start_times, self.end_times)):
            start_times.append(start_time.get())
            end_times.append(end_time.get())
            check_if_string_value_is_valid_video_timestamp(name=f'Clip {cnt+1} start', value=start_time.get())
            check_if_string_value_is_valid_video_timestamp(name=f'Clip {cnt+1} end', value=end_time.get())
            check_that_hhmmss_start_is_before_end(start_time=start_time.get(), end_time=end_time.get(), name=f'Clip {cnt+1}')
            check_if_hhmmss_timestamp_is_valid_part_of_video(timestamp=start_time.get(), video_path=video_path)
            check_if_hhmmss_timestamp_is_valid_part_of_video(timestamp=end_time.get(), video_path=video_path)

        print('Preparing video and data clips...')
        temp_dir = os.path.join(self.input_frames_dir, 'advanced_clip_annotator', file_name)
        if os.path.isdir(temp_dir): remove_a_folder(folder_dir=temp_dir)
        if not os.path.isdir(temp_dir): os.makedirs(temp_dir)
        multi_split_video(file_path=video_path, start_times=start_times, end_times=end_times, out_dir=temp_dir, include_clip_time_in_filename=True)
        ...
        for cnt, (start_time, end_time) in enumerate(zip(start_times, end_times)):
            frm_numbers = find_frame_numbers_from_time_stamp(start_time=start_time, end_time=end_time, fps=fps)
            sliced = df.iloc[frm_numbers, :]
            new_start_time = start_time.replace(':', '-')
            new_end_time = start_time.replace(':', '-')
            save_path = os.path.join(temp_dir, f'{file_name}_{new_start_time}_{new_end_time}.{self.file_type}')
