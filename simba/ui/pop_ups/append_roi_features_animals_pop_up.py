...
class AppendROIFeaturesByAnimalPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        if not os.path.isfile(self.roi_coordinates_path):
            raise NoROIDataError(msg=f'SIMBA ERROR: No ROIs have been defined. Please define ROIs before appending ROI-based features (no data file found at path {self.roi_coordinates_path})', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for bp_cnt, bp_dropdown in self.body_parts_dropdowns.items():
            self.config.set(ConfigKey.ROI_SETTINGS.value, f'animal_{bp_cnt+1}_bp', bp_dropdown.getChoices())
