...
class SelectAdvancedLabellingVideoPupUp():
    ...
    """
    Launch PopUp to select video for **advanced** labelling

    :example:

    >>> SelectAdvancedLabellingVideoPupUp(config_path=r"C:\troubleshooting\mitra\project_folder\project_config.ini", continuing=False)
    """
    def __init__(self,
        ...
        if not continuing: win_title = "SELECT VIDEO FILE - NEW ADVANCED VIDEO ANNOTATION"
        else: win_title = "SELECT VIDEO FILE - CONTINUING ADVANCED VIDEO ANNOTATION"
        ...
        stdout_information(f"INITIATING ADVANCED ANNOTATION INTERFACE FOR {video_meta['video_name']} \n VIDEO INFO: {video_meta}")
