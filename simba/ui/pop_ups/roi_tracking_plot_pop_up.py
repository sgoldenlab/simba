...
from simba.utils.checks import check_float, check_file_exist_and_readable
...
class VisualizeROITrackingPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.single_video_dropdown = DropDownMenu(self.single_video_frm, "Select video", self.video_names, "15", com=lambda x: self.update_file_select_box_from_dropdown(filename=x, fileselectbox=self.select_video_file_select))
        ...
        self.single_video_btn = Button(self.single_video_frm, text="Create SINGLE ROI video", fg="blue", command=lambda: self.run(multiple=False))
        self.all_videos_frm = LabelFrame(self.run_frm,text="ALL videos",pady=10,font=Formats.LABELFRAME_HEADER_FORMAT.value,fg="black")
        self.all_videos_btn = Button(self.all_videos_frm, text=f"Create ALL ROI videos ({len(self.video_names)} videos found)",fg="red", command=lambda: self.run(multiple=True))
    ...
    def run(self, multiple: bool):
        ...
        style_attr = {'show_body_part': self.show_pose_var.get(),
        ...
        for video_path in videos:
            if not self.multiprocess_var.get():
                roi_plotter = ROIPlot(config_path=self.config_path,
