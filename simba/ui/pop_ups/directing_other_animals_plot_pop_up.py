...
class DirectingOtherAnimalsVisualizerPopUp(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        self.show_animal_names_cb = Checkbutton(self.style_settings_frm, text="Show animal names", variable=self.show_animal_names_var)
        ...
        self.run_multiple_video_btn = Button(self.run_multiple_videos, text=f"Create multiple videos ({len(list(self.files_found_dict.keys()))} video(s) found)", fg="blue", command=lambda: self.__create_directionality_plots(multiple_videos=True))
