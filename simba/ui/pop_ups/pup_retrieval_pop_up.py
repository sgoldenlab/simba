...
from simba.utils.read_write import read_config_entry, read_config_file, get_all_clf_names
...
class PupRetrievalPopUp(object):
    def __init__(self,
        ...
        self.project_path = read_config_entry(self.config, ConfigKey.GENERAL_SETTINGS.value, ConfigKey.PROJECT_PATH.value, data_type=ConfigKey.FOLDER_PATH.value)
        self.ROI_path = os.path.join(self.project_path, Paths.ROI_DEFINITIONS.value)
        if not os.path.isfile(self.ROI_path):
            raise NoFilesFoundError(msg=f'Requires ROI definitions: no file found at {self.ROI_path}', source=self.__class__.__name__)
