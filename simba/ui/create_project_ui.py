...
class ProjectCreatorPopUp(PopUpMixin):
    ...
    def __init__(self):
        ...
        parent_tab.add(
            ...
            text=f'{"[ Create project config ]": ^20s}',
            compound="left",
            image=self.btn_icons["create"]["img"],
        )
        parent_tab.add(
            self.import_videos_tab,
            text=f'{"[ Import videos ]": ^20s}',
            compound="left",
            image=self.btn_icons["video"]["img"],
        )
        parent_tab.add(
            self.import_data_tab,
            text=f'{"[ Import tracking data ]": ^20s}',
            compound="left",
            image=self.btn_icons["pose"]["img"],
        )
        parent_tab.add(
            self.extract_frms_tab,
            text=f'{"[ Extract frames ]": ^20s}',
            compound="left",
            image=self.btn_icons["frames"]["img"],
        )

        parent_tab.grid(row=0, column=0, sticky=NW)

        self.settings_frm = CreateLabelFrameWithIcon(
            parent=self.create_project_tab,
            header="SETTINGS",
            icon_name=Keys.DOCUMENTATION.value,
            icon_link=Links.CREATE_PROJECT.value,
        )
        self.general_settings_frm = LabelFrame(
            self.settings_frm,
        ...
        self.project_dir_select = FolderSelect(
            self.general_settings_frm, "Project directory:", lblwidth="25"
        )
        self.project_name_eb = Entry_Box(
            self.general_settings_frm, "Project name:", labelwidth="25"
        )
        self.file_type_dropdown = DropDownMenu(
            self.general_settings_frm,
            "Workflow file type:",
            Options.WORKFLOW_FILE_TYPE_OPTIONS.value,
            "25",
        )
        self.file_type_dropdown.setChoices(
            choice=Options.WORKFLOW_FILE_TYPE_OPTIONS.value[0]
        )

        self.clf_name_entries = []
        self.ml_settings_frm = LabelFrame(
            self.settings_frm,
        ...
        self.clf_cnt = Entry_Box(
            self.ml_settings_frm,
            "Number of classifiers (behaviors): ",
            "25",
            validation="numeric",
        )
        add_clf_btn = Button(
            self.ml_settings_frm,
            text="<Add predictive classifier(s)>",
            fg="blue",
            command=lambda: self.create_entry_boxes_from_entrybox(
                count=self.clf_cnt.entry_get,
                parent=self.ml_settings_frm,
                current_entries=self.clf_name_entries,
            ),
        )

        self.animal_settings_frm = LabelFrame(
            self.settings_frm,
            text="ANIMAL SETTINGS",
            font=Formats.LABELFRAME_HEADER_FORMAT.value,
        )
        self.tracking_type_dropdown = DropDownMenu(
            self.animal_settings_frm,
            "Type of Tracking",
            Options.TRACKING_TYPE_OPTIONS.value,
            "25",
            com=self.update_body_part_dropdown,
        )
        ...
        self.multi_tracking_option_dict = {
            k: self.bp_lu[k] for k in self.multi_tracking_options
        }
        ...
        self.selected_tracking_dropdown = DropDownMenu(
            self.animal_settings_frm,
            "Body-part config",
            Options.CLASSICAL_TRACKING_OPTIONS.value,
            "25",
        ...
        self.img_lbl = Label(
            self.animal_settings_frm,
            image=self.bp_lu[self.classical_tracking_options[0]]["img"],
        )
        reset_btn = Button(
            self.animal_settings_frm,
            text="RESET USER DEFINED POSE-CONFIGS",
            fg="red",
            command=lambda: PoseResetterPopUp(),
        )
        run_frm = Frame(master=self.settings_frm)
        create_project_btn = Button(
            run_frm,
        ...
        extract_frames_frm = LabelFrame(
            self.extract_frms_tab,
            text="EXTRACT FRAMES INTO PROJECT",
            fg="black",
            font=Formats.LABELFRAME_HEADER_FORMAT.value,
            pady=5,
            padx=5,
    ...
    def update_body_part_dropdown(self, selected_value):
        ...
        if selected_value == Methods.MULTI_TRACKING.value:
            ...
            self.selected_tracking_dropdown = DropDownMenu(
                self.animal_settings_frm,
                "Body-part config",
                self.classical_tracking_options,
                "25",
                com=self.update_img,
            )
            self.selected_tracking_dropdown.setChoices(
                self.classical_tracking_options[0]
            )
            self.selected_tracking_dropdown.grid(row=1, column=0, sticky=NW)
            ...
            self.selected_tracking_dropdown = DropDownMenu(
                self.animal_settings_frm,
                "Body-part config",
                self.three_dim_tracking_options,
                "25",
                com=self.update_img,
            )
            self.selected_tracking_dropdown.setChoices(
                self.three_dim_tracking_options[0]
            )
            self.selected_tracking_dropdown.grid(row=1, column=0, sticky=NW)
    ...
    def update_img(self, selected_value):
        if selected_value != Methods.CREATE_POSE_CONFIG.value:
            ...
            _ = CreateUserDefinedPoseConfigurationPopUp(
                master=self.main_frm, project_config_class=ProjectCreatorPopUp
            )

    def extract_frames(self):
        if not hasattr(self, "config_path"):
            raise MissingProjectConfigEntryError(
                msg="Create PROJECT CONFIG before extracting frames"
            )
    ...
    def run(self):
        ...
        config_creator = ProjectConfigCreator(
