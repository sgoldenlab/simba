...
class ImportPoseFrame(ConfigReader, PopUpMixin):
    ...
    def create_import_menu(self, data_type_choice: Literal["CSV (DLC/DeepPoseKit)", "JSON (BENTO)", "H5 (multi-animal DLC)", "SLP (SLEAP)", "CSV (SLEAP)", "H5 (SLEAP)", "TRK (multi-animal APT)", "MAT (DANNCE 3D)", "CSV (SimBA BLOB)", "CSV (SimBA YOLO)"]):
        ...
        self.interpolation_dropdown = SimBADropDown(parent=self.interpolation_frm, dropdown_options=Options.INTERPOLATION_OPTIONS_W_NONE.value, label='INTERPOLATION METHOD: ', label_width=25, dropdown_width=35, value=Options.INTERPOLATION_OPTIONS_W_NONE.value[0], img='fill', tooltip_key='IMPORT_POSE_INTERPOLATION')
        ...
        self.smoothing_dropdown = SimBADropDown(parent=self.smoothing_frm, dropdown_options=Options.SMOOTHING_OPTIONS_W_NONE.value, label='SMOOTHING: ', label_width=25, dropdown_width=35, value=Options.SMOOTHING_OPTIONS_W_NONE.value[0], command=self.__show_smoothing_entry_box_from_dropdown, img='smooth', tooltip_key='IMPORT_POSE_SMOOTHING')
        self.smoothing_time_eb = Entry_Box(self.smoothing_frm, "SMOOTHING PERIOD (MS):", labelwidth=25,  validation="numeric", entry_box_width=35, img='timer_2', justify='center', tooltip_key='IMPORT_POSE_SMOOTHING_PERIOD_MS')
        ...
        if data_type_choice in ["CSV (DLC/DeepPoseKit)", "MAT (DANNCE 3D)", "JSON (BENTO)", "CSV (SimBA BLOB)", 'H5 (FaceMap)', 'CSV (SimBA YOLO)']: # DATA TYPES WHERE NO TRACKS HAVE TO BE SPECIFIED
            ...
            self.import_directory_select = FolderSelect(self.import_directory_frm, "Input data DIRECTORY:", lblwidth=25, initialdir=self.project_path, lbl_icon='folder', tooltip_key='IMPORT_POSE_INPUT_DATA_DIRECTORY')
            ...
            self.import_file_select = FileSelect(self.import_single_frm, "Input data FILE:", lblwidth=25, file_types=[("Pose data file", FILE_TYPES[data_type_choice])], lbl_icon='file_type', tooltip_key='IMPORT_POSE_INPUT_DATA_FILE')
            ...
            if data_type_choice == "CSV (DLC/DeepPoseKit)":
                ...
                self.import_dir_btn = Button(self.import_directory_frm, fg="blue", font=Formats.FONT_REGULAR.value, text="Import YOLO CSV DIRECTORY to SimBA project", command=lambda: self.__import_yolo_data(interpolation_settings=self.interpolation_dropdown.getChoices(), smoothing_time=self.smoothing_time_eb.entry_get, data_path=self.import_directory_select.folder_path, smoothing_setting=self.smoothing_dropdown.getChoices()))
            ...
            if data_type_choice == "H5 (multi-animal DLC)":
                ...
                self.data_dir_select = FolderSelect(self.data_dir_frm, "SLP SLEAP DIRECTORY: ", lblwidth=25, lbl_icon='folder', tooltip_key='IMPORT_POSE_MULTI_ANIMAL_DATA_DIRECTORY')
                ...
                self.data_dir_select = FolderSelect(self.data_dir_frm, "TRK APT DIRECTORY: ", lblwidth=25, lbl_icon='folder', tooltip_key='IMPORT_POSE_MULTI_ANIMAL_DATA_DIRECTORY')
                ...
                self.data_dir_select = FolderSelect(self.data_dir_frm, "CSV SLEAP DIRECTORY:", lblwidth=25, lbl_icon='folder', tooltip_key='IMPORT_POSE_MULTI_ANIMAL_DATA_DIRECTORY')
                ...
                self.data_dir_select = FolderSelect(self.data_dir_frm, "H5 SLEAP DIRECTORY", lblwidth=25, lbl_icon='folder', tooltip_key='IMPORT_POSE_MULTI_ANIMAL_DATA_DIRECTORY')
                ...
                self.data_dir_select = FolderSelect(self.data_dir_frm, "H5 SuperAnimal DIRECTORY:", lblwidth=25, lbl_icon='folder', tooltip_key='IMPORT_POSE_MULTI_ANIMAL_DATA_DIRECTORY')
