...
class PoseConfigCreator(object):
    ...
    def __init__(self,
        ...
        if not isinstance(self.img, (list, tuple, np.ndarray)):
            print('SIMBA ERROR: The chosen image file could not be read as an image ({})'.format(img_path))
            raise ValueError('SIMBA ERROR: The chosen image file could not be read as an image')
