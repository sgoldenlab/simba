...
class DatasetCreator(ConfigReader, UnsupervisedMixin):
    ...
    def run(self):
        if (self.settings[Unsupervised.DATA_SLICE_SELECTION.value] == Unsupervised.ALL_FEATURES_EX_POSE.value):
            self._run(drop_bps=True, user_defined=False)
        elif (self.settings[Unsupervised.DATA_SLICE_SELECTION.value] == Unsupervised.USER_DEFINED_SET.value):
    ...
    def _read_files(self):
        ...
        for cnt, file_path in enumerate(self.machine_results_paths):
            ...
            print(f'Reading {video_name} (File {cnt + 1}/{len(self.machine_results_paths)})...')
    ...
    def _run(self, drop_bps: bool, user_defined: bool):
        ...
        self.raw_bp_df = self.raw_x_df[[Unsupervised.VIDEO.value, Unsupervised.FRAME.value] + self.bp_col_names]
        ...
        timer.stop_timer()
        stdout_success(msg='Reading data complete!', elapsed_time=timer.elapsed_time_str)
        self._clf_slicer()
        self.save()
    ...
    def __aggregate_dataset_stats(self):
        ...
        stdout_success(msg=f"Log for unsupervised learning dataset saved at {self.log_save_path}")

    def save(self):
        print('Saving SimBa unsupervised dataset...')
        if len(self.bouts_x_df) == 0:
            raise NoDataError(msg="The data contains zero frames after the selected slice setting", source=self.__class__.__name__)
        results = {}
        results["DATETIME"] = self.datetime
        results[Unsupervised.BOUT_AGGREGATION_TYPE.name] = self.settings[Unsupervised.BOUT_AGGREGATION_TYPE.value]
        results[Unsupervised.MIN_BOUT_LENGTH.name] = self.settings[Unsupervised.MIN_BOUT_LENGTH.value]
