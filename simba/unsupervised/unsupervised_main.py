...
from simba.unsupervised.pop_ups.cluster_frequentist_stats_popup import ClusterFrequentistStatisticsPopUp
...
from simba.unsupervised.pop_up_classes import (TransformClustererPopUp, TransformDimReductionPopUp)
...
class UnsupervisedGUI(ConfigReader, PopUpMixin):
    ...
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.dim_reduction_fit_btn = Button(self.dim_reduction_frm,text="DIMENSIONALITY REDUCTION: FIT",fg="blue",command=lambda: FitDimReductionPopUp(config_path=self.config_path))
        self.dim_reduction_transform_btn = Button(self.dim_reduction_frm,text="DIMENSIONALITY REDUCTION: TRANSFORM", fg="green",command=lambda: TransformDimReductionPopUp())
        ...
        self.cluster_fit_btn = Button(self.clustering_frm,text="CLUSTERING: FIT ",fg="blue",command=lambda: FitClusterModelsPopUp(config_path=self.config_path))
        self.cluster_transform_btn = Button(self.clustering_frm,text="CLUSTERING: TRANSFORM ",fg="green",command=lambda: TransformClustererPopUp())
    ...
    def create_dataset(self):
        ...
        dataset_creator = DatasetCreator(settings=settings, config_path=self.config_path)
