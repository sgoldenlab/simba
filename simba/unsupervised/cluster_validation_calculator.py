...
from simba.utils.checks import check_file_exist_and_readable, check_if_dir_exists, check_valid_extension, check_if_keys_exist_in_dict
...
class ClusterValidators(ConfigReader):
    ...
    def __init__(self,
        ...
        self.save_path = os.path.join(self.logs_path, f"{self.validator_name}_{self.datetime}.xlsx")
        with pd.ExcelWriter(self.save_path, mode="w") as writer:
            pd.DataFrame().to_excel(writer, sheet_name=" ", index=True)

    def run(self):
        print(f"Analyzing {self.validator_name} for {len(self.data_paths)} clusterer(s)...")
        self.results = {}
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            print(f"Performing {self.validator_name} for cluster model {v[Clustering.CLUSTER_MODEL.value][Unsupervised.HASHED_NAME.value]}...")
            ...
            cluster_cnt = get_unique_values_in_iterable(data=y, name=v[Clustering.CLUSTER_MODEL.value][Unsupervised.HASHED_NAME.value], min=1)
            validator_statistic = self.validator(x.astype(np.float32), y.astype(np.int64))
            ...
            self.results[file_cnt] = {**self.results[file_cnt],
                                      **{self.validator_name: validator_statistic},
                                      **{CLUSTER_COUNT: cluster_cnt}}
            model_timer.stop_timer()
            stdout_success(msg=f"DUNN INDEX complete for model {self.results[file_cnt][CLUSTERER_NAME]} ...", elapsed_time=model_timer.elapsed_time_str)
    ...
    def save(self):
        ...
        stdout_success(msg=f"ALL {self.validator_name} calculations complete and saved in {self.save_path}", elapsed_time=self.timer.elapsed_time_str)
