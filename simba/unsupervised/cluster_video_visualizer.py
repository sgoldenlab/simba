...
class ClusterVideoVisualizer(ConfigReader, UnsupervisedMixin):
    ...
    def sample_df_n_by_unique(
        self, df: pd.DataFrame, criteria: Dict[str, int]
    ) -> pd.DataFrame:
    ...
    def run(self):
        ...
        if self.max_videos != None:
            cluster_df = self.sample_df_n_by_unique(
                df=cluster_df, criteria={"CLUSTER": self.max_videos}
            )
