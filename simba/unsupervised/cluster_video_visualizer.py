...
class ClusterVideoVisualizer(ConfigReader, UnsupervisedMixin):
    ...
    def __init__(self, config_path: Union[str, os.PathLike], data_path: Union[str, os.PathLike], max_videos: Optional[Union[int, None]] = None, speed: Optional[int] = 1.0, bg_clr: Optional[Tuple[int, int, int]] = (255, 255, 255), plot_type: Optional[Literal["VIDEO", "HULL", "SKELETON", "POINTS"]] = "SKELETON"):
        ...
        check_if_keys_exist_in_dict(data=self.data, key=[Clustering.CLUSTER_MODEL.value, Unsupervised.DATA.value], name=data_path)
        self.max_videos, self.speed, self.plot_type, self.bg_clr = max_videos, speed, plot_type, bg_clr
    ...
    def run(self):
        ...
        if self.max_videos != None:
            cluster_df = sample_df_n_by_unique(df=cluster_df, field='CLUSTER', n=self.max_videos)
            ...
            for event in event_idx:
                ...
                for name, bps in self.animal_bp_cols.items():
                    if self.plot_type == "HULL":
                        ...
                        shapes.append(GeometryMixin().multiframe_bodyparts_to_polygon(data=animal_event_pose_data,
                                                                                      pixels_per_mm=1,
                                                                                      parallel_offset=1,
                                                                                      verbose=False,
                                                                                      core_cnt=-1))
                        ...
                        shapes.append(GeometryMixin().multiframe_bodyparts_to_multistring_skeleton(data_df=event_df,
                ...
                _ = GeometryMixin.geometry_video(shapes=shapes, save_path=save_path, fps=out_fps, size=(w, h), bg_img=bg_img)
