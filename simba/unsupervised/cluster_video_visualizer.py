...
class ClusterVideoVisualizer(ConfigReader, UnsupervisedMixin):
    ...
    def __init__(self,
        ...
        check_if_keys_exist_in_dict(data=self.data, key=[Clustering.CLUSTER_MODEL.value], name=ClusterVideoVisualizer.__name__)
        check_if_valid_rgb_tuple(data=bg_clr)
        if max_videos != None: check_int(name='max_videos', value=max_videos, min_value=1)
        ...
        self.cl_mdl_name = self.data[Clustering.CLUSTER_MODEL.value][Unsupervised.HASHED_NAME.value]
        self.animal_bp_cols, self.skeleton_perm = {}, {}
        self.save_dir = os.path.join(self.project_path, self.frames_output_dir, 'clusters')
        if not os.path.isdir(self.save_dir): os.makedirs(self.save_dir)
        for animal_nme in self.animal_bp_dict.keys():
            animal_bp_cols = []
            for x, y in zip(self.animal_bp_dict[animal_nme]["X_bps"], self.animal_bp_dict[animal_nme]["Y_bps"]): animal_bp_cols.extend((x, y))
    ...
    def sample_df_n_by_unique(self, df: pd.DataFrame, criteria: Dict[str, int]) -> pd.DataFrame:
        check_that_column_exist(df=df, column_name=list(criteria.keys()), file_name='')
        results = []
        for field in criteria.keys():
            for field_val in df[field].unique():
                sample = df[df[field] == field_val]
                if len(sample) < criteria[field]:
                    results.append(sample)
                else:
                    results.append(sample.sample(n=criteria[field]))
        return pd.concat(results, axis=0)

    def run(self):
        cluster_idx = self.data[Unsupervised.DATA.value][Unsupervised.BOUTS_FEATURES.value].index
        cluster_lbls = self.data[Clustering.CLUSTER_MODEL.value][Unsupervised.MODEL.value].labels_
        cluster_df = pd.DataFrame(cluster_lbls, columns=['CLUSTER'], index=cluster_idx)
        if self.max_videos != None: cluster_df = self.sample_df_n_by_unique(df=cluster_df, criteria={'CLUSTER': self.max_videos})
        for cluster_id in sorted(cluster_df['CLUSTER'].unique()):
            ...
            for event in event_idx:
                video_pose = self.data[Unsupervised.DATA.value][Unsupervised.FRAME_POSE.value].loc[event[0], :].reset_index()
                event_df = video_pose[(video_pose['FRAME'] >= event[1]) & (video_pose['FRAME'] <= event[2])].astype(np.int64)
                save_path = os.path.join(self.save_dir, f'{event[0]}_{event[1]}_{event[2]}_{cluster_id}.mp4')
                video_info, _, fps = self.read_video_info(video_name=event[0])
                out_fps = int(fps * self.speed)
                w, h = int(video_info['Resolution_width']), int(video_info['Resolution_height'].astype(int))
                ...
                for name, bps in self.animal_bp_cols.items():
                    if self.plot_type == 'HULL':
                        ...
                        animal_event_pose_data = animal_event_pose_data.reshape(len(animal_event_pose_data), -1, 2)
                        shapes.append(GeometryMixin().multiframe_bodyparts_to_polygon(data=animal_event_pose_data, pixels_per_mm=1, parallel_offset=1, verbose=False, core_cnt=-1))
                    elif self.plot_type == 'SKELETON':
                        animal_skeleton_bps = self.skeleton_perm[name]
                        shapes.append(GeometryMixin().multiframe_bodyparts_to_multistring_skeleton(data_df=event_df, skeleton=animal_skeleton_bps, core_cnt=-1, verbose=False))
                    elif self.plot_type == 'POINTS':
                        animal_event_pose_data = event_df[bps].values
                        animal_event_pose_data = animal_event_pose_data.reshape(len(animal_event_pose_data), -1, 2)
                        results = GeometryMixin().multiframe_bodypart_to_point(data=animal_event_pose_data)
                        multi_points = []
                        for frm in range(len(results)): multi_points.append(MultiPoint(results[frm]))
                        shapes.append(multi_points)
                GeometryMixin.geometry_video(shapes=shapes, save_path=save_path, fps=out_fps, size=(w, h), bg_img=bg_img)
