...
class GridSearchVisualizer(UnsupervisedMixin):
    ...
    def categorical_visualizer(self, categoricals: list):
        ...
        for k, v in data.items():
            ...
            for variable in categoricals:
                save_path = os.path.join(
                    self.save_dir,
                    f"{v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value]}_{variable}.png",
                )
        ...
        stdout_success(
            msg="All cluster images created.", elapsed_time=self.timer.elapsed_time_str
        )

    def continuous_visualizer(self, continuous_vars: list):
        data = self.read_pickle(self.model_dir)
        for k, v in data.items():
            self.check_key_exist_in_object(object=v, key=Unsupervised.DR_MODEL.value)
        for k, v in data.items():
