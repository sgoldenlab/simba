...
class GridSearchVisualizer(UnsupervisedMixin):
    ...
    def __init__(self,
        ...
        check_if_keys_exist_in_dict(data=self.settings, key=["CATEGORICAL_PALETTE", "CONTINUOUS_PALETTE", "SCATTER_SIZE"], name='')
    ...
    def categorical_visualizer(self, categorical_vars: List[str]):
        ...
        for k, v in data.items():
            check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value, Unsupervised.DATA.value], name=k)
        for k, v in data.items():
        ...
        stdout_success(msg=f"{img_cnt} categorical cluster images created.", elapsed_time=self.timer.elapsed_time_str)
    ...
    def continuous_visualizer(self, continuous_vars: List[str]):
        ...
        for k, v in data.items():
            ...
            for variable in continuous_vars:
                ...
                stdout_success(msg=f"Saved {save_path}...")
        self.timer.stop_timer()
        stdout_success(msg="All continuous cluster images created.", elapsed_time=self.timer.elapsed_time_str)
