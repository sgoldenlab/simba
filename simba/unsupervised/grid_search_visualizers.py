...
from simba.unsupervised.enums import Clustering, Unsupervised, UMLOptions
from simba.utils.checks import (check_if_dir_exists,
                                check_if_filepath_list_is_empty,
                                check_if_keys_exist_in_dict, check_instance,
                                check_that_column_exist, check_valid_lst, check_int, check_str)
...
class GridSearchVisualizer(UnsupervisedMixin):
    ...
    def __init__(self, model_dir: Union[str, os.PathLike], save_dir: Union[str, os.PathLike], settings: Dict[str, Any]):
        super().__init__()
        ...
        check_if_keys_exist_in_dict(data=self.settings, key=["CATEGORICAL_PALETTE", "CONTINUOUS_PALETTE", "SCATTER_SIZE"], name=f'{self.__class__.__name__} settings')
        check_if_filepath_list_is_empty(filepaths=self.data_paths, error_msg=f"SIMBA ERROR: No pickle files found in {model_dir}")
        check_int(name=f'{self.__class__.__name__} scatter size', value=self.settings['SCATTER_SIZE'], min_value=1)
        check_str(name=f'{self.__class__.__name__} categorical palette', value=self.settings['CATEGORICAL_PALETTE'], options=Options.PALETTE_OPTIONS_CATEGORICAL.value)
        check_str(name=f'{self.__class__.__name__} continuous palette', value=self.settings['CONTINUOUS_PALETTE'], options=Options.PALETTE_OPTIONS.value)
    ...
    def __extract_plot_data(self, data: dict):
        ...
        bouts_data = data[Unsupervised.DATA.value][Unsupervised.BOUTS_FEATURES.value].reset_index()
    ...
    def categorical_visualizer(self, categorical_vars: List[str]):
        check_valid_lst(data=categorical_vars, source=self.__class__.__name__, valid_dtypes=(str,), min_len=1)
        print(f'Creating {len(categorical_vars)} categorical plot(s) from {len(self.data_paths)} data files...')
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            for variable in categorical_vars:
                ...
                _ = PlottingMixin.categorical_scatter(data=data, columns=('X', 'Y', variable), palette=self.settings["CATEGORICAL_PALETTE"], size=self.settings["SCATTER_SIZE"], save_path=save_path, show_box=False)
                stdout_success(msg=f"Saved {save_path}...")
                plt.close("all")
        self.timer.stop_timer()
        stdout_success(msg=f"{int(len(categorical_vars) * len(self.data_paths))} categorical plot(s) created.", elapsed_time=self.timer.elapsed_time_str)

    def continuous_visualizer(self, continuous_vars: List[str]):
        check_valid_lst(data=continuous_vars, source=self.__class__.__name__, valid_dtypes=(str,), min_len=1)
        print(f'Creating {len(continuous_vars)} categorical plot(s) from {len(self.data_paths)} data files...')
        for file_cnt, file_path in enumerate(self.data_paths):
            v = read_pickle(data_path=file_path)
            check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value, Unsupervised.DATA.value], name=file_path)
            data = self.__extract_plot_data(data=v)
            for variable in continuous_vars:
                check_that_column_exist(df=data,column_name=variable,file_name=v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value])
                save_path = os.path.join(self.save_dir,f"{v[Clustering.CLUSTER_MODEL.value][Unsupervised.HASHED_NAME.value]}_{variable}.png")
                _ = PlottingMixin.continuous_scatter(data=data, columns=('X', 'Y', variable), palette=self.settings["CONTINUOUS_PALETTE"], size=self.settings["SCATTER_SIZE"], save_path=save_path, show_box=False)
                plt.close("all")
                stdout_success(msg=f"Saved {save_path}...")
        self.timer.stop_timer()
        stdout_success(msg=f"{int(len(continuous_vars) * len(self.data_paths))} continuous plot(s) created.", elapsed_time=self.timer.elapsed_time_str)
