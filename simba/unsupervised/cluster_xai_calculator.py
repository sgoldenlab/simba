    ...
    def __shap_values(self):
        if self.settings[SHAP][METHOD] == PAIRED:
            ...
            for cluster_one_id, cluster_two_id in cluster_combinations:
                ...
                cluster_one_sample["target"] = 1
                cluster_two_sample["target"] = 0
                x_df = pd.concat(
                    [cluster_one_sample, cluster_two_sample], axis=0
                ).reset_index(drop=True)
                y_df = x_df.pop("target")
                shap_df, _ = TrainModelMixin().create_shap_log_mp(
                ...
                results_cluster_two = (
                    ...
                    .drop(
                        ["target", "Expected_value", "Sum", "Prediction_probability"],
                        axis=0,
                    )
                )
                results_cluster_one = (
            ...
            for cluster_id, cluster_mdl in mdls.items():
                ...
                self.__save_results(
                    df=shap_df, name=f"SHAP CLUSTER {cluster_id} vs. ALL"
                )
                timer.stop_timer()
                stdout_success(
