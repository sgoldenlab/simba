...
class GridSearchVisualizerPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.data_dir_select = FolderSelect(data_frm, "DATA DIRECTORY:", lblwidth=25, initialdir=self.project_path)
        self.save_dir_select = FolderSelect(data_frm, "OUTPUT DIRECTORY: ", lblwidth=25, initialdir=self.project_path)
    ...
    def run(self):
        ...
        grid_search_visualizer = GridSearchVisualizer(model_dir=self.data_dir_select.folder_path, save_dir=self.save_dir_select.folder_path, settings=settings)
        if len(continuous_vars) > 0:
            threading.Thread(target=grid_search_visualizer.continuous_visualizer(continuous_vars=continuous_vars)).start()
        if len(categorical_vars) > 0:
            threading.Thread(target=grid_search_visualizer.categorical_visualizer(categorical_vars=categorical_vars)).start()
