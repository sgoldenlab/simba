...
class ClusterVisualizerPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.plot_type_dropdown = DropDownMenu(self.data_frm, 'PLOT TYPE:', plt_types, "25", com= lambda x: self._activate_video(selection=x))
        self.plot_type_dropdown.setChoices('SKELETON')
        self.dataset_file_selected = FileSelect(self.data_frm,"CLUSTER DATASET (PICKLE): ",lblwidth=25, initialdir=self.project_path, file_types=[("SimBA model", f"*.{Formats.PICKLE.value}")])
        ...
        self.max_vid_per_cluster = DropDownMenu(self.settings_frm, "MAX VIDEOS PER CLUSTER:", max_vid_lst, "25")
        ...
        self.bg_clr_dropdown = DropDownMenu(self.settings_frm, "BACKGROUND COLOR:", list(self.bg_clr_dict.keys()), "25")
    ...
    def _activate_video(self, selection):
        if selection == 'VIDEO':
            self.videos_dir_select = FolderSelect(self.data_frm, "VIDEOS DIRECTORY:", lblwidth=15, initialdir=self.video_dir)
    ...
    def run(self):
        ...
        if max_videos == 'None': max_videos = None
        else: max_videos = int(max_videos)
        plt_type = self.plot_type_dropdown.getChoices()
        bg_clr = self.color_dict[self.bg_clr_dropdown.getChoices()]

        cluster_visualizer = ClusterVideoVisualizer(config_path=self.config_path,
                                                    data_path=data_path,
                                                    plot_type=plt_type,
                                                    bg_clr=bg_clr,
