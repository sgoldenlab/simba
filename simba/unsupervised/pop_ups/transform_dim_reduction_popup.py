...
class TransformDimReductionPopUp(PopUpMixin, ConfigReader):
    def __init__(self,
        ...
        self.model_select = FileSelect(self.dim_reduction_frm, "MODEL (PICKLE):", lblwidth=25, file_types=[('SimBA model pickle', f'.{Formats.PICKLE.value}')], initialdir=self.project_path)
        self.dataset_select = FileSelect(self.dim_reduction_frm, "DATASET (PICKLE):", lblwidth=25, file_types=[('SimBA model pickle', f'.{Formats.PICKLE.value}')], initialdir=self.project_path)
        self.save_dir = FolderSelect(self.dim_reduction_frm, "SAVE DIRECTORY: ", lblwidth=25, initialdir=self.project_path)
        self.additional_data = DropDownMenu(self.dim_reduction_frm,"ADDITIONAL DATA:",UMLOptions.UMAP_ADDITIONAL_DATA.value,"25")
    ...
    def run(self):
        ...
        if settings_choice == 'SCALED FEATURES':
            ...
            settings['DATA_FORMAT'] = 'scaled'; settings['CLASSIFICATIONS'] = True
        elif settings_choice == 'RAW FEATURES & CLASSIFICATIONS':
            settings['DATA_FORMAT'] = 'RAW'; settings['CLASSIFICATIONS'] = True

        umap_searcher = UmapEmbedder()
        threading.Thread(target=umap_searcher.transform(data_path=self.dataset_select.file_path, model=self.model_select.file_path, save_dir=self.save_dir.folder_path, settings=settings)).start()
