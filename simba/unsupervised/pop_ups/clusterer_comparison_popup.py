...
class ClustererComparisonPopUp(PopUpMixin, ConfigReader):
    ...
    def __init__(self, config_path: str):
        ...
        self.data_dir_select = FolderSelect(data_frm, "DATA DIRECTORY (PICKLES):", lblwidth=25, initialdir=self.project_path)
        data_frm.grid(row=0, column=0, sticky=NW)
        self.data_dir_select.grid(row=0, column=0, sticky=NW)
        self.cb_dict = self.create_cb_frame(main_frm=self.main_frm, cb_titles=[ADJ_MUTUAL_INFO, FOWLKES_MALLOWS, ADJ_RAND_INDEX], frm_title='STATISTICS')
    ...
    def run(self):
        ...
        for k, v in self.cb_dict.items():
            if v.get(): stats_selections.append(k)
        if len(stats_selections) == 0:
            raise CountError(msg='Select at least ONE measurement.', source=self.__class__.__name__)
        cluster_comparer = ClustererComparisonCalculator(config_path=self.config_path, data_dir=self.data_dir_select.folder_path, statistics=[x.lower() for x in stats_selections])
