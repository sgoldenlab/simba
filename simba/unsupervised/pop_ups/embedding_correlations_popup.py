...
from simba.unsupervised.embedding_correlation_calculator import EmbeddingCorrelationCalculator
...
class EmbedderCorrelationsPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.data_file_selected = FileSelect(self.data_frm, "DATASET (PICKLE):", initialdir=self.project_path, file_types=[('SimBA model (pickle', f'*.{Formats.PICKLE.value}')])
        ...
        self.plot_correlation_clr_dropdown = DropDownMenu(self.settings_frm, "PLOT PALETTE:", Options.PALETTE_OPTIONS.value, "25")
    ...
    def run(self):
        ...
        calculator = EmbeddingCorrelationCalculator(config_path=self.config_path,data_path=self.data_file_selected.file_path,settings=settings)
