...
class ClusterFrequentistStatisticsPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.model_select = FileSelect(self.data_frm, "CLUSTERER PATH:", lblwidth=25, initialdir=self.project_path, file_types=[('Cluster model pickle', f'*.{Formats.PICKLE.value}')])
    ...
    def run(self):
        ...
        calculator = ClusterFrequentistCalculator(config_path=self.config_path,data_path=self.model_select.file_path,settings=settings)
        calculator.run()
