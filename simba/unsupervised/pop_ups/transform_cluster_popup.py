...
class TransformClustererPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.model_select = FileSelect(self.settings_frm, "CLUSTER MODEL (PICKLE):", lblwidth=25, file_types=[('SimBA model pickle', f'.{Formats.PICKLE.value}')], initialdir=self.project_path)
        self.data_select = FileSelect(self.settings_frm, "DATASET (PICKLE):", lblwidth=25, file_types=[('SimBA data pickle', f'.{Formats.PICKLE.value}')], initialdir=self.project_path)
        self.save_dir = FolderSelect(self.settings_frm, "SAVE DIRECTORY:", lblwidth=25, initialdir=self.project_path)
        self.additional_data_drpdwn = DropDownMenu(self.settings_frm, "INCLUDE FEATURES:", UMLOptions.UMAP_ADDITIONAL_DATA.value, "25")
    ...
    def run(self):
        ...
        threading.Thread(
            target=clusterer.transform(data_path=self.data_select.file_path,
