...
from simba.utils.checks import (check_if_dir_exists, check_if_filepath_list_is_empty, check_file_exist_and_readable)
from simba.utils.enums import Formats
from simba.utils.printing import stdout_success, SimbaTimer

VALIDATION_METRICS = ['DENSITY-BASED CLUSTER VALIDATION (DBCV)', 'DUNN INDEX', 'DAVIS-BUILDIN INDEX', 'CALINSKI-HARABASZ SCORE']
...
class ClusterValidatorPopUp(PopUpMixin, ConfigReader, UnsupervisedMixin):
    def __init__(self,
        ...
        self.folder_selected = FolderSelect(self.data_frm, "DATASETS (DIRECTORY WITH PICKLES):", lblwidth=35, initialdir=self.project_path)
        self.data_frm.grid(row=0, column=0, sticky=NW)
        self.folder_selected.grid(row=0, column=0, sticky=NW)
        self.cb_settings = self.create_cb_frame(main_frm=self.main_frm, cb_titles=VALIDATION_METRICS, frm_title="VALIDATION METHODS")
    ...
    def run(self):
        ...
        data_paths = glob.glob(self.folder_selected.folder_path + f"/*.{Formats.PICKLE.value}")
        check_if_filepath_list_is_empty(filepaths=data_paths, error_msg=f"No pickle files found in {self.folder_selected.folder_path}")
        selections = []
        ...
        for selection in selections:
            if selection == DBCV:
                ...
                if selection == DUNN:
                    ...
                    raise InvalidInputError(msg=f'Validator algorithm {selection} is not a valid options. Options: {VALIDATION_METRICS}', source=self.__class__.__name__)
                print(f'Running {selection}...')
                calculator = ClusterValidators(config_path=self.config_path, data_path=self.folder_selected.folder_path, validator_func=validator_func)
                calculator.run()
                calculator.save()
        timer.stop_timer()
        stdout_success(msg='Cluster validations complete!', source=timer.elapsed_time_str)
