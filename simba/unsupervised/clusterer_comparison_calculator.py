...
from simba.utils.checks import check_if_dir_exists, check_file_exist_and_readable, check_valid_lst, check_if_keys_exist_in_dict
from simba.utils.read_write import find_files_of_filetypes_in_directory, read_pickle, get_fn_ext
...
class ClustererComparisonCalculator(ConfigReader, Statistics):
    ...
    def __init__(self,
        ...
        self.data_paths = find_files_of_filetypes_in_directory(directory=data_dir, extensions=[f'.{Formats.PICKLE.value}'])
        if len(self.data_paths) < 2: raise CountError(msg=f'Cluster comparisons require at least two models. Found {len(self.data_paths)} in {data_dir}', source=self.__class__.__name__)
        check_valid_lst(data=statistics, source=self.__class__.__name__, valid_dtypes=(str,), valid_values=STATS_OPTIONS)
        self.statistics, self.config_path, self.data_dir = statistics, config_path, data_dir
        self.save_path = os.path.join(self.logs_path, f'clusterer_comparisons_{self.datetime}.{Formats.XLXS.value}')
        with pd.ExcelWriter(self.save_path, mode="w") as writer: pd.DataFrame().to_excel(writer, sheet_name=" ", index=True)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            check_if_keys_exist_in_dict(data=self.data_, key=[Unsupervised.METHODS.value, Clustering.CLUSTER_MODEL.value], name=file_path)
            mdl_name =  self.data_[Clustering.CLUSTER_MODEL.value][Unsupervised.HASHED_NAME.value]
            cluster_data = self.data_[Clustering.CLUSTER_MODEL.value][Unsupervised.MODEL.value].labels_
            self.data[mdl_name] = cluster_data
            obs_cnts[mdl_name] = cluster_data.shape[0]

        counts = list(set([v for k, v in obs_cnts.items()]))
        if len(counts) > 1: raise CountError(msg=f'Cluster comparisons require models built using the data. Found different number of observations in the different mdoels in {self.data_dir}. {obs_cnts}', source=self.__class__.__name__)
        ...
        stdout_success(f'Cluster comparisons saved at {self.save_path}', elapsed_time=self.timer.elapsed_time_str)
