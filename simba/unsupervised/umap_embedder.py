...
class UmapEmbedder(UnsupervisedMixin):
    ...
    def fit(self, data_path: str, save_dir: str, hyper_parameters: dict):
        ...
        if hyper_parameters[Unsupervised.VARIANCE.value] > 0:
            self.low_var_cols = self.find_low_variance_fields(
                data=self.umap_df,
                variance=hyper_parameters[Unsupervised.VARIANCE.value],
            )
        ...
        self.scaled_umap_data = self.scaler_transform(
    ...
    def transform(
        ...
        self.umap_df = deepcopy(data[Unsupervised.BOUTS_FEATURES.value]).set_index(
            [
                Unsupervised.VIDEO.value,
                Unsupervised.START_FRAME.value,
                Unsupervised.END_FRAME.value,
        ...
        self.check_expected_fields(
            data_fields=list(self.scaled_umap_data.columns),
        ...
        self.results = pd.DataFrame(
            model[Unsupervised.DR_MODEL.value][Unsupervised.MODEL.value].transform(
                self.scaled_umap_data
            ),
            columns=["X", "Y"],
            index=self.umap_df.index,
        )
