...
class UmapEmbedder(UnsupervisedMixin):
    ...
    def fit(self, data_path: Union[str, os.PathLike], save_dir: [str, os.PathLike], hyper_parameters: dict):
        check_file_exist_and_readable(file_path=data_path)
        check_that_directory_is_empty(directory=save_dir)
        self.data_path = data_path
        self.data = self.read_pickle(data_path=data_path)
        self.umap_df = deepcopy(self.data[Unsupervised.BOUTS_FEATURES.value]).reset_index().set_index([Unsupervised.VIDEO.value, Unsupervised.START_FRAME.value, Unsupervised.END_FRAME.value])
    ...
    def __fit_umaps(self):
        for cnt, h in enumerate(self.search_space):
            ...
            self.model[Unsupervised.PARAMETERS.value] = {Unsupervised.N_NEIGHBORS.value: h[0],Unsupervised.MIN_DISTANCE.value: h[1],Unsupervised.SPREAD.value: h[2]}
    ...
    def __save(self, data: dict) -> None:
        self.write_pickle(data=data, save_path=os.path.join(self.save_dir, f"{self.model[Unsupervised.HASHED_NAME.value]}.{Formats.PICKLE.value}"))
    ...
    def transform(self,
                  ...
                  save_format: Optional[Literal['csv', 'pickle']] = 'csv') -> Union[None, pd.DataFrame]:
        ...
        if save_dir is not None:
            ...
            check_str(name=f'{UmapEmbedder.transform.__name__} save_format', value=save_format, options=('csv', 'pickle',))
        if isinstance(model, str):
            check_file_exist_and_readable(file_path=model)
            model = read_pickle(data_path=model)
        data = read_pickle(data_path=data_path)
        check_if_keys_exist_in_dict(data=data, key=[Unsupervised.BOUTS_FEATURES.value, Unsupervised.BOUTS_TARGETS.value], name=data_path)
        check_if_keys_exist_in_dict(data=model, key=[Unsupervised.METHODS.value], name=data_path)
        ...
        umap_df = self.drop_fields(data=umap_df, fields=model[Unsupervised.METHODS.value][Unsupervised.LOW_VARIANCE_FIELDS.value])

        scaled_umap_data = TrainModelMixin.scaler_transform(data=umap_df, scaler=model[Unsupervised.METHODS.value][Unsupervised.SCALER.value])
        scaled_umap_data = self.drop_fields(data=scaled_umap_data, fields=model[Unsupervised.METHODS.value][Unsupervised.LOW_VARIANCE_FIELDS.value])
        self.check_expected_fields(data_fields=list(scaled_umap_data.columns), expected_fields=model[Unsupervised.METHODS.value][Unsupervised.FEATURE_NAMES.value])
        results = pd.DataFrame(model[Unsupervised.DR_MODEL.value][Unsupervised.MODEL.value].transform(scaled_umap_data), columns=["X", "Y"], index=umap_df.index)
        if settings is not None:
            check_instance(source=f'{UmapEmbedder.transform.__name__} settings', instance=settings, accepted_types=(dict,))
            ...
            if 'CLASSIFICATIONS' in settings.keys():
                target_data = data[Unsupervised.BOUTS_TARGETS.value].set_index(['VIDEO', 'START_FRAME', 'END_FRAME'])
                results = results.join(target_data, how='inner')
        if save_dir:
            if save_format == Formats.CSV.value:
                save_path = os.path.join(save_dir, f"Transformed_{mdl_name}_{self.datetime}.{Formats.CSV.value}")
                results.to_csv(path_or_buf=save_path)
            else:
                save_path = os.path.join(save_dir, f"Transformed_{mdl_name}_{self.datetime}.{Formats.PICKLE.value}")
                write_pickle(data=results, save_path=save_path)
            timer.stop_timer()
            stdout_success(msg=f"Transformed data saved at {save_path} (elapsed time: {timer.elapsed_time_str}s)")
        else:
            timer.stop_timer()
            stdout_success(msg=f"Data transformed using UMAP model {mdl_name} (elapsed time: {timer.elapsed_time_str}s)")
            return results
