...
from simba.utils.read_write import (find_files_of_filetypes_in_directory, read_pickle, write_pickle, get_fn_ext)
...
class DataExtractor(UnsupervisedMixin, ConfigReader):
    ...
    def __init__(self,
        ...
        if len(data_types) == 0:
            raise InvalidInputError(msg=f'data_types is an empty list. Accepted options: {UMLOptions.DATA_TYPES.value}.', source=self.__class__.__name__)
        if len(invalid_dtypes) > 0:
            raise InvalidInputError(msg=f'Found invalid data types: {invalid_dtypes}. Accepted: {UMLOptions.DATA_TYPES.value}.', source=self.__class__.__name__)
        self.save_dir = os.path.join(self.logs_path, f'extracted_unsupervised_model_data_{self.datetime}')
        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            for data_type in self.data_types:
                if data_type == CLUSTERER_PARAMETERS:
                    ...
                    save_path = os.path.join(save_subdir, f"cluster_parameters_{mdl_name}.json")
                    json.dump(v[Clustering.CLUSTER_MODEL.value][Unsupervised.PARAMETERS.value], open(save_path, "w"), indent=4, sort_keys=True)
                    print(f"Saved cluster parameters for model {mdl_name} at {save_path}...")
                    ...
                    save_path = os.path.join(save_subdir, f"dimensionality_reduction_parameters_{mdl_name}.json")
                    json.dump(v[Unsupervised.DR_MODEL.value][Unsupervised.PARAMETERS.value], open(save_path, "w"), indent=4, sort_keys=True)
                    print(f"Saved dimension reduction parameters for model {mdl_name} at {save_path} ...")
            #
                    ...
                    check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value, Unsupervised.METHODS.value], name=file_path,)
                    save_path = os.path.join(save_subdir, f"low_variance_fields_{mdl_name}.csv")
                    out_df = pd.DataFrame(data=v[Unsupervised.METHODS.value][Unsupervised.LOW_VARIANCE_FIELDS.value], columns=["FIELD_NAMES"],)
                    out_df.to_csv(save_path)
                    print(f"Saved low variance fields for model {mdl_name} at {save_path}...")
                    ...
                    save_path = os.path.join(save_subdir, f"bout_targets_{mdl_name}.csv")
                    v[Unsupervised.DATA.value][Unsupervised.BOUTS_TARGETS.value].to_csv(
                        save_path
                    )
                    print(f"Saved bout target data for model {mdl_name} at {save_path}...")
                    ...
                    check_if_keys_exist_in_dict(
                        data=v,
                        key=[Unsupervised.DR_MODEL.value, Clustering.CLUSTER_MODEL.value],
                        name=file_path,
                    )
                    ...
                    raise InvalidInputError(msg=f"Invalid datatype {data_type}.", source=self.__class__.__name__)

            file_timer.stop_timer()
            stdout_success(msg=f'{mdl_name} model data extraction complete', elapsed_time=file_timer.elapsed_time)
        self.timer.stop_timer()
        stdout_success(msg=f'Data for {len(self.data_paths)} model(s) extracted', elapsed_time=self.timer.elapsed_time)
