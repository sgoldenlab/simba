...
from simba.utils.read_write import write_pickle, read_pickle
    ...
    def run(self):
        if self.data_type == CLUSTERER_PARAMETERS:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Clustering.CLUSTER_MODEL.value], name=DataExtractor.__name__)
            ...
            stdout_success(msg=f"Cluster parameters for {len(list(self.data.keys()))} model(s) saved at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)


        elif self.data_type == DIMENSIONALITY_REDUCTION_PARAMETERS:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value], name=DataExtractor.__name__)
                ...
                print(f"Dimension reduction parameters for model {mdl_name} saved at ...")
            self.timer.stop_timer()
            stdout_success(msg=f"Dimension reduction parameters for {len(list(self.data.keys()))} model(s) saved at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Scaler(s) saved for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Scaled data saved for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Low variance fields for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Feature names for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Frame-wise features for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)

        elif self.data_type == FRAME_POSE:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value], name=DataExtractor.__name__)
                mdl_name = v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value]
                save_path = os.path.join(self.logs_path, f"frame_wise_pose_estimation_{mdl_name}.csv")
                v[Unsupervised.DATA.value][Unsupervised.FRAME_POSE.value].to_csv(save_path)
                print(f"Frame-wise pose saved for model {mdl_name}...")
            self.timer.stop_timer()
            stdout_success(msg=f"Frame-wise pose for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)

        elif self.data_type == FRAME_TARGETS:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value], name=DataExtractor.__name__)
                mdl_name = v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value]
                save_path = os.path.join(self.logs_path, f"frame_wise_target_data_{mdl_name}.csv")
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Frame-wise target data for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)

        elif self.data_type == BOUTS_FEATURES:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value], name=DataExtractor.__name__)
                mdl_name = v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value]
                save_path = os.path.join(self.logs_path, f"bout_features_data_{mdl_name}.csv")
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Bout features data for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Bout target data for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)

        elif self.data_type == BOUTS_DIM_CORDS:
            for k, v in self.data.items():
                check_if_keys_exist_in_dict(data=v, key=[Unsupervised.DR_MODEL.value], name=DataExtractor.__name__)
                mdl_name = v[Unsupervised.DR_MODEL.value][Unsupervised.HASHED_NAME.value]
                save_path = os.path.join(self.logs_path, f"bout_dim_cords_{mdl_name}.csv")
                mdl_data = v[Unsupervised.DR_MODEL.value][Unsupervised.MODEL.value].embedding_.astype(np.float32)
                idx = v[Unsupervised.METHODS.value][Unsupervised.SCALED_DATA.value].index
                mdl_data = pd.DataFrame(data=mdl_data, index=idx, columns=['X', 'Y'])
                mdl_data.to_csv(save_path)
                print(f"Saved bout dimensionality reduction data for model {mdl_name}...")
            self.timer.stop_timer()
            stdout_success(msg=f"Bout dimensionality reduction data for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
            ...
            self.timer.stop_timer()
            stdout_success(msg=f"Bout cluster label data for {len(list(self.data.keys()))} model(s) at {self.logs_path}!", elapsed_time=self.timer.elapsed_time_str)
