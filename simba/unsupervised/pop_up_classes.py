...
from simba.unsupervised.cluster_statistics import ClusterStatisticsCalculator
...
class GridSearchVisualizationPopUp(object):
    ...
    def run(self):
        ...
        grid_search_visualizer = GridSearchVisualizer(embedders_path=embedders_path,
...
class TransformDimReductionPopUp(object):
    def __init__(self):
        ...
        self.dim_reduction_model = FileSelect(self.dim_reduction_frm, 'MODEL (PICKLE):')
        self.dim_reduction_dataset = FileSelect(self.dim_reduction_frm, 'DATASET (PICKLE):')
        self.save_dir = FolderSelect(self.dim_reduction_frm, "SAVE DIRECTORY: ")
...
class FitClusterModelsPopUp(object):
    def __init__(self):
        ...
        self.dimensionality_reduction_data_selected = FolderSelect(self.cluster_dataset_frm, "DATASET DIRECTORY: ")
...
class TransformClustererPopUp(object):
    def __init__(self):
        ...
        self.dim_reduction_model_select = FileSelect(self.transform_clusterer_frm, 'DIMENSIONALITY REDUCTION MODEL (PICKLE):')
        self.cluster_model_select = FileSelect(self.transform_clusterer_frm, 'CLUSTER MODEL (PICKLE):')
        self.dataset_select = FileSelect(self.transform_clusterer_frm, 'DATASET (PICKLE):')
        self.save_dir = FolderSelect(self.transform_clusterer_frm, "SAVE DIRECTORY: ")
        self.feature_option_dropdown = DropDownMenu(self.transform_clusterer_frm, 'INCLUDE FEATURES:', Options.UNSUPERVISED_FEATURE_OPTIONS.value, '25')

        run_frm = LabelFrame(self.main_frm, text='RUN', font=Formats.LABELFRAME_HEADER_FORMAT.value, padx=5, pady=5, fg='black')
        run_btn = self.create_btn = Button(run_frm, text='RUN', fg='blue', command=lambda: self.run())

        self.transform_clusterer_frm.grid(row=0, column=0, sticky=NW)
        self.dim_reduction_model_select.grid(row=0, column=0, sticky=NW)
        self.cluster_model_select.grid(row=1, column=0, sticky=NW)
        self.save_dir.grid(row=2, column=0, sticky=NW)
        self.feature_option_dropdown.grid(row=3, column=0, sticky=NW)

        run_frm.grid(row=1, column=0, sticky=NW)
        run_btn.grid(row=0, column=0, sticky=NW)

    def run(self):
        embedder_path = self.dim_reduction_model_select.file_path
        clusterer_path = self.cluster_model_select.file_path
        data_path = self.dataset_select.file_path
        save_dir = self.save_dir.folder_path
        settings = {'include_features': False, 'scaled_features': False, 'save_format': None}
        if self.feature_option_dropdown.getChoices() == Options.UNSUPERVISED_FEATURE_OPTIONS.value[0]:
            settings['include_features'] = True
        if self.feature_option_dropdown.getChoices() == Options.UNSUPERVISED_FEATURE_OPTIONS.value[1]:
            settings['include_features'] = True
            settings['scaled_features'] = True

        _ = HDBSCANTransform(embedding_model_path=embedder_path,
                             clusterer_model_path=clusterer_path,
                             data_path=data_path,
                             save_dir=save_dir,
                             settings=settings)
...
class ClusterFrequentistStatisticsPopUp(object):
    ...
    def run(self):
        ...
        descriptive_calculator = ClusterStatisticsCalculator(config_path=self.config_path,
