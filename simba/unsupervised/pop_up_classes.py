...
class FitDimReductionPopUp(PopUpMixin, ConfigReader):
    def __init__(self):
        ...
        self.dataset_file_selected = FileSelect(
            self.dataset_frm, "DATASET (PICKLE):", lblwidth=25
        )
...
class TransformDimReductionPopUp(PopUpMixin):
    def __init__(self):
        ...
        self.model_select = FileSelect(
            self.dim_reduction_frm, "MODEL (PICKLE):", lblwidth=25
        )
        self.dataset_select = FileSelect(
            self.dim_reduction_frm, "DATASET (PICKLE):", lblwidth=25
        )
        self.save_dir = FolderSelect(
            self.dim_reduction_frm, "SAVE DIRECTORY: ", lblwidth=25
        )
        self.features_dropdown = DropDownMenu(
            self.dim_reduction_frm,
            "INCLUDE FEATURES:",
            UMLOptions.DATA_FORMATS.value,
            "25",
        )
...
class FitClusterModelsPopUp(PopUpMixin):
    def __init__(self):
        ...
        self.data_dir_selected = FolderSelect(
            self.dataset_frm, "DATA DIRECTORY (PICKLES): ", lblwidth=25
        )
        self.save_frm = LabelFrame(
            self.main_frm,
            text="SAVE",
...
class TransformClustererPopUp(PopUpMixin):
    def __init__(self):
        ...
        self.model_select = FileSelect(
            self.settings_frm, "CLUSTER MODEL (PICKLE):", lblwidth=25
        )
        self.data_select = FileSelect(
            self.settings_frm, "DATASET (PICKLE):", lblwidth=25
        )
        self.save_dir = FolderSelect(self.settings_frm, "SAVE DIRECTORY:", lblwidth=25)
        self.features_dropdown = DropDownMenu(
            self.settings_frm, "INCLUDE FEATURES:", UMLOptions.DATA_FORMATS.value, "25"
        )
        self.features_dropdown.setChoices(UMLOptions.DATA_FORMATS.value[0])
        self.save_format_dropdown = DropDownMenu(
    ...
    def run(self):
        ...
        check_if_dir_exists(self.save_dir.folder_path)
        settings = {
            Unsupervised.DATA.value: self.features_dropdown.getChoices(),
...
class ClusterVisualizerPopUp(PopUpMixin, ConfigReader):
    def __init__(self, config_path: str):
        ...
        self.videos_dir_select = FolderSelect(
            self.data_frm, "VIDEOS DIRECTORY:", lblwidth=25
        )
        self.dataset_file_selected = FileSelect(
