...
def drop_bp_cords(df: pd.DataFrame,
    ...
    """
    Removes pose-estimation data from dataframe.
...
def bodypartConfSchematic():
    """Returns the names body-part schematics of all pose-estimation schemas in SimBA installation"""
...
def checkDirectionalityCords(animal_bp_dict: dict):
    """
    Helper to check of ear and nose body-parts are present within the pose-estimation data.
...
def createColorListofList(no_animals: int,
                          map_size: int):
    """
    Creates a list of lists of bgr colors. Each list is a different palette.
...
def get_workflow_file_format(config: configparser.ConfigParser):
    """
    Helper to read the workflow file format in SimBA project.
