...
class OutlierCorrecterLocationAdvanced(ConfigReader, FeatureExtractionMixin):
    ...
    def _save_data_log(self, data_log: dict):
        ...
        stdout_success(msg=f'Location outlier log saved at {save_path}')
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_files):
            ...
            stdout_success(msg=f'Location outliers complete for video {self.video_name}.', elapsed_time=video_timer.elapsed_time_str)
            data_log[self.video_name] = video_log

        self._save_data_log(data_log=data_log)
        stdout_success(msg=f'{len(self.data_files)} video(s) corrected for location outliers. Saved in {self.outlier_corrected_dir}')
