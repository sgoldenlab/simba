...
from simba.misc_tools import get_video_meta_data, remove_a_folder
...
class FrameMergererFFmpeg(object):
    ...
    def __init__(self,
        ...
        if concat_type == 'horizontal':
            self.horizontal_concatenator(out_path=self.output_path, frames_dict=self.frame_types, final_img=True)
        elif concat_type == 'vertical':
            self.vertical_concatenator(out_path=self.output_path, frames_dict=self.frame_types, final_img=True)
        elif concat_type == 'mosaic':
            self.mosaic_concatenator(output_path=self.output_path, frames_dict=self.frame_types, final_img=True)
        elif concat_type == 'mixed_mosaic':
            self.mixed_mosaic_concatenator()
    ...
    def horizontal_concatenator(self, frames_dict: dict, out_path: str, include_resize=True, final_img=True):
        if include_resize: self.resize_height(new_height=self.video_height)
        video_path_str = ''
    ...
    def vertical_concatenator(self, frames_dict: dict, out_path: str, include_resize=True, final_img=True):
        if include_resize: self.resize_width(new_width=self.video_width)
        video_path_str = ''
        for video_type in frames_dict.keys():
            video_path_str += ' -i "{}"'.format(os.path.join(self.temp_dir, video_type + '.mp4'))
    ...
    def mosaic_concatenator(self, frames_dict: dict, output_path: str, final_img:bool):
        self.resize_width_and_height(new_width=self.video_width, new_height=self.video_height)
        if not self.even_bool:
            self.create_blank_video()
        ...
        self.horizontal_concatenator(frames_dict=upper_dict, out_path=os.path.join(self.temp_dir, 'upper.mp4'), include_resize=False, final_img=False)
        self.horizontal_concatenator(frames_dict=lower_dict, out_path=os.path.join(self.temp_dir, 'lower.mp4'), include_resize=False, final_img=False)
        frames_dict = {'upper': os.path.join(self.temp_dir, 'upper.mp4'), 'lower': os.path.join(self.temp_dir, 'lower.mp4')}
        self.vertical_concatenator(frames_dict=frames_dict, out_path=output_path, include_resize=False, final_img=final_img)

    def mixed_mosaic_concatenator(self):
        ...
        self.mosaic_concatenator(frames_dict=self.frame_types, output_path=output_path, final_img=False)
        self.frame_types = large_mosaic_dict
        self.resize_height(new_height=get_video_meta_data(video_path=output_path)['height'])
        ...
        self.horizontal_concatenator(frames_dict=self.frame_types, out_path=self.output_path, include_resize=False, final_img=True)
