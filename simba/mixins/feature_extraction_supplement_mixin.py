...
class FeatureExtractionSupplemental(FeatureExtractionMixin):
    ...
    def velocity_aggregator(config_path: Union[str, os.PathLike],
        ...
        check_if_filepath_list_is_empty(filepaths=file_paths,
        ...
        for file_cnt, file_path in enumerate(file_paths):
            ...
            frm_dist = FeatureExtractionMixin().framewise_euclidean_distance(location_1=data_df.values,
                                                                             location_2=shifted_df.values,
                                                                             px_per_mm=px_per_mm,
                                                                             centimeter=True).astype(np.float32)
            rolling = TimeseriesFeatureMixin.sliding_descriptive_statistics(data=frm_dist, window_sizes=np.array([1.0]),
                                                                            sample_rate=int(fps),
                                                                            statistics=typed.List(['sum'])).flatten()
        ...
        rolling_results.to_csv(os.path.join(config.logs_path, f'rolling_velocities_{config.datetime}.csv'))
        if ts_plot:
            sns.set(style="whitegrid")
            plt.figure(figsize=(10, 6))
            sns.lineplot(data=rolling_results, palette="tab10", linewidth=2.5)
            plt.savefig(os.path.join(config.logs_path, f'rolling_velocities_{config.datetime}.png'))
        timer.stop_timer()
        stdout_success(
