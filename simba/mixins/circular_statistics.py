...
class CircularStatisticsMixin(object):
    ...
    def direction_three_bps(nose_loc: np.ndarray,
        ...
        for i in prange(nose_loc.shape[0]):
            left_ear_to_nose = np.arctan2(nose_loc[i][0] - left_ear_loc[i][0], left_ear_loc[i][1] - nose_loc[i][1])
            right_ear_nose = np.arctan2(nose_loc[i][0] - right_ear_loc[i][0], right_ear_loc[i][1] - nose_loc[i][1])
    ...
    def direction_two_bps(anterior_loc: np.ndarray,
                          posterior_loc: np.ndarray) -> np.ndarray:
    ...
    def fit_circle(data: np.ndarray, max_iterations: int) -> np.ndarray:
        ...
        for i in range(data.shape[0]):
            ...
            Zi = Xi * Xi + Yi * Yi
            Mxx, Mxy, Mxz = (Xi * Xi).sum() / n, (Xi * Yi).sum() / n, (Xi * Zi).sum() / n
            ...
            results[i, :] = np.array(
                [Xcenter + x.mean(), Ycenter + y.mean(), np.sqrt(abs(Xcenter ** 2 + Ycenter ** 2 + Mz))])
