...
class GeometryMixin(object):
    ...
    def bucket_img_into_grid_square(bucket_size_mm: float,
        ...
        .. warning::
           If a whole row / column of the specific bucket / bin size cannot be fit, then the rightmost column and/or bottom row will be omitted as in the below image.

            .. image:: _static/img/bucket_img_into_grid_square_2.png
              :width: 500
              :align: center
        ...
        h_bin_cnt, v_bin_cnt = int(img_size[0] / bin_size_px), int(img_size[1] / bin_size_px)
        polygons = {}
        for i in range(h_bin_cnt):
            for j in range(v_bin_cnt):
                x1, y1 = i * bin_size_px, j * bin_size_px
                x2, y2 = x1 + bin_size_px, y1 + bin_size_px
                polygons[(i, j)] = Polygon([(x1, y1), (x2, y1), (x2, y2), (x1, y2)])
        return polygons, round((v_bin_cnt / h_bin_cnt), 3)
