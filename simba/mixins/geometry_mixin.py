...
from shapely.ops import cascaded_union, linemerge, split, unary_union
...
class GeometryMixin(object):
    ...
    def multiframe_minimum_rotated_rectangle(
        self, shapes: List[Polygon], core_cnt: int = -1
    ) -> List[Polygon]:
        """
    ...
    def extend_line_to_bounding_box_edges(
        ...
        if x1 == x2:
            intersection_points = np.array(
                [[x1, max(min_y, 0)], [x1, min(max_y, 50)]]
            ).astype(np.float32)
        elif y1 == y2:
            ...
            intersection_points = np.array(
                [
                    [(min_y - intercept) / slope, min_y],
                    [(max_y - intercept) / slope, max_y],
                ]
            ).astype(np.float32)
    ...
    def line_split_bounding_box(
        ...
        .. note::
          Extended line can be found from body-parts using ``GeometryMixin().extend_line_to_bounding_box_edges``.
