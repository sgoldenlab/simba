...
class GeometryMixin(object):
    ...
    def mulifrm_compute_pct_shape_overlap(self,
        ...
        if len(shape_1) != len(shape_2):
            raise InvalidInputError(msg=f'shape_1 and shape_2 are unequal sizes: {len(shape_1)} vs {len(shape_2)}', source=GeometryMixin.mulifrm_compute_pct_shape_overlap.__name__)
        input_dtypes = list(set([type(x) for x in shape_1] + [type(x) for x in shape_2]))
        if len(input_dtypes) > 1:
            raise InvalidInputError(msg=f'shape_1 and shape_2 contains more than 1 dtype {input_dtypes}', source=GeometryMixin.mulifrm_compute_pct_shape_overlap.__name__)
        elif not isinstance(shape_1[0], Polygon):
            raise InvalidInputError(msg=f'The shape input is not Polygons: {input_dtypes}', source=GeometryMixin.mulifrm_compute_pct_shape_overlap.__name__)
