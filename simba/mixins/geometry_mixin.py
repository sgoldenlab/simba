...
class GeometryMixin(object):
    ...
    def get_geometry_brightness_intensity(self,
                                          img: np.ndarray,
        ...
        check_instance(source=f'{GeometryMixin.get_geometry_brightness_intensity.__name__} img', instance=img, accepted_types=np.ndarray)
        check_instance(source=f'{GeometryMixin.get_geometry_brightness_intensity.__name__} roi_vertices', instance=roi_vertices, accepted_types=np.ndarray)
        check_if_valid_input(name=rois_type, input=rois_type, options=['rectangles', 'circle', 'polygons'])
    ...
    def geometry_histocomparison(self,
        ...
        check_iterable_length(f'{GeometryMixin.geometry_histocomparison.__name__} imgs', val=len(imgs), min=2)
        if method not in GeometryEnum.HISTOGRAM_COMPARISON_MAP.value.keys(): raise InvalidInputError(
            msg=f'Method {method} is not supported. Accepted options: {GeometryEnum.HISTOGRAM_COMPARISON_MAP.value.keys()}', source=GeometryMixin.geometry_histocomparison.__name__)
        for i in range(len(imgs)): check_instance(source=f'{GeometryMixin.geometry_histocomparison.__name__} imgs {i}', instance=imgs[i], accepted_types=np.ndarray)
        if shapes:
            check_instance(source=f'{GeometryMixin.geometry_histocomparison.__name__} shapes', instance=shapes, accepted_types=list)
            if len(imgs) != len(shapes): raise CountError(msg=f'Images and shapes have to be the same size. imgs size: {len(imgs)}, shapes size: {len(shapes)}', source=GeometryMixin.geometry_histocomparison.__name__)
            for i in range(len(shapes)): check_instance(source=f'{GeometryMixin.geometry_histocomparison.__name__} shapes {i}', instance=shapes[i], accepted_types=(Polygon, MultiPolygon))
            ...
            for img_cnt, img in enumerate(imgs):
                ...
                roi_img = img[y:y + h, x:x + w].copy()
                mask = np.zeros(roi_img.shape[:2], np.uint8)
                cv2.drawContours(mask, [shared_shape_arr - shared_shape_arr.min(axis=0)], -1, (255, 255, 255), -1,
                                 cv2.LINE_AA)
                bg = np.ones_like(roi_img, np.uint8)
                cv2.bitwise_not(bg, bg, mask=mask)
                roi_img = bg + cv2.bitwise_and(roi_img, roi_img, mask=mask)
