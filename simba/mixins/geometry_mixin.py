...
class GeometryMixin(object):
    ...
    def cascaded_union(shapes: List[Polygon]) -> Polygon:
        ...
        # new_pol = Polygon(ops.cascaded_union(shapes).
    ...
    @staticmethod
    def difference(shape = Union[LineString, Polygon],
        ...
        >>> difference = GeometryMixin().difference(shape = polygon_1, overlap_shapes=[polygon_2])
        """
    ...
    @staticmethod
    def union(shapes: Union[LineString, Polygon, MultiPolygon]) -> Union[MultiPolygon, Polygon]:
        """

        """
        for shape in shapes:
            check_instance(source=GeometryMixin.union.__name__, instance=shape, accepted_types=(LineString, Polygon, MultiPolygon))
        check_iterable_length(source=GeometryMixin.union.__name__, val=len(shapes), min=2)
        return unary_union(shapes)


    @staticmethod
    def view_shapes(shapes: List[Union[LineString, Polygon, MultiPolygon]]):
        ...
        for i in shapes:
            check_instance(source=GeometryMixin.view_shapes.__name__, instance=i, accepted_types=(LineString, Polygon, MultiPolygon))
...
# union = GeometryMixin().union(shapes=[polygon_1, polygon_3])

#diff = GeometryMixin().difference(shape = polygon_1, overlap_shapes=[polygon_2, polygon_3])

#
# img = GeometryMixin.view_shapes(shapes=[union])
#
