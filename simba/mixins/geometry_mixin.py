...
class GeometryMixin(object):
    ...
    def bodyparts_to_line(data: np.ndarray,
        ...
        if buffer is not None:
            check_int(name=f'{GeometryMixin.bodyparts_to_line} buffer', value=buffer, min_value=1)
            check_float(name=f'{GeometryMixin.bodyparts_to_line} px_per_mm', value=px_per_mm, min_value=1)
            area = buffer * px_per_mm
        else:
            area = None
    ...
    def multiframe_bodyparts_to_line(self,
        ...
        results = []
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.LARGE_MAX_TASK_PER_CHILD.value) as pool:
            constants = functools.partial(GeometryMixin.bodyparts_to_line,
                                          buffer=buffer,
                                          px_per_mm=px_per_mm)
            for cnt, result in enumerate(pool.imap(constants, data, chunksize=1)):
                results.append(result)
    ...
    def _multifrm_geometry_histocomparison_helper(frm_index: np.ndarray,
        ...
        for frm_range_idx in range(frm_index.shape[0]):
            ...
            if shape_type == 'circle':
                ...
                loc = loc.reshape(2, int(loc.shape[1] / 2), 2)
                shape_1 = GeometryMixin().bodyparts_to_line(data=loc[0], buffer=parallel_offset, px_per_mm=pixels_per_mm)
                shape_2 = GeometryMixin().bodyparts_to_line(data=loc[1], buffer=parallel_offset, px_per_mm=pixels_per_mm)
    ...
    def bucket_img_into_grid_hexagon(bucket_size_mm: float,
        ...
        timer.stop_timer()
        stdout_success(msg='Bucket image into hexagon grid complete', elapsed_time=timer.elapsed_time_str)
        return polygons, round((v_hex_cnt[0] / h_hex_cnt[0]), 3)


    @staticmethod
    def _cumsum_coord_geometries_helper(data: np.ndarray,
    ...
    def cumsum_coord_geometries(self,
        ...
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.LARGE_MAX_TASK_PER_CHILD.value) as pool:
            constants = functools.partial(self._cumsum_coord_geometries_helper,
    ...
    @staticmethod
    def _cumsum_animal_geometries_grid_helper(data: np.ndarray,
    ...
    def cumsum_animal_geometries_grid(self,
        ...
        if core_cnt == -1: core_cnt = find_core_cnt()[0]
        w, h = 0, 0
        for k in grid.keys(): w, h = max(w, k[0]), max(h, k[1])
        frm_id = np.arange(0, len(data)).reshape(-1, 1)
        data = np.hstack((frm_id, np.array(data).reshape(-1, 1)))
        img_arr = np.zeros((data.shape[0], h+1, w+1))
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.LARGE_MAX_TASK_PER_CHILD.value) as pool:
            constants = functools.partial(self._cumsum_animal_geometries_grid_helper,
                                          grid=grid,
                                          size=(h, w),
                                          verbose=verbose)
            for cnt, result in enumerate(pool.imap(constants, data, chunksize=1)):
