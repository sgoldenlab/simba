...
class GeometryMixin(object):
    ...
    @staticmethod
    def bucket_img_into_grid_square(bucket_size_mm: float,
        ...
        .. image:: _static/img/bucket_img_into_grid_square.png
           :width: 500
           :align: center

        .. warning::
           If a whole row / column of the specific bucket / bin size cannot be fit, then the rightmost column and/or bottom row will be omitted unless add_correction equals True.

        :param float bucket_size_mm: The width/height of each square bucket in millimeters. E.g., 50 will create 5cm by 5cm squares.
        ...
        :param Optional[bool] add_correction: If True, performs correction by adding extra columns or rows to cover any remaining space. Default True.
        :return Tuple[Dict[Tuple[int, int], Polygon], float]: First value dictionary where keys are (row, column) indices of the bucket, and values are Shapely Polygon objects representing the corresponding square buckets. Second value is the aspect ratio of the square grid.

        :example:
        >>> polygons = GeometryMixin.bucket_img_into_grid_square(bucket_size_mm=10, img_size=(800, 600), px_per_mm=5.0)
        """

        check_float(name=f'{GeometryMixin.bucket_img_into_grid_square.__name__} bucket_size_mm', value=bucket_size_mm)
        ...
        check_float(name=f'{GeometryMixin.bucket_img_into_grid_square.__name__} px_per_mm', value=px_per_mm)
        bin_size_px = int(px_per_mm * bucket_size_mm)
    ...
    @staticmethod
    def _cumsum_geometries_helper(data: np.ndarray, rectangles: Dict[Tuple[int, int], Polygon]):
...
# polygons = GeometryMixin.bucket_img_into_grid_square(bucket_size_mm=5, img_size=(img.shape[1], img.shape[0]) , px_per_mm=5.0)[0]
