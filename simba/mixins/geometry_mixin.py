...
import pandas as pd
from shapely.geometry import Polygon, LineString, Point, MultiPolygon, MultiLineString, GeometryCollection, MultiPoint
from shapely.ops import unary_union, linemerge, cascaded_union, split, triangulate
...
class GeometryMixin(object):
        ...
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.MAX_TASK_PER_CHILD.value) as pool:
            ...
            for cnt, mp_return in enumerate(pool.imap(constants, data, chunksize=1)):
                if not video_name:
                    ...
                    print(f'Computing polygon {cnt + 1}/{data.shape[0]} (Video: {video_name})...')
    ...
    def multifrm_compute_pct_shape_overlap(self,
                                          ...
                                          names: Optional[Tuple[str]] = None) -> List[float]:
        """
        ...
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.MAX_TASK_PER_CHILD.value) as pool:
            for cnt, result in enumerate(pool.imap(GeometryMixin.compute_pct_shape_overlap, data, chunksize=1)):
                if not names:
                    ...
                    print(f'Computing percent overlap {cnt + 1}/{data.shape[0]} (Shape 1: {names[1]}, Shape 2: {names[2]}, Video: {names[0]}...')
                results.append(result)
    ...
    def multiframe_minimum_rotated_rectangle(self,
        ...
        with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.MAX_TASK_PER_CHILD.value) as pool:
            for cnt, result in enumerate(pool.imap(GeometryMixin.minimum_rotated_rectangle, shapes, chunksize=1)):
                if not video_name:
                    print(f'Computing minimum bounding rectangle {cnt+1}/{len(shapes)}...')
                else:
                    print(f'Computing minimum bounding rectangle {cnt + 1}/{len(shapes)} (Video: {video_name})...')
                results.append(result)
