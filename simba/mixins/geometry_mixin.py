...
class GeometryMixin(object):
    ...
    @staticmethod
    def view_shapes(shapes: List[Union[LineString, Polygon, MultiPolygon]]) -> np.ndarray:
        ...
        for shape_cnt, shape in enumerate(shapes):
            ...
            if isinstance(shape, MultiPolygon):
                for polygon_cnt, polygon in enumerate(shape.geoms):
                    ...
                    cv2.polylines(img, [polygon_np], True, (colors[shape_cnt+ polygon_cnt + 4][::-1]),thickness=2)
        return img
