...
from simba.utils.enums import Labelling, Formats
...
class AnnotatorMixin(ConfigReader):
    ...
    def __init__(self,
        ...
        self.save_path = os.path.join(self.targets_folder, f'{self.video_name}.{self.file_type}')
        self.max_frm_no, self.min_frm_no  = max(self.original_data.index), min(self.original_data.index)
    ...
    def video_frm_label(self,
        ...
        if frm_number > self.video_meta_data['frame_count']:
            raise FrameRangeError(msg=f'Cannot display frame {frm_number}. Video contains {self.video_meta_data["frame_count"]} frames.')
    ...
    def h_nav_bar(self,
                  ...
                  previous_next_clf: Optional[bool] = False) -> None:
        ...
        if previous_next_clf:
            ...
            next_absent_btn = Button(next_clf_btn_frm, text="NEXT ABSENT", fg='red', command=lambda: self.find_proximal_annotated_frm(forwards=True, present=False))
            next_present_btn = Button(next_clf_btn_frm, text="NEXT PRESENT", fg='blue', command=lambda: self.find_proximal_annotated_frm(forwards=True, present=True))
            last_absent_btn = Button(next_clf_btn_frm, text="PREVIOUS ABSENT", fg='red', command=lambda: self.find_proximal_annotated_frm(forwards=False, present=False))
            last_present_btn = Button(next_clf_btn_frm, text="PREVIOUS PRESENT", fg='blue', command=lambda: self.find_proximal_annotated_frm(forwards=False, present=True))
    ...
    def v_navigation_pane_targeted(self,
        ...
        def bind_shortcut_keys(parent):
            ...
            parent.bind('<Control-a>', lambda x: self.change_frame(new_frm_id=int(self.frm_box.entry_get)+1, keep_radio_btn_choices=True))
    ...
    def targeted_frames_selection_pane(self,
        ...
        for target_cnt, target in enumerate(self.target_lst):
            self.clf_radio_btns[target] = StringVar(value=0)
            Label(pane, text=target, font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=target_cnt+6, column=0, sticky=NW)
            present = Radiobutton(pane, text=None, variable=self.clf_radio_btns[target], value=1, command=None)
            absent = Radiobutton(pane, text=None, variable=self.clf_radio_btns[target], value=0, command=None)
    ...
    def advanced_labelling_pane(self,
                                parent: Frame,
                                data: Optional[Dict[str, List[int]]] = None,
                                loc: Optional[Tuple[int]] = (0, 1)) -> None:

        pane = Frame(parent, bd=2)
        Label(pane, text="SELECT BEHAVIOR:", font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=0, column=0, sticky=N)
        Label(pane, text=f"PRESENT", font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=1, column=1, pady=20, sticky=NW)
        Label(pane, text=f"ABSENT", font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=1, column=2, pady=20, sticky=NW)
        Label(pane, text=f"NONE", font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=1, column=3, pady=20, sticky=NW)
        self.results = deepcopy(data)
        self.clf_radio_btns = {}
        for target_cnt, target in enumerate(self.target_lst):
            self.clf_radio_btns[target] = IntVar(value=self.get_annotation_of_frame(frm_num=int(self.frm_box.entry_get), clf=target, allowed_vals=Labelling.VALID_ANNOTATIONS_ADVANCED.value))
            Label(pane, text=target, font=Formats.LABELFRAME_HEADER_FORMAT.value).grid(row=target_cnt+6, column=0, sticky=NW)
            present = Radiobutton(pane, text=None, variable=self.clf_radio_btns[target], value=1, command=lambda: self.store_annotation())
            absent = Radiobutton(pane, text=None, variable=self.clf_radio_btns[target], value=0, command=lambda: self.store_annotation())
            none = Radiobutton(pane, text=None, variable=self.clf_radio_btns[target], value=2, command=lambda: self.store_annotation())
            present.grid(row=target_cnt+6, column=1, pady=5, sticky=NW)
            absent.grid(row=target_cnt+6, column=2, pady=5, sticky=NW)
            none.grid(row=target_cnt + 6, column=3, pady=5, sticky=NW)
        self.save_button = Button(pane, text=f"SAVE DATA", fg='green', font=("Helvetica", 18, "bold"), anchor="center", command= lambda: self.save_data())
        self.save_button.grid(row=6 + len(self.target_lst), column=0, pady=100, sticky=S)
        pane.grid(row=loc[0], column=loc[1], sticky=NW)

    def get_annotation_of_frame(self, frm_num: int, clf: str, allowed_vals: Optional[List[Union[int, None]]] = None) -> Union[int, None]:
        ...
        if int(frm_num) not in self.results[clf]:
            raise FrameRangeError(msg=f'You are annotating frame {frm_num} with is not a frame in the dataset (row count: {self.max_frm_no}', source=self.__class__.__name__)
        if allowed_vals is not None:
            if self.results[clf][frm_num] not in allowed_vals:
                raise InvalidInputError(msg=f'Found labelling value {self.results[clf][frm_num]}. Allowed: {allowed_vals}')
        else:
    ...
    def change_frame(self,
                     ...
                     update_funcs: Optional[List[Callable[[int], None]]] = None,
                     store_funcs: Optional[List[Callable[[], None]]] = None,
                     keep_radio_btn_choices: Optional[bool] = False) -> None:
    ...
    def find_proximal_annotated_frm(self, forwards: bool, present: bool):
        ...
        target_frames = set()
        if present:
            for clf in self.clf_names: target_frames = target_frames.union(set([k for k, v in self.results[clf].items() if v == 1]))
        else:
            for clf in self.clf_names: target_frames = target_frames.union(set([k for k, v in self.results[clf].items() if v == 0]))
        if forwards:
            target_frames = [x for x in list(target_frames) if x > int(self.frm_box.entry_get)]
            if len(target_frames) == 0:
                if present:
                    raise FrameRangeError(msg='No LATER frames with behavior annotated as PRESENT found', source=self.__class__.__name__)
                else:
                    raise FrameRangeError(msg='No LATER frames with behavior annotated as ABSENT found', source=self.__class__.__name__)

        else:
            target_frames = [x for x in list(target_frames) if x < int(self.frm_box.entry_get)]
            if len(target_frames) == 0:
                if present:
                    raise FrameRangeError(msg='No PRIOR frames with behavior annotated as PRESENT found', source=self.__class__.__name__)
                else:
                    raise FrameRangeError(msg='No PRIOR frames with behavior annotated as ABSENT found', source=self.__class__.__name__)
    ...
    def save_data(self):
        ...
        for target_col in results.columns:
            self.original_data = self.original_data.drop([target_col], axis=1, errors='ignore')
