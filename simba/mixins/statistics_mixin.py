...
class Statistics(FeatureExtractionMixin):
    ...
    def rolling_independent_sample_t(
        ...
        for cnt, i in enumerate(prange(1, len(data))):
            ...
            pooled_std = np.sqrt(
                (
                    (len(data[i - 1]) - 1) * stdev_1**2
                    + (len(data[i]) - 1) * stdev_2**2
                )
    ...
    @staticmethod
    @njit("(float32[:], float32[:])")
    def spearman_rank_correlation(sample_1: np.ndarray, sample_2: np.ndarray):
        """
        :example:
        >>> sample_1 = np.array([7, 2, 9, 4, 5, 6, 7, 8, 9]).astype(np.float32)
        >>> sample_2 = np.array([1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5]).astype(np.float32)
    ...
    def sliding_pearsons_r(
        ...
        :returns np.ndarray: 2d array of Pearsons R of size len(sample_1) x len(time_windows). Note, if sliding window is 10 frames, the first 9 entries will be filled with -1.
        ...
        for i in prange(time_windows.shape[0]):
            ...
            for left, right in zip(
                ...
                s1, s2 = sample_1[left : right - 1], sample_2[left : right - 1]
    ...
    def chi_square(
        ...
        sample_2: np.ndarray,
        critical_values: Optional[np.ndarray] = None,
        type: Optional[str] = "goodness_of_fit",
...
# data_1, data_2 = np.random.normal(loc=10, scale=2, size=10).astype(np.float64), np.random.normal(loc=20, scale=2, size=10).astype(np.float64)
# data = np.hstack([data_1, data_2])
