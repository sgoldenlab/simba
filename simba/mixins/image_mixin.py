...
class ImageMixin(object):
    ...
    def adjust_geometries(geometries: List[Polygon],
        ...
        check_instance(source=f'{ImageMixin().adjust_geometries.__name__} geometries', instance=geometries, accepted_types=list)
        if len(geometries) == 0:
            raise CountError(msg='Geometry list is empty', source=ImageMixin().adjust_geometries.__name__)
        for i in geometries: check_instance(source=f'{ImageMixin().adjust_geometries.__name__} geometries {i}', instance=geometries[i], accepted_types=Polygon)
        results = []
