...
import functools
from numba import njit, uint8, prange, int64, jit
...
class ImageMixin(object):
    ...
    def img_emd(imgs: List[np.ndarray] = None,
        ...
        if (img_1 is not None) and (img_2 is not None):
            ...
            if imgs[0].ndim > 2: img_1 = cv2.cvtColor(imgs[0], cv2.COLOR_BGR2GRAY)
            if imgs[1].ndim > 2: img_2 = cv2.cvtColor(imgs[1], cv2.COLOR_BGR2GRAY)
    ...
    @staticmethod
    def _slice_shapes_in_video_file_helper(data: np.ndarray, video_path: Union[str, os.PathLike]):
        cap = cv2.VideoCapture(video_path)
    ...
    def slice_shapes_in_imgs(self,
        ...
        if isinstance(imgs, np.ndarray):
            ...
            if shapes.shape[0] != imgs.shape[0]: raise ArrayError(msg=f'The image array ({imgs.shape[0]}) and shapes array ({shapes.shape[0]}) have unequal length.', source=ImageMixin().slice_shapes_in_imgs.__name__)
        else:
        ...
        if isinstance(imgs, np.ndarray):
            with multiprocessing.Pool(core_cnt, maxtasksperchild=Defaults.LARGE_MAX_TASK_PER_CHILD.value) as pool:
                for cnt, result in enumerate(pool.imap(self._slice_shapes_in_imgs_array_helper, zip(imgs, shapes), chunksize=1)):
                    results.append(result)
        else:
