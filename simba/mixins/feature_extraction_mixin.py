...
class FeatureExtractionMixin(object):
    ...
    @staticmethod
    def windowed_frequentist_distribution_tests(data: np.ndarray,
                                                feature_name: str,
    ...
    # @staticmethod
    # @jit(nopython=True)
    # def _relative_distances(distances: np.ndarray,
    ...
    def change_in_bodypart_euclidean_distance(self,
    ...
    def insert_default_headers_for_feature_extraction(self,
        ...
        if len(headers) != len(df.columns):
            raise CountError(f'Your SimBA project is set to using the default {pose_config} pose-configuration. '
                             ...
                             f'within file {filename} file, SimBA found {str(len(df.columns))} columns.', source=self.__class__.__name__)
        else:
            df.columns = headers
            return df
