...
class ConfigReader(object):
    ...
    def __init__(self,
        ...
        self.input_frames_dir = os.path.join(self.project_path, Paths.INPUT_FRAMES_DIR.value)
        ...
        self.body_part_directionality_df_dir = os.path.join(self.project_path, Paths.BODY_PART_DIRECTIONALITY_DF_DIR.value)
        ...
        self.frames_output_dir = os.path.join(self.project_path, Paths.FRAMES_OUTPUT_DIR.value)
        ...
        self.directing_body_part_animal_video_output_path = os.path.join(self.project_path, Paths.DIRECTING_BETWEEN_ANIMAL_BODY_PART_OUTPUT_PATH.value)
        ...
        self.bodypart_direction = self.read_config_entry(self.config, section=ConfigKey.DIRECTIONALITY_SETTINGS.value, option=ConfigKey.BODYPART_DIRECTION_VALUE.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value)
        ...
        self.body_part_directionality_paths = glob.glob(self.body_part_directionality_df_dir + f"/*.{self.file_type}")
        self.outlier_corrected_paths = glob.glob(self.outlier_corrected_dir + f'/*.{self.file_type}')
        self.outlier_corrected_movement_paths = glob.glob(self.outlier_corrected_movement_dir + f'/*.{self.file_type}')
        ...
        if create_logger: self.create_logger()
    ...
    def get_number_of_header_columns_in_df(self,
        ...
        for i in range(len(df)):
            try:
                ...
                pass
        raise DataHeaderError(msg='Could find the count of header columns in dataframe. All values appear to be non-numeric', source=self.__class__.__name__)
    ...
    def find_video_of_file(self,
        ...
        try:
            ...
            raise NoFilesFoundError(msg=f'No files found in the {video_dir} directory', source=self.__class__.__name__)
        ...
        if return_path is None and not raise_error:
            NoFileFoundWarning(msg=f'SimBA could not find a video file representing {filename} in the project video directory', source=self.__class__.__name__)
        if return_path is None and raise_error:
            raise NoFilesFoundError(msg=f'SimBA could not find a video file representing {filename} in the project video directory', source=self.__class__.__name__)
        return return_path
    ...
    def remove_a_folder(self,
                        folder_dir: str,
                        raise_error: Optional[bool] = False) -> None:
        """
    ...
    def remove_multiple_folders(self,
        ...
        for folder_path in folders:
            if raise_error and not os.path.isdir(folder_path):
                raise NotDirectoryError(msg=f'Cannot delete directory {folder_path}. The directory does not exist.', source=self.__class__.__name__)
    ...
    def drop_bp_cords(self,
        ...
        if len(missing_body_part_fields) > 0 and not raise_error:
            ...
            raise BodypartColumnNotFoundError(msg=f'SimBA could not drop body-part coordinates, some body-part names are missing in dataframe. SimBA expected the following body-parts, that could not be found inside the file: {missing_body_part_fields}', source=self.__class__.__name__)
        else:
            ...
            if config.has_option(section, option):
                if data_type == Dtypes.FLOAT.value:
                    ...
                    if not os.path.isdir(value):
                        raise NotDirectoryError(msg=f'The SimBA config file includes paths to a folder ({value}) that does not exist.', source=self.__class__.__name__)
                if options != None:
                    if value not in options:
                        raise InvalidInputError(
                            msg=f'{option} is set to {str(value)} in SimBA, but this is not among the valid options: ({options})', source=self.__class__.__name__)
                    else:
                        return value
                return value
                ...
                raise MissingProjectConfigEntryError(
                    msg=f'SimBA could not find an entry for option {option} under section {section} in the project_config.ini. Please specify the settings in the settings menu.', source=self.__class__.__name__)
        except ValueError:
    ...
    def read_video_info_csv(self,
        ...
        if not os.path.isfile(file_path):
            raise NoFilesFoundError(msg=f'Could not find the video_info.csv table in your SimBA project. Create it using the [Video parameters] tab. SimBA expects the file at location {file_path}', source=self.__class__.__name__)
        ...
        for c in ['Video', 'fps', 'Resolution_width', 'Resolution_height', 'Distance_in_mm', 'pixels/mm']:
            if c not in info_df.columns:
                raise ParametersFileError(
                    msg=f'The project "project_folder/logs/video_info.csv" does not not have an anticipated header ({c}). Please re-create the file and make sure each video has a {c} value', source=self.__class__.__name__)
        ...
        for c in ['fps', 'Resolution_width', 'Resolution_height', 'Distance_in_mm', 'pixels/mm']:
            try:
                ...
                raise ParametersFileError(msg=f'One or more values in the {c} column of the "project_folder/logs/video_info.csv" file could not be interpreted as a numeric value. Please re-create the file and make sure the entries in the {c} column are all numeric.', source=self.__class__.__name__)
        if info_df['fps'].min() <= 1:
            InvalidValueWarning(msg='Videos in your SimBA project have an FPS of 1 or less. Please use videos with more than one frame per second, or correct the inaccurate fps inside the `project_folder/logs/videos_info.csv` file', source=self.__class__.__name__)
        return info_df
    ...
    def read_video_info(self,
        ...
        if len(video_settings) > 1:
            raise DuplicationError(msg=f'SimBA found multiple rows in the project_folder/logs/video_info.csv named {str(video_name)}. Please make sure that each video name is represented ONCE in the video_info.csv', source=self.__class__.__name__)
        elif len(video_settings) < 1:
            if raise_error:
                raise ParametersFileError(msg=f'SimBA could not find {str(video_name)} in the video_info.csv file. Make sure all videos analyzed are represented in the project_folder/logs/video_info.csv file.', source=self.__class__.__name__)
            else:
    ...
    def remove_roi_features(self,
        ...
        for file_cnt, file_path in enumerate(filepaths):
            ...
            if len(roi_cols) == 0:
                NoDataFoundWarning(msg=f'NO ROI data found in {file_path}', source=self.__class__.__name__)
            else:
        ...
        timer.stop_timer()
        stdout_success(msg=f'ROI features removed from {len(filepaths)} files in the {data_dir} directory. The ROI features has been moved to the {roi_dir} directory', elapsed_time=timer.elapsed_time_str, source=self.__class__.__name__)

    def create_log_msg_from_init_args(self,
                                      locals: dict):
        ...
        def remove_keys_with_methods_or_classes(dictionary):
            return {key: value for key, value in dictionary.items() if not has_methods_or_classes(value)}

        msg = ''
        locals.pop('self', None); locals.pop('__class__', None)
    ...
    def add_video_to_video_info(self, video_name: str, fps: float, width: int, height: int, pixels_per_mm: int, distance_mm: float = 0.0) -> None:
        self.video_info_df.loc[len(self.video_info_df)] = [video_name,
                                                           fps,
                                                           width,
                                                           height,
                                                           distance_mm,
        ...
        try:
            self.video_info_df.to_csv(os.path.join(self.project_path, Paths.VIDEO_INFO.value))
        except:
        ...
        stdout_success(msg='Video info has been UPDATED at project_folder/logs/video_info.csv', source=self.__class__.__name__)
