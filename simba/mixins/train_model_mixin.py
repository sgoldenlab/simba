...
from simba.utils.enums import ConfigKey, Dtypes, MLParamKeys, Defaults, Methods, Options
...
class TrainModelMixin(object):
    ...
    def calc_pr_curve(self,
        ...
        if multiclass and classifier_map is None:
            raise InvalidInputError(msg='Creating PR curve for multi-classifier but classifier_map not defined. Add classifier_map argument')
    ...
    def print_machine_model_information(self,
        ...
        table_view = [["Model name", model_dict[MLParamKeys.CLASSIFIER.value]], ["Ensemble method", 'RF'],
                      ["Estimators (trees)", model_dict[MLParamKeys.RF_ESTIMATORS.value]],
                      ["Max features", model_dict[MLParamKeys.RF_MAX_FEATURES.value]],
                      ["Under sampling setting", model_dict[MLParamKeys.UNDERSAMPLE_SETTING.value]],
    ...
    def partial_dependence_calculator(self,
        ...
        for feature_cnt, feature_name in enumerate(x_df.columns):
            ...
            df.to_csv(save_path)
            print(f'Partial dependencies for {feature_name} complete ({feature_cnt+1}/{len(x_df.columns)})...')
    ...
    def read_all_files_in_folder_mp_futures(self,
        ...
        try:
            ...
            return self.read_all_files_in_folder(file_paths=annotations_file_paths,
                                                file_type=file_type,
                                                classifier_names=classifier_names,
                                                raise_bool_clf_error=raise_bool_clf_error)
    ...
    def read_model_settings_from_config(self,
        ...
        if self.rf_max_depth == 'None': self.rf_max_depth = None
        ...
        if config.has_option(ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.CLASS_WEIGHTS.value):
            ...
            if self.class_weights == 'custom':
                self.class_weights = ast.literal_eval(read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.CLASS_CUSTOM_WEIGHTS.value, data_type=Dtypes.STR.value))
        ...
        if self.generate_learning_curve in Options.PERFORM_FLAGS.value:
            self.shuffle_splits = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.LEARNING_CURVE_K_SPLITS.value, data_type=Dtypes.INT.value, default_value=Dtypes.NAN.value)
            self.dataset_splits = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.LEARNING_DATA_SPLITS.value, data_type=Dtypes.INT.value, default_value=Dtypes.NAN.value)
            check_int(name=MLParamKeys.LEARNING_CURVE_K_SPLITS.value, value=self.shuffle_splits)
            check_int(name=MLParamKeys.LEARNING_DATA_SPLITS.value, value=self.dataset_splits)
        else:
        ...
        if self.generate_features_importance_bar_graph in Options.PERFORM_FLAGS.value:
            self.feature_importance_bars = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.IMPORTANCE_BARS_N.value, Dtypes.INT.value, Dtypes.NAN.value)
            check_int(name=MLParamKeys.IMPORTANCE_BARS_N.value, value=self.feature_importance_bars, min_value=1)
        else:
        ...
        if self.generate_shap_scores in Options.PERFORM_FLAGS.value:
            self.shap_target_present_cnt = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.SHAP_PRESENT.value, data_type=Dtypes.INT.value, default_value=0)
            self.shap_target_absent_cnt = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.SHAP_ABSENT.value, data_type=Dtypes.INT.value, default_value=0)
            self.shap_save_n = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.SHAP_SAVE_ITERATION.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value)
            self.shap_multiprocess = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.SHAP_MULTIPROCESS.value, data_type=Dtypes.STR.value, default_value='False')
            try:
                self.shap_save_n = int(self.shap_save_n)
            except ValueError or TypeError:
                self.shap_save_n = self.shap_target_present_cnt + self.shap_target_absent_cnt
            check_int(name=MLParamKeys.SHAP_PRESENT.value, value=self.shap_target_present_cnt)
            check_int(name=MLParamKeys.SHAP_ABSENT.value, value=self.shap_target_absent_cnt)
    ...
    def check_validity_of_meta_files(self,
        ...
        for config_cnt, path in enumerate(meta_file_paths):
            ...
            errors.append(check_str(name=meta_dict[MLParamKeys.CLASSIFIER.value], value=meta_dict[MLParamKeys.CLASSIFIER.value], raise_error=False)[1])
            ...
            if meta_dict[MLParamKeys.UNDERSAMPLE_SETTING.value].lower() == Methods.RANDOM_UNDERSAMPLE.value:
                ...
                try:
                    present_len, absent_len = len(data_df[data_df[meta_dict[MLParamKeys.CLASSIFIER.value]] == 1]), len(data_df[data_df[meta_dict[MLParamKeys.CLASSIFIER.value]] == 0])
                    ratio_n = int(present_len * meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value])
                    if absent_len < ratio_n:
                        errors.append(f'The under-sample ratio of {meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value]} in \n classifier {meta_dict[MLParamKeys.CLASSIFIER.value]} demands {ratio_n} behavior-absent annotations.')
                except:
                    pass
    ...
    def random_multiclass_frm_sampler(self,
                                      x_df: pd.DataFrame,
                                      y_df: pd.DataFrame,
                                      target_field: str,
                                      target_var: int,
        ...
        if type(sampling_ratio) == dict:
            if target_var in sampling_ratio.keys():
                raise SamplingError(msg=f'The target variable {target_var} cannot feature in the sampling ratio dictionary', source=self.__class__.__name__)
        ...
        for var in target_vars:
            ...
            if sampling_n <= 0:
                raise SamplingError(msg=f'The variable {var} has a sampling ratio of {sampling_ratio[var]} which gives a sample of zero or less sampled observations')

            if (len(var_df) < sampling_n) and raise_error:
                raise SamplingError(msg=f'SimBA wanted to sample {sampling_n} examples of behavior {var} but found {len(var_df)}. Change the sampling_ratio or set raise_error to False to sample the maximum number of present observations.', source=self.__class__.__name__)
            if (len(var_df) < sampling_n) and not raise_error:
                SamplingWarning(msg=f'SimBA wanted to sample {sampling_n} examples of behavior {var} but found {len(var_df)}. Sampling {len(var_df)} observations.', source=self.__class__.__name__)
                sample = var_df
            else:
    ...
    def random_multiclass_bout_sampler(self,
                                       x_df: pd.DataFrame,
                                       y_df: pd.DataFrame,
                                       target_field: str,
                                       target_var: int,
        ...
        target_annot_idx = list(
            target_bouts.apply(lambda x: list(range(int(x['Start_frame']), int(x['End_frame']) + 1)), 1))
        ...
        for var in target_vars:
            ...
            if (len(var_bout_df) < sampling_n) and not raise_error:
                SamplingWarning(msg=f'SimBA wanted to sample {sampling_n} examples of behavior {var} bouts but found {len(var_bout_df)}. Sampling {len(var_bout_df)} observations.', source=self.__class__.__name__)
                sample = var_bout_df
            else:
