...
class TrainModelMixin(object):
    ...
    def create_shap_log(
        ...
        if save_file_no == None:
            ...
            out_df_raw_path = os.path.join(
                save_path, f"RAW_SHAP_feature_values_{clf_name}.csv"
            )
        else:
            out_df_shap_path = os.path.join(
        ...
        if len(new_headers) != len(data_df.columns):
            ...
            if difference < 0:
                print(
                    ...
                    "include {} more body-part in the imported data".format(
                        str(len(new_headers)),
                        filepath,
                ...
                print(
                    ...
                    "include {} less body-part in the imported data".format(
                        str(len(new_headers)),
                        filepath,
                        str(len(data_df.columns)),
                        str(abs(difference)),
    ...
    def _read_data_file_helper_futures(
        file_path: str, file_type: str, clf_names: Optional[List[str]] = None
    ):
        """
    ...
    def create_shap_log_mp(
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=cores) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if (
                    (processed_counter != 0)
                    and (processed_counter % save_it == 0)
                    or (processed_counter == len(shap_data_df))
                ):
