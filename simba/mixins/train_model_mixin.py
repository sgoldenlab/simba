...
from simba.utils.enums import ConfigKey, Dtypes, MetaKeys, Defaults, Methods, Options
...
class TrainModelMixin(object):
    ...
    def read_all_files_in_folder(self,
                                 ...
                                 raise_bool_clf_error: bool = True) -> pd.DataFrame:
        """
    ...
    def delete_other_annotation_columns(self,
                                        df: pd.DataFrame,
                                        annotations_lst: List[str],
                                        raise_error: bool = True) -> pd.DataFrame:
        """
        ...
        for col_name in annotations_lst:
            if col_name not in df.columns and raise_error:
                raise NoDataError(msg=f'Could not find expected column {col_name} in the data')
        ...
        for n in range(model_cnt):
            try:
                ...
                if config.has_option(ConfigKey.SML_SETTINGS.value, f'classifier_map_{n+1}'):
                    model_dict[n]['classifier_map'] = config.get(ConfigKey.SML_SETTINGS.value, f'classifier_map_{n+1}')
                    model_dict[n]['classifier_map'] = ast.literal_eval(model_dict[n]['classifier_map'])
                    if type(model_dict[n]['classifier_map']) != dict:
                        raise InvalidInputError(msg=f'SimBA found a classifier map for classifier {n+1} that could not be interpreted as a dictionary', source=self.__class__.__name__)
    ...
    def find_highly_correlated_fields(data: np.ndarray,
                                      threshold: float,
                                      field_names: types.ListType(types.unicode_type)) -> List[str]:
        ...
        remove_col_idx = {-1}
        for i in remove_set: remove_col_idx.add(i[0])
    ...
    def clf_predict_proba(self,
        ...
        if len(x_df.columns) != clf_n_features:
            if model_name and data_path:
                ...
                raise FeatureNumberMismatchError(f'The model expects {clf_n_features} features. The data contains {len(x_df.columns)} features.', source=self.__class__.__name__)
    ...
    def read_all_files_in_folder_mp(self,
        ...
        try:
            with ProcessPoolExecutor(int(np.ceil(cpu_cnt / 2))) as pool:
                for res in pool.map(self._read_data_file_helper, file_paths, repeat(file_type), repeat(classifier_names), repeat(raise_bool_clf_error)):
                    df_lst.append(res)
    ...
    def read_all_files_in_folder_mp_futures(self,
        ...
        try:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_cnt) as executor:
                results = [executor.submit(self._read_data_file_helper_futures, data, file_type, classifier_names, raise_bool_clf_error) for data in file_paths]
    ...
    def read_model_settings_from_config(self,
                                        config: configparser.ConfigParser):
        ...
        self.clf_name = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.CLASSIFIER.value, data_type=Dtypes.STR.value)
        self.tt_size = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.TT_SIZE.value, data_type=Dtypes.FLOAT.value)
        self.algo = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.MODEL_TO_RUN.value, data_type=Dtypes.STR.value)
        self.split_type = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SPLIT_TYPE.value, data_type=Dtypes.STR.value, options=Options.TRAIN_TEST_SPLIT.value, default_value=Methods.SPLIT_TYPE_FRAMES.value)
        self.under_sample_setting = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.UNDERSAMPLE_SETTING.value, data_type=Dtypes.STR.value).lower().strip()
        self.over_sample_setting = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.OVERSAMPLE_SETTING.value, data_type=Dtypes.STR.value).lower().strip()
        self.n_estimators = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.RF_ESTIMATORS.value, data_type=Dtypes.INT.value)
        self.max_features = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.RF_MAX_FEATURES.value, data_type=Dtypes.STR.value)
        self.criterion = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.RF_CRITERION.value, data_type=Dtypes.STR.value, options=Options.CLF_CRITERION.value)
        self.min_sample_leaf = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.MIN_LEAF.value, data_type=Dtypes.INT.value)
        self.compute_permutation_importance = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.PERMUTATION_IMPORTANCE.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_learning_curve = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.LEARNING_CURVE.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_precision_recall_curve = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.PRECISION_RECALL.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_example_decision_tree = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.EX_DECISION_TREE.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_classification_report = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.CLF_REPORT.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_features_importance_log = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.IMPORTANCE_LOG.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_features_importance_bar_graph = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.IMPORTANCE_LOG.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_example_decision_tree_fancy = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.EX_DECISION_TREE_FANCY.value, data_type=Dtypes.STR.value, default_value=False)
        self.generate_shap_scores = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SHAP_SCORES.value, data_type=Dtypes.STR.value, default_value=False)
        self.save_meta_data = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.RF_METADATA.value, data_type=Dtypes.STR.value, default_value=False)
        self.compute_partial_dependency = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.PARTIAL_DEPENDENCY.value, data_type=Dtypes.STR.value, default_value=False)
        if self.under_sample_setting == Methods.RANDOM_UNDERSAMPLE.value:
            self.under_sample_ratio = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.UNDERSAMPLE_RATIO.value, data_type=Dtypes.FLOAT.value, default_value=Dtypes.NAN.value)
        ...
        if (self.over_sample_setting == Methods.SMOTEENN.value.lower()) or (self.over_sample_setting == Methods.SMOTE.value.lower()):
            self.over_sample_ratio = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.OVERSAMPLE_RATIO.value, data_type=Dtypes.FLOAT.value, default_value=Dtypes.NAN.value)
            check_float(name=ConfigKey.OVERSAMPLE_RATIO.value, value=self.over_sample_ratio)
        else:
            self.over_sample_ratio = Dtypes.NAN.value

        if config.has_option(ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.CLASS_WEIGHTS.value):
            self.class_weights = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.CLASS_WEIGHTS.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value)
            if self.class_weights == 'custom':
                self.class_weights = ast.literal_eval(read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.CUSTOM_WEIGHTS.value, data_type=Dtypes.STR.value))
        ...
        if self.generate_learning_curve in Options.PERFORM_FLAGS.value:
            self.shuffle_splits = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.LEARNING_CURVE_K_SPLITS.value, data_type=Dtypes.INT.value, default_value=Dtypes.NAN.value)
            self.dataset_splits = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.LEARNING_DATA_SPLITS.value, data_type=Dtypes.INT.value, default_value=Dtypes.NAN.value)
            check_int(name=ConfigKey.LEARNING_CURVE_K_SPLITS.value, value=self.shuffle_splits)
            check_int(name=ConfigKey.LEARNING_DATA_SPLITS.value, value=self.dataset_splits)
        else:
            self.shuffle_splits, self.dataset_splits = Dtypes.NAN.value, Dtypes.NAN.value
        if self.generate_features_importance_bar_graph in Options.PERFORM_FLAGS.value:
            self.feature_importance_bars = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.IMPORTANCE_BARS_N.value, Dtypes.INT.value, Dtypes.NAN.value)
            check_int(name=ConfigKey.IMPORTANCE_BARS_N.value, value=self.feature_importance_bars, min_value=1)
        else:
            self.feature_importance_bars = Dtypes.NAN.value
        self.shap_target_present_cnt, self.shap_target_absent_cnt, self.shap_save_n, self.shap_multiprocess = None, None, None, None
        if self.generate_shap_scores in Options.PERFORM_FLAGS.value:
            self.shap_target_present_cnt = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SHAP_PRESENT.value, data_type=Dtypes.INT.value, default_value=0)
            self.shap_target_absent_cnt = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SHAP_ABSENT.value, data_type=Dtypes.INT.value, default_value=0)
            self.shap_save_n = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SHAP_SAVE_ITERATION.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value)
            self.shap_multiprocess = read_config_entry(config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.SHAP_MULTIPROCESS.value, data_type=Dtypes.STR.value, default_value='False')
            try:
                self.shap_save_n = int(self.shap_save_n)
            except ValueError or TypeError:
                self.shap_save_n = self.shap_target_present_cnt + self.shap_target_absent_cnt
            check_int(name=ConfigKey.SHAP_PRESENT.value, value=self.shap_target_present_cnt)
            check_int(name=ConfigKey.SHAP_ABSENT.value, value=self.shap_target_absent_cnt)
    ...
    def check_validity_of_meta_files(self,
        ...
        for config_cnt, path in enumerate(meta_file_paths):
            ...
            if meta_dict[ConfigKey.UNDERSAMPLE_SETTING.value].lower() == Methods.RANDOM_UNDERSAMPLE.value:
                ...
                try:
                    present_len, absent_len = len(data_df[data_df[meta_dict[MetaKeys.CLF_NAME.value]] == 1]), len(data_df[data_df[meta_dict[MetaKeys.CLF_NAME.value]] == 0])
            ...
            if ConfigKey.CLASS_WEIGHTS.value in meta_dict.keys():
                ...
                if meta_dict[ConfigKey.CLASS_WEIGHTS.value] == 'custom':
                    meta_dict[ConfigKey.CLASS_WEIGHTS.value] = ast.literal_eval(meta_dict['class_custom_weights'])
            ...
            if 'classifier_map' in meta_dict.keys():
                meta_dict['classifier_map'] = ast.literal_eval(meta_dict['classifier_map'])
    ...
    def random_multiclass_frm_undersampler(self,
                                           data_df: pd.DataFrame,
                                           target_field: str,
                                           target_var: int,
        ...
        """
        Multiclass random undersampler.
        ...
        :param str target_var: The variable in the target that should serve as baseline. E.g., ``0`` if ``0`` represents no behavior.
        :param float sampling_ratio: The ratio of target_var observations that sgould be sampled of non-target_var observations. E.g., if ``1.0``, and there are `10`` target_var observations in the dataset, then 10 of each non-target_var observations will be sampled.
        ...
        :examples:
        >>> df = pd.DataFrame(np.random.randint(0, 50, (100, 3)), columns=['F1', 'F2', 'F3', 'F4'])
        >>> df['syllable_class'] = np.random.randint(0, 3, (100,))
        ...
        if len(results_df_lst[0]) == 0:
            raise SamplingError(msg=f'No observations found in field {target_field} with value {target_var}.', source=self.__class__.__name__)
        ...
        for var in target_vars:
            ...
            if (len(var_df) < sampling_n) and raise_error:
                raise SamplingError(
                    msg=f'SimBA wanted to sample {sampling_n} examples of behavior {var} but found {len(var_df)}. Change the sampling_ratio or set raise_error to False to sample the maximum number of present observations.', source=self.__class__.__name__)
