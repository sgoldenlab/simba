...
class PlottingMixin(object):
    ...
    def make_path_plot(self,
        ...
        if not isinstance(style_attr['bg color'], np.ndarray):
            ...
            img = np.zeros((int(style_attr['bg color'].shape[0]), int(style_attr['bg color'].shape[1]), 3))
        ...
        if clf_attr:
            for clf_cnt, clf_name in enumerate(clf_attr['data'].columns):
                ...
                positions = clf_attr['positions'].loc[clf_sliced_idx].reset_index(drop=True).values
    ...
    def make_location_heatmap_plot(frm_data: np.array,
        ...
        if final_img:
            cv2.imwrite(file_name, image)
            stdout_success(msg=f'Final location heatmap image saved at at {file_name}', source=PlottingMixin.make_location_heatmap_plot.__class__.__name__)
        else:
            return image
        return None
        ...
        def __insert_shapes(img: np.array, shape_info: dict):
            for shape_name, shape_info in shape_info.items():
                ...
                if shape_info['Shape_type'] == 'Polygon':
                    ...
                    if style_attr['ROI_ear_tags']:
                        for tag_data in shape_info['Tags'].values():
                            cv2.circle(img, tuple(tag_data), scalers['circle_size'], shape_info['Color BGR'], -1)
            ...
            for animal_name in animal_ids:
                for _, shape in shape_df.iterrows():
                    ...
                    cv2.putText(border_img, loc_dict[animal_name][shape_name]['timer_text'],
                                ...
                                shape_color, TextOptions.TEXT_THICKNESS.value)
        ...
        while current_frm < end_frm:
            ...
            for animal_cnt, animal_name in enumerate(animal_ids):
                if style_attr['Show_body_part'] or style_attr['Show_animal_name']:
                    ...
                    if threshold < bp_data[2]:
                        ...
                        if style_attr['Show_animal_name']:
                            cv2.putText(border_img, animal_name, (int(bp_data[0]), int(bp_data[1])), font, scalers['font_size'], colors[animal_cnt], TextOptions.TEXT_THICKNESS.value)
                ...
                for shape_name in video_shape_names:
                    ...
                    cv2.putText(border_img, str(timer), loc_dict[animal_name][shape_name]['timer_data_loc'], font, scalers['font_size'], shape_meta_data[shape_name]['Color BGR'], TextOptions.TEXT_THICKNESS.value)
                    cv2.putText(border_img, str(entries), loc_dict[animal_name][shape_name]['entries_data_loc'], font, scalers['font_size'], shape_meta_data[shape_name]['Color BGR'], TextOptions.TEXT_THICKNESS.value)
    ...
    def path_plot_mp(data: np.array,
        ...
        for i in range(data.shape[0]):
            ...
            img = np.zeros((int(video_info['Resolution_height'].values[0]), int(video_info['Resolution_width'].values[0]), 3))
            if (type(style_attr['bg color']) == dict) and (style_attr['bg color']['type']) == 'moving':
                style_attr['bg color'] = read_frm_of_video(video_path=video_cap, opacity=style_attr['bg color']['opacity'], frame_index=frame_id)
