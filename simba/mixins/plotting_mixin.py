...
class PlottingMixin(object):
    ...
    def split_and_group_df(
        self, df: pd.DataFrame, splits: int, include_split_order: bool = True
    ...
    def validation_video_mp(
        ...
        video_path: str,
        video_meta_data: dict,
        gantt_setting: Optional[
            Literal[
                "Gantt chart: final frame only (slightly faster)", "Gantt chart: video"
            ]
        ],
        ...
        def create_gantt(
            bouts_df: pd.DataFrame, clf_name: str, image_index: int, fps: int
        ):
            fig, ax = plt.subplots(
        ...
        while current_frm < end_frm:
            ...
            cv2.putText(
                img,
                "Timer",
                (10, settings["styles"]["space_scale"]),
                font,
                settings["styles"]["font size"],
                (0, 255, 0),
                2,
