...
class PopUpMixin(object):
    ...
    def create_cb_frame(self,
                        ...
                        cb_titles: List[str],
                        frm_title: str,
                        command: Optional[object] = None) -> Dict[str, BooleanVar]:
    ...
    def frame_of_radiobuttons(self,
                              main_frm: Frame,
                              titles: List[str],
                              frm_title: str,
        ...
        radiobuttons_frm = LabelFrame(main_frm, text=frm_title, font=Formats.LABELFRAME_HEADER_FORMAT.value)
        for cnt, title in enumerate(titles):
            radio_button = Radiobutton(main_frm, text=titles[cnt], variable=selection_var, value=titles[cnt], command=command)
    ...
    def place_frm_at_top_right(self,
                               frm: Toplevel):
    ...
    def create_dropdown_frame(self,
                              ...
                              frm_title: str) -> Dict[str, DropDownMenu]:
        """
    ...
    def add_values_to_several_listboxes(self,
        ...
        if len(list_boxes) != len(values):
            raise CountError(msg='Value count and list-boxes count are not equal', source=self.__class__.__name__)
    ...
    def update_file_select_box_from_dropdown(self, filename: str, fileselectbox: FileSelect):
        """
        Updates the text inside a tkinter FileSelect entrybox with a new string.
        """
        fileselectbox.filePath.set(filename)

    def check_if_selected_video_path_exist_in_project(self, video_path: Union[str, os.PathLike]):
        if not os.path.isfile(video_path):
            raise NoFilesFoundError(msg=f'Selected video {os.path.basename(video_path)} is not a video file in the SimBA project video directory.')
    ...
    def create_entry_boxes_from_entrybox(self,
                                         count: int,
                                         parent: Frame,
        ...
        def import_menu(data_type_choice: str):
            ...
            if data_type_choice in ['CSV (DLC/DeepPoseKit)', 'MAT (DANNCE 3D)', 'JSON (BENTO)']:
                if data_type_choice == 'CSV (DLC/DeepPoseKit)':
                    ...
                    self.import_file_select = FileSelect(self.import_single_frm, 'Input FILE:', lblwidth=25, file_types=[('CSV', '*.csv')])
    ...
    def create_multiprocess_choice(self,
                                   parent: Frame,
                                   cb_text: str = 'Multiprocess videos (faster)',
                                   ):
        self.multiprocess_var = BooleanVar(value=False)
        self.multiprocess_dropdown = DropDownMenu(parent, 'CPU cores:', list(range(2, self.cpu_cnt)), '12')
        multiprocess_cb = Checkbutton(parent, text=cb_text, variable=self.multiprocess_var, command= lambda: self.enable_dropdown_from_checkbox(check_box_var=self.multiprocess_var, dropdown_menus=[self.multiprocess_dropdown]))
        ...
        self.multiprocess_dropdown.grid(row=multiprocess_cb.grid_info()['row'], column=1, sticky=NW)
