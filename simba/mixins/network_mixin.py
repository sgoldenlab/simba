...
class NetworkMixin(object):
    ...
    def girvan_newman(graph: nx.Graph,
        ...
        check_instance(source=NetworkMixin.multigraph_page_rank.__name__, instance=graph, accepted_types=nx.Graph)
        if levels > G.number_of_nodes():
            raise CountError(msg=f'Number of nodes ({G.number_of_nodes()}) is less than numer of girvan newman levels ({levels}).', source=NetworkMixin.__class__.__name__)

        communities = list(nx.algorithms.community.girvan_newman(graph, most_valuable_edge=most_valuable_edge))
