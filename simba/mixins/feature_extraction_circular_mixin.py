...
class FeatureExtractionCircularMixin(object):
    ...
    """
    Mixin for circular statistics. Support for multiple animals and base radial
    directions derived from two or three body-parts.
    ...
    def rolling_circular_stdev(data: np.ndarray,
        ...
        :example:
        >>> data_1, data_2 = np.random.normal(loc=45, scale=3, size=10), np.random.normal(loc=500, scale=700, size=10)
        >>> data = np.hstack([data_1, data_2])
