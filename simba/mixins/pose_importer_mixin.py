...
class PoseImporterMixin(object):
    ...
    def link_video_paths_to_data_paths(self,
                                       ...
                                       filename_cleaning_func: Optional[object] = None) -> dict:
        """
                ...
                try:
                    ...
                    if type(err) == OverflowError:
                        raise IntegerError(f'SimBA encountered a pose-estimated body-part located at pixel position {str(bp_tuple)}. '
                              ...
                              'Please check your pose-estimation data to make sure that it is accurate.', source=self.__class__.__name__)
    ...
    def insert_multi_idx_columns(self,
        ...
        for click_key_combination in itertools.combinations(list(self.animal_order.keys()), 2):
            ...
            if animal_1 == animal_2:
                raise InvalidInputError(msg=f'The animal most proximal to click number {str(click_n)} is animal named {animal_1}. The animal most proximal to click number {str(click_n1)} is also animal {animal_2}.'
                      'Please indicate which animal is which using a video frame where the animals are clearly separated', source=self.__class__.__name__)
    ...
    def read_apt_trk_file(self,
        ...
        if track_cnt != self.animal_cnt:
            raise CountError(msg=f'There are {str(track_cnt)} tracks in the .trk file {file_path}. But your SimBA project expects {str(self.animal_cnt)} tracks.', source=self.__class__.__name__)
