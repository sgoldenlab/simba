from simba.ROI_analysis_3 import ROIAnalyzer
...
import itertools
import cv2
from simba.misc_tools import get_video_meta_data, add_missing_ROI_cols
import numpy as np
...
class ROIPlot(object):
    def __init__(
        ...
        if not os.path.isfile(self.roi_analyzer.files_found[0]):
            raise FileNotFoundError(('Could not find the file at path {}. Please make sure you have corrected body-part outliers or indicated that you want to skip outlier correction'.format(self.roi_analyzer.files_found[0])))
        ...
        if len(list(set(self.video_shapes))) != len(self.video_shapes):
            raise AttributeError('ERROR: Some SIMBA ROI shapes have identical names. Please use unique names to visualize ROI data.')
    ...
    def insert_texts(self, shape_df):
        ...
        while (self.cap.isOpened()):
            ...
            try:
                if ret:
                    ...
                    self.insert_texts(self.roi_analyzer.video_recs)
                    self.insert_texts(self.roi_analyzer.video_circs)
                    self.insert_texts(self.roi_analyzer.video_polys)
