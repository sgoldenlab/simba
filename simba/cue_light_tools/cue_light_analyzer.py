...
class CueLightAnalyzer(object):
    ...
    def remove_outlier_events(self, frame_threshold=2):
        for cue_light_name in self.cue_light_names:
            que_light_bouts = detect_bouts(data_df=self.data_df, target_lst=[cue_light_name], fps=self.fps)
    ...
    # def detect_when_animals_are_obscuring_cue_lights(self):
    #      def multiprocess_obscuring(animal_polygons, roi_polygons):

    #     print('Analyzing when animals are obscuring cue lights in video {}...'.format(self.video_name))
    #     roi_polygons = {}
    #     for rec_light in self.video_recs.to_dict(orient='records'):
    #         roi_polygons[rec_light['Name']] = Polygon(np.array([[rec_light['topLeftX'], rec_light['topLeftY']],
    #                               [rec_light['topLeftX'] + rec_light['width'], rec_light['topLeftY']],
    #                               [rec_light['Bottom_right_X'], rec_light['Bottom_right_Y']],
    #                               [rec_light['topLeftX'], rec_light['topLeftY'] - rec_light['height']]]))
    #     for circ_light in self.video_circs.to_dict(orient='records'):
    #         pass
    #     for poly_light in self.video_polys.to_dict(orient='records'):
    #         pass
    #     for animal_name, animal_bps in self.animal_bp_dict.items():
    #         animal_arr = self.data_df[[x for y in zip(animal_bps['X_bps'], animal_bps['Y_bps']) for x in y]].values
    #         animal_arr = [Polygon(x) for x in animal_arr.reshape(-1, animal_arr.shape[1], 2)]
    #         Parallel(n_jobs=1, verbose=2, backend="loky")(delayed(multiprocess_insurrection)(x, roi_polygons) for x in animal_arr)

    def analyze_files(self):
        start_time = time.time()
        for file_cnt, file_path in enumerate(self.files_found):
            self.data_df = read_df(file_path, self.file_type)
            #self.data_df = self.data_df.head(100)
            ...
            #self.detect_when_animals_are_obscuring_cue_lights()
