...
class GraphCreator(object):
    ...
    def __weigh_time(self,
                         df: pd.DataFrame):
        results = pd.DataFrame(columns=['source', 'target', 'value'])
        if self.edge_weight_settings['Aggregation_method'] is 'Sum':
            results.loc[len(results)] = [df['ROI 1'].values[0], df['ROI 2'].values[0], df['BOUT TIME (s)'].sum()]
        elif self.edge_weight_settings['Aggregation_method'] is 'Mean':
            results.loc[len(results)] = [df['ROI 1'].values[0], df['ROI 2'].values[0], df['BOUT TIME (s)'].mean()]
        return results

    def __weigh_events(self,
                       df: pd.DataFrame):
        results = pd.DataFrame(columns=['source', 'target', 'value'])
        results.loc[len(results)] = [df['ROI 1'].values[0], df['ROI 2'].values[0], len(df)]
    ...
    def run(self):
        ...
        for video_name in self.data['VIDEO'].unique():
            ...
            for node_pair in node_pairs:
                node_pair_df = self.data.loc[(self.data['ROI 1'].isin(node_pair)) & (self.data['ROI 2'].isin(node_pair))]

                if self.edge_weight_settings['Variable'] == 'Time':
                    if self.filter_settings['Time_threshold'] is not None:
                        node_pair_df = node_pair_df[node_pair_df['BOUT TIME (s)'] > self.filter_settings['Time_threshold']]
...
test = GraphCreator(data_path='/Users/simon/Desktop/envs/simba_dev/tests/test_data/misc_test_files/termite_rois.csv',
