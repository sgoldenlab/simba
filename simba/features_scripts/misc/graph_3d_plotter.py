...
class Graph3DPlotter(object):
    def __init__(self,
        ...
        for i in self.networks.keys():
            node_locations = force_atlas.forceatlas2_networkx_layout(self.networks[i], pos=None, iterations=100)
    ...
    def plot(self,
        ...
        self.plotter.add_mesh(node_cloud, scalars=None, render_points_as_spheres=True, point_size=45, cmap='Accent', ambient=0.5, categories=True, name='non_target_baseline')

        for edge in self.networks[video_name].edges.data():
            line_points = np.array([self.node_locations[video_name][edge[0]], self.node_locations[video_name][edge[1]]])
