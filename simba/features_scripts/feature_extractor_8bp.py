    ...
    def extract_features(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            for cnt, animal_frm in enumerate(mouse_array):
                ...
                self.hull_dict['M1_largest___euclidean_distance_hull'].append(np.amax(animal_dists) / self.px_per_mm)
                self.hull_dict['M1_smallest___euclidean_distance_hull'].append(np.amin(animal_dists) / self.px_per_mm)
                self.hull_dict['M1_mean___euclidean_distance_hull'].append(np.mean(animal_dists) / self.px_per_mm)
                self.hull_dict['M1_sum___euclidean_distance_hull'].append(np.sum(animal_dists) / self.px_per_mm)
            ...
            self.out_data['Mouse_angle'] = self.out_data.apply(lambda x: self.__angle3pt(x['Nose_x'], x['Nose_y'], x['Center_x'], x['Center_y'], x['Tail_base_x'], x['Tail_base_y']), axis=1)
