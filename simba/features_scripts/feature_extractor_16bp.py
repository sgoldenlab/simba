...
class ExtractFeaturesFrom16bps(object):
    ...
    def extract_features(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            self.out_data['Movement_mouse_1_tail_end'] = self.euclidean_distance(self.in_data['Tail_end_1_x_shifted'].values, self.in_data['Tail_end_1_y_shifted'].values, self.in_data['Tail_end_1_x'].values, self.in_data['Tail_end_1_y'].values, self.px_per_mm)
            self.out_data['Movement_mouse_2_tail_end'] = self.euclidean_distance(self.in_data['Tail_end_2_x_shifted'].values, self.in_data['Tail_end_2_y_shifted'].values, self.in_data['Tail_end_2_x'].values, self.in_data['Tail_end_2_y'].values, self.px_per_mm)
            ...
            self.out_data['Mouse_1_angle'] = self.out_data.apply(lambda x: self.angle3pt(x['Nose_1_x'], x['Nose_1_y'], x['Center_1_x'], x['Center_1_y'], x['Tail_base_1_x'], x['Tail_base_1_y']), axis=1)
            self.out_data['Mouse_2_angle'] = self.out_data.apply(lambda x: self.angle3pt(x['Nose_2_x'], x['Nose_2_y'], x['Center_2_x'], x['Center_2_y'], x['Tail_base_2_x'],x['Tail_base_2_y']), axis=1)
            ...
            win_size = 3
            centroid_lst_mouse1_x = as_strided(self.out_data.Center_1_x, (len(self.out_data) - (win_size - 1), win_size),
                                               (self.out_data.Center_1_x.values.strides * 2))
            centroid_lst_mouse1_y = as_strided(self.out_data.Center_1_y, (len(self.out_data) - (win_size - 1), win_size),
                                               (self.out_data.Center_1_y.values.strides * 2))
            centroid_lst_mouse2_x = as_strided(self.out_data.Center_2_x, (len(self.out_data) - (win_size - 1), win_size),
                                               (self.out_data.Center_2_x.values.strides * 2))
            centroid_lst_mouse2_y = as_strided(self.out_data.Center_2_y, (len(self.out_data) - (win_size - 1), win_size),
                                               (self.out_data.Center_2_y.values.strides * 2))
            ...
            for window in self.roll_windows_values:
                ...
                for frame in range(len(self.out_data)):
                    ...
                    for frame_in_window in range(len(c_centroid_lst_mouse1_x)):
                        move_angle_mouse_1 = (self.angle3pt(c_centroid_lst_mouse1_x[frame_in_window][0],
                        ...
                        move_angle_mouse_2 = (self.angle3pt(c_centroid_lst_mouse2_x[frame_in_window][0],
            ...
            for k, v in in_range_dict.items():
                self.out_data["Low_prob_detections_{}".format(k)] = p_df.apply(
                    func=lambda row: self.count_values_in_range(row, v[0], v[1]), axis=1)
...
# test = ExtractFeaturesFrom16bps(config_path='/Users/simon/Desktop/train_model_project/project_folder/project_config.ini')
