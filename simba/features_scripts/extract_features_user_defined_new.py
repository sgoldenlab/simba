...
class UserDefinedFeatureExtractor(object):
    ...
    def euclid_dist_between_bps_of_other_animals(self):
    ...
    def rolling_windows_bp_distances(self):
    ...
    def pose_probability_filters(self):
        ...
        p_ranges_dict = {"0.1": [0.000000000, 0.1], "0.5": [0.000000000, 0.5], "0.75": [0.000000000, 0.75]}
        for name, p_range in p_ranges_dict.items():
            self.data_df["Low_prob_detections_" + name] = p_df.apply(func=lambda row: count_values_in_range(row, p_range[0], p_range[1]), axis=1)
            #
    ...
    def extract_features(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            self.euclid_dist_between_bps_of_other_animals()
            self.movement_of_all_bps()
            self.rolling_windows_bp_distances()
            self.rolling_windows_movement()
            self.pose_probability_filters()
