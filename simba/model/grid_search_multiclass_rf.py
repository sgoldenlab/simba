...
class GridSearchMulticlassRandomForestClassifier(ConfigReader, TrainModelMixin):
    ...
    def perform_sampling(self, meta_dict: dict):
        ...
        if meta_dict[MLParamKeys.UNDERSAMPLE_SETTING.value] == 'random undersample multiclass frames':
            meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value] = ast.literal_eval(meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value])
            self.x_train, self.y_train = self.random_multiclass_frm_sampler(x_df=self.x_train,
                                                                            ...
                                                                            sampling_ratio=meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value]['sampling_ratio'])
        elif meta_dict[MLParamKeys.UNDERSAMPLE_SETTING.value] == 'random undersample multiclass bouts':
            meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value] = ast.literal_eval(meta_dict[MLParamKeys.UNDERSAMPLE_RATIO.value])
    ...
    def run(self):
        ...
        for config_cnt, meta_dict in self.meta_dicts.items():
            ...
            print(f'Training model {config_cnt+1}/{len(self.meta_dicts.keys())} ({meta_dict[MLParamKeys.CLASSIFIER.value]})...')
            self.class_names = list(meta_dict['classifier_map'].values())
            annotation_cols_to_remove = self.read_in_all_model_names_to_remove(self.config, self.clf_cnt, meta_dict[MLParamKeys.CLASSIFIER.value])
            self.x_y_df = self.delete_other_annotation_columns(self.data_df, annotation_cols_to_remove, raise_error=False)
            self.x_df, self.y_df = self.split_df_to_x_y(self.x_y_df, meta_dict[MLParamKeys.CLASSIFIER.value])
            ...
            if meta_dict[MLParamKeys.LEARNING_CURVE.value] in Options.PERFORM_FLAGS.value:
                self.calc_learning_curve(self.x_y_df, self.clf_name, meta_dict[MLParamKeys.LEARNING_CURVE_K_SPLITS.value], meta_dict[MLParamKeys.LEARNING_CURVE_DATA_SPLITS.value], meta_dict[MLParamKeys.TT_SIZE.value], self.rf_clf, self.model_dir_out, save_file_no=config_cnt, multiclass=True)
