    ...
    def perform_sampling(self, meta_dict: dict):
        if (
            ...
            self.x_train, self.x_test, self.y_train, self.y_test = (
                self.bout_train_test_splitter(
                    x_df=self.x_df,
                    y_df=self.y_df,
                    test_size=meta_dict["train_test_size"],
                )
            )
        else:
            raise InvalidInputError(
                msg=f"{meta_dict[MLParamKeys.TRAIN_TEST_SPLIT_TYPE.value]} is not recognized as a valid SPLIT TYPE (OPTIONS: FRAMES, BOUTS).",
        ...
        if (
            ...
            self.x_train, self.y_train = self.smoteen_oversampler(
                self.x_train,
                self.y_train,
                meta_dict[MLParamKeys.OVERSAMPLE_RATIO.value],
            )
        elif (
            meta_dict[MLParamKeys.OVERSAMPLE_SETTING.value].lower()
