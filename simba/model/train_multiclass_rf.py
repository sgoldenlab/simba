...
class TrainMultiClassRandomForestClassifier(ConfigReader, TrainModelMixin):
    def __init__(self,
        ...
        self.data_df, self.frm_idx = self.read_all_files_in_folder_mp_futures(annotations_file_paths=self.target_file_paths, file_type=self.file_type, classifier_names=self.clf_names, raise_bool_clf_error=False)
        ...
        self.under_sample_setting = read_config_entry(self.config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.UNDERSAMPLE_SETTING.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value).lower().strip()
        self.under_sample_ratio = ast.literal_eval(read_config_entry(self.config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, MLParamKeys.UNDERSAMPLE_RATIO.value, data_type=Dtypes.STR.value))
        self.x_df, self.y_df = self.split_df_to_x_y(self.x_y_df, self.clf_name)
        check_if_list_contains_values(data=list(self.y_df), values=list(self.classifier_map.keys()), name=self.clf_name)
    ...
    def _check_presence_of_classes_post_sampling(self):
        for (set, clf_code) in product([self.y_train, self.y_test], self.classifier_map.keys()):
            obs_cnt = len(self.y_df[self.y_df == clf_code])
            if obs_cnt == 0:
                raise SamplingError(msg=f'Zero observations of {self.classifier_map[clf_code]} found in the training and/or test set.')
    ...
    def _perform_sampling(self):
        ...
        if self.under_sample_setting.lower() != Dtypes.NONE.value.lower():
            if self.under_sample_setting == 'random undersample multiclass frames':
                self.x_train, self.y_train = self.random_multiclass_frm_sampler(x_df=self.x_train,
                                                                                y_df=self.y_train,
                ...
                self.x_train, self.y_train = self.random_multiclass_bout_sampler(x_df=self.x_train,
                                                                                 y_df=self.y_train,
                                                                                 target_field=self.clf_name,
                                                                                 target_var=self.under_sample_ratio['target_var'],
                                                                                 sampling_ratio=self.under_sample_ratio['sampling_ratio'])
            else:
                raise SamplingError(msg=f'Under sample setting {self.under_sample_setting} not recognized. Options: [None, random undersample multiclass frames, random undersample multiclass bouts]', source=self.__class__.__name__)
        ...
        if self.save_train_test_frm_info:
            ...
            write_df(df=train_data, file_type=Formats.CSV.value,
                     save_path=os.path.join(self.eval_out_path, 'train_idx.csv'))
            write_df(df=test_data, file_type=Formats.CSV.value,
                     save_path=os.path.join(self.eval_out_path, 'test_idx.csv'))
            print(f'Frame index of train and test set saved in {self.eval_out_path} directory...')

    def run(self):
        print('Training and evaluating model...')
        self._perform_sampling()
        self.timer = SimbaTimer(start=True)
        self.rf_clf = RandomForestClassifier(n_estimators=self.n_estimators,
                                             max_depth=self.rf_max_depth,
                                             max_features=self.max_features,
                                             n_jobs=-1, criterion=self.criterion,
        ...
        if self.generate_precision_recall_curve in Options.PERFORM_FLAGS.value:
            self.calc_pr_curve(rf_clf=self.rf_clf,
                               x_df=self.x_test,
                               y_df=self.y_test,
                               clf_name=self.clf_name,
                               save_dir=self.eval_out_path,
        ...
        if self.generate_example_decision_tree in Options.PERFORM_FLAGS.value:
            self.create_example_dt(self.rf_clf, self.clf_name, self.feature_names, list(self.classifier_map.values()), self.eval_out_path)
        if self.generate_classification_report in Options.PERFORM_FLAGS.value:
            self.create_clf_report(self.rf_clf, self.x_test, self.y_test, list(self.classifier_map.values()), self.eval_out_path, clf_name=self.clf_name)
        ...
        if self.generate_features_importance_bar_graph in Options.PERFORM_FLAGS.value:
            self.create_x_importance_bar_chart(self.rf_clf, self.feature_names, self.clf_name, self.eval_out_path, self.feature_importance_bars)
        if self.generate_example_decision_tree_fancy in Options.PERFORM_FLAGS.value:
            self.dviz_classification_visualization(self.x_train, self.y_train, self.clf_name, list(self.classifier_map.values()), self.eval_out_path)
        ...
        if self.generate_shap_scores in Options.PERFORM_FLAGS.value:
            ...
            if self.save_meta_data in Options.PERFORM_FLAGS.value:
                meta_data_lst = [self.clf_name, self.criterion, self.max_features, self.min_sample_leaf,
                                 self.n_estimators, self.compute_permutation_importance, self.generate_classification_report,
                                 self.generate_example_decision_tree, self.generate_features_importance_bar_graph,
                                 self.generate_features_importance_log, self.generate_precision_recall_curve, self.save_meta_data,
                                 self.generate_learning_curve, self.dataset_splits, self.shuffle_splits, self.feature_importance_bars,
                                 self.over_sample_ratio, self.over_sample_setting, self.tt_size, self.split_type,
                                 self.under_sample_ratio, self.under_sample_setting, str(self.class_weights), self.rf_max_depth]
