...
def pup_retrieval_1(cofigini, prob_pup, prob_mother, dist_start_crit, carry_frames_seconds, smooth_factor, corenest_name, nest_name, dam_name, pup_name, smooth_function, max_time, carry_classifier_name, approach_classifier_name, dig_classifier_name):
    ...
    video_info_df = read_video_info_csv(os.path.join(logs_dir_path, 'video_info.csv'))
    ...
    for counter, video_file in enumerate(features_files):
        ...
        file_directory, video_base_name_no_file_ending, extension = get_file_path_parts(video_file)
        ...
        currVideoSettings, currPixPerMM, video_fps = read_video_info(vidinfDf=video_info_df, currVidName=video_base_name_no_file_ending)
        if max_time != 'none':
