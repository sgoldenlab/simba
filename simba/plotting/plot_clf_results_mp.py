        ...
        if ret:
            ...
            for animal_cnt, (animal_name, animal_data) in enumerate(bp_dict.items()):
                ...
                if show_animal_names:
                    ...
                    bp_cords = data.loc[current_frm, [x_bp, y_bp, p_bp]]
                    img = PlottingMixin().put_text(img=img, text=animal_name, pos=(int(bp_cords[x_bp]), int(bp_cords[y_bp])), font_size=font_size, font=font, font_thickness=text_thickness, font_path=font_path, text_color=tuple(int(v) for v in pose_clr_lst[0]), text_bg_alpha=0.0)
                if bbox is not None:
            ...
            if timer is not None:
                img = PlottingMixin().put_text(img=img, text="TIMERS:", pos=(TextOptions.BORDER_BUFFER_Y.value, ((video_meta_data["height"] - video_meta_data["height"]) + space_size)), font_size=font_size, font_path=font_path, font_thickness=text_thickness, font=font, text_bg_alpha=text_opacity, text_color_bg=text_bg_clr, text_color=text_color)
            add_spacer = 2
            ...
            for clf_name in clf_cumsum.keys():
                if data.loc[current_frm, clf_name] == 1:
                    img = PlottingMixin().put_text(img=img, text=clf_name, pos=(TextOptions.BORDER_BUFFER_Y.value, (video_meta_data["height"] - video_meta_data["height"]) + space_size * add_spacer), font_size=font_size, font_path=font_path, font_thickness=text_thickness, font=font, text_color=TextOptions.FLAMINGO.value, text_bg_alpha=text_opacity)
                    add_spacer += 1
            if video_setting:
...
class PlotSklearnResultsMultiProcess(ConfigReader, TrainModelMixin, PlottingMixin):
    ...
    :param bool show_confidence: If True, displays per-frame classifier confidence values (probabilities) for each behavior. Default False.
    ...
    :param Optional[Literal['seconds', 'hh:mm:ss.ssss']] print_timer: Timer display mode for cumulative classifier durations. ``'seconds'`` shows numeric seconds, ``'hh:mm:ss.ssss'`` shows clock-style timestamps with sub-second precision, and ``None`` disables timer display. Default ``'seconds'``.
    :param bool overwrite: If True, existing output files in the target output location may be overwritten. Default True.
    ...
    :param Optional[Dict[str, str]] time_slice: Optional time interval to render in ``{'start_time': 'HH:MM:SS', 'end_time': 'HH:MM:SS'}`` format. If None, processes full video duration. Default None.
    ...
    def __init__(self,
                 ...
                 print_timer: Optional[Literal['seconds', 'hh:mm:ss.ssss']] = 'seconds',
                 overwrite: bool = True,
        ...
        for i in [video_setting, frame_setting, rotate, animal_names, show_pose, gpu, show_confidence]:
        ...
        if font is not None:
            self.font, self.font_path = get_simba_font_name_and_path(font=font)
        ...
        if print_timer is not None: check_str(name=f'{self.__class__.__name__} timer', value=print_timer, options=(SECONDS, HHMMSSSSSS,))
    ...
    def __get_print_settings(self):
        ...
        if self.font_path is not None:
            ...
            default_space_size = self.get_optimal_font_spacing_ttf(font_path=self.font_path, size_px=self.video_font_size_px, text=['TIMERS:', 'ENSEMBLE PREDICTION:'] + list(self.clf_names) + [f'{c} CONFIDENCE: 0.0000' for c in self.clf_names], gap=0)
        else:
            self.video_font_size_px, default_space_size = None, optimal_spacing_scale
...
#                                                               r"I:\mitra\nick_ressler\project_folder\videos\2025-10-30 10-15-32_box2.mp4",
#                                                               r"I:\mitra\nick_ressler\project_folder\videos\2025-10-28 11-12-48_box4.mp4",
#                                                               r"I:\mitra\nick_ressler\project_folder\videos\2025-10-28 10-16-18_box4.mp4",
#                                                               r"I:\mitra\nick_ressler\project_folder\videos\2025-11-05 14-56-27_box2.mp4"],
#                                                  data_dir=r"I:\mitra\nick_ressler\grooming_results",
#                                                  save_dir=r'I:\mitra\nick_ressler\grooming_results\videos',
#                                                  clf_names=('GROOMING',),
#                                                  video_setting=True,
#                                                  frame_setting=False,
#                                                  rotate=False,
