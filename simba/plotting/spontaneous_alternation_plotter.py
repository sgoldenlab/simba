...
from simba.utils.checks import check_valid_lst, check_int, check_float, check_file_exist_and_readable, check_str
from simba.utils.errors import AnimalNumberError, InvalidInputError
from simba.mixins.config_reader import ConfigReader
from simba.utils.read_write import find_core_cnt, find_video_of_file, get_video_meta_data, read_frm_of_video, concatenate_videos_in_folder
from simba.utils.data import detect_bouts
from simba.utils.enums import Formats, TextOptions, Paths
...
def spontaneous_alternator_video_mp(frm_index: np.ndarray,
    ...
    writer = cv2.VideoWriter(save_path, fourcc, video_meta_data["fps"],(int(video_meta_data["width"] + (video_meta_data["width"]/2)), video_meta_data["height"]))
    while current_frm < end_frm:
        while current_frm < end_frm:
            sequence_lst = event_txt[current_frm]
            border = np.zeros((int(video_meta_data["height"]), int(video_meta_data["width"] / 2), 3), dtype=np.uint8)
            ret, img = cap.read()
            for shape_cnt, (k, v) in enumerate(roi_geometries.items()):
                cv2.polylines(img, [np.array(v.exterior.coords).astype(np.int)], True, (roi_geometry_clrs[k]), thickness=2)
            cv2.polylines(img, [np.array(animal_geometries[current_frm].exterior.coords).astype(np.int)], True, (178, 102, 255), thickness=2)
            if len(list(set(sequence_lst))) == len(list(roi_geometries.keys()))-1: txt_clr = ALTERNATION_COLOR
            else: txt_clr = ERROR_COLOR
            error_1 = len([x for x in alt_dict['same_arm_return_errors'] if x <= current_frm])
            error_2 = len([x for x in alt_dict['alt_arm_return_errors'] if x <= current_frm])
            alternations = len([x for x in alt_dict['alternations'] if x <= current_frm])
            cv2.putText(border, 'Sequence:' + ','.join(sequence_lst), (10, 50), TextOptions.FONT.value, 1, txt_clr, 2)
            cv2.putText(border, f'Alternation #: {alternations}', (10, 80), TextOptions.FONT.value, 1, txt_clr, 2)
            cv2.putText(border, f'Errors #: {error_1 + error_2}', (10, 110), TextOptions.FONT.value, 1, txt_clr, 2)
...
class SpontaneousAlternationsPlotter(ConfigReader):
    ...
    def run(self):
        sa_computer = SpontaneousAlternationCalculator(config_path=self.config_path,
        ...
        video_path = find_video_of_file(video_dir=self.video_dir, filename=sa_computer.video_name, raise_error=True)
        bout_df = detect_bouts(data_df=sa_computer.roi_df, target_lst=self.arm_names + [self.center_name], fps=1)[['Event', 'Start_frame']].sort_values(['Start_frame']).reset_index(drop=True)
        ...
        for cnt, i in enumerate(frm_index): frm_index[cnt] = np.insert(i, 0, cnt)
        event_txt = []
        for idx in range(len(sa_computer.data_df)):
            preceding_entries = list(bout_df['Event'][bout_df['Start_frame'] <= idx].tail(len(self.arm_names)))
            event_txt.append(preceding_entries)
        self.temp_folder = os.path.join(self.project_path, Paths.SPONTANEOUS_ALTERNATION_VIDEOS_DIR.value, sa_computer.video_name, "temp")
        if os.path.isdir(self.temp_folder):
            shutil.rmtree(self.temp_folder)
        os.makedirs(self.temp_folder)
        alt_dict = {'same_arm_return_errors': [x for xs in sa_computer.video_results['same_arm_returns_dict'].values() for x in xs],
                    'alt_arm_return_errors': [x for xs in sa_computer.video_results['alternate_arm_returns_dict'].values() for x in xs],
                    'alternations': [x for xs in sa_computer.video_results['alternations_dict'].values() for x in xs]}
        save_path = os.path.join(self.project_path, Paths.SPONTANEOUS_ALTERNATION_VIDEOS_DIR.value, f'{sa_computer.video_name}.mp4')
        with multiprocessing.Pool(self.core_cnt, maxtasksperchild=self.maxtasksperchild) as pool:
            constants = functools.partial(spontaneous_alternator_video_mp,
            ...
            for cnt, result in enumerate(pool.imap(constants, frm_index, chunksize=self.multiprocess_chunksize)):
        ...
        stdout_success(f'Alternation video saved at {save_path}', elapsed_time=self.timer.elapsed_time_str)
