...
from simba.utils.read_write import get_fn_ext, remove_a_folder, concatenate_videos_in_folder, read_df, find_core_cnt
...
class HeatMapperLocationMultiprocess(ConfigReader, PlottingMixin):
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            self.width, self.height = int(self.video_info['Resolution_width'].values[0]), int(
            ...
            squares, aspect_ratio = GeometryMixin().bucket_img_into_grid_square(bucket_size_mm=self.style_attr['bin_size'], img_size=(self.width, self.height), px_per_mm=self.px_per_mm)
            cum_sum_squares = GeometryMixin().cumsum_geometries(data=self.data_df.values, fps=self.fps, geometries=squares)
            if self.max_scale == 'auto':
                self.max_scale = np.round(np.max(np.max(cum_sum_squares[-1], axis=0)), 3)
                if self.max_scale == 0: self.max_scale = 1
            else: self.max_scale = self.style_attr['max_scale']
            ...
            if self.video_setting or self.frame_setting:
                ...
                print('Heatmap video {} complete (elapsed time: {}s) ...'.format(self.video_name,
