...
from simba.utils.checks import check_file_exist_and_readable, check_if_keys_exist_in_dict, check_if_valid_rgb_tuple, check_valid_lst, check_video_and_data_frm_count_align, check_int
...
from simba.utils.read_write import (concatenate_videos_in_folder, find_core_cnt, get_fn_ext, get_video_meta_data, read_df)
...
def _directing_animals_mp(frm_range: Tuple[int, np.ndarray],
    ...
    while current_frm <= end_frm:
        ...
        if ret:
            ...
            print(f"Frame: {current_frm} / {video_meta_data['frame_count']}. Core batch: {batch}")
...
class DirectingOtherAnimalsVisualizerMultiprocess(ConfigReader, PlottingMixin):
    ...
    def __init__(self,
        ...
        if not os.path.exists(self.directing_animals_video_output_path):
            os.makedirs(self.directing_animals_video_output_path)
        self.save_path = os.path.join(self.directing_animals_video_output_path, self.video_name + ".mp4")
        self.save_temp_path = os.path.join(self.directing_animals_video_output_path, "temp")
    ...
    def run(self):
        ...
        if len(video_data) < 1:
            ...
            concatenate_videos_in_folder(in_folder=self.save_temp_path,
