...
from simba.utils.read_write import get_fn_ext, read_df, find_video_of_file, read_frm_of_video, get_video_meta_data
from simba.utils.errors import NoSpecifiedOutputError, FrameRangeError
from simba.utils.checks import check_if_filepath_list_is_empty, check_int, check_if_valid_rgb_str
...
class PathPlotterSingleCore(ConfigReader, PlottingMixin):
    ...
    def __init__(self,
        ...
        check_if_filepath_list_is_empty(filepaths=self.files_found, error_msg='Zero files found in the project_folder/csv/machine_results directory. To plot paths without performing machine classifications, use path plotter functions in [ROI] tab.')
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if self.slicing:
                ...
                if len(set(frm_numbers) - set(self.data_df.index)) > 0:
                    ...
                    self.data_df = self.data_df.loc[frm_numbers[0]:frm_numbers[-1]].reset_index(drop=True)
            self.__get_styles()
            ...
            if self.input_style_attr is not None:
                ...
                if (type(self.input_style_attr['bg color']) == dict) and (self.input_style_attr['bg color']['type']) == 'static':
                    self.style_attr['bg color'] = read_frm_of_video(video_path=self.capture, opacity=self.input_style_attr['bg color']['opacity'], frame_index=self.input_style_attr['bg color']['type'])
                    ...
                    if self.input_style_attr is not None:
                        if (type(self.input_style_attr['bg color']) == dict) and (self.input_style_attr['bg color']['type']) == 'moving':
                            ...
                            if self.style_attr['bg color'] is None:
                                if self.video_setting:
                                    self.writer.release()
                                raise FrameRangeError(msg=f'Process terminated early: could not read frame number {self.data_df.index.get_loc(frm_cnt)} in video {self.video_path}. Part of video saved.')
        ...
        stdout_success(msg=f'Path visualizations for {str(len(self.files_found))} videos saved in project_folder/frames/output/path_plots directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
#               'font thickness': 2,
#               'circle size': 5,
#               'bg color': {'type': 'static', 'opacity': 100, 'frame_index': 200},
