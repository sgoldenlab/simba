...
class PathPlotterSingleCore(ConfigReader, PlottingMixin):
    ...
    def __init__(self,
        ...
        if roi:
            self.read_roi_data()
        (self.video_setting, self.frame_setting, self.input_style_attr, self.files_found, self.animal_attr, self.clf_attr, self.last_frame, self.roi) = ( video_setting, frame_setting, input_style_attr, files_found, animal_attr, clf_attr, last_frame, roi)
