...
class PlotSklearnResultsSingleCore(ConfigReader, TrainModelMixin, PlottingMixin):
    ...
    def __init__(self,
        ...
        self.clr_lst = create_color_palette(
        ...
        while self.cap.isOpened():
            ...
            try:
                if ret:
                    ...
                    for animal_name, animal_data in self.animal_bp_dict.items():
                        ...
                        for bp_no in range(len(animal_data["X_bps"])):
                            ...
                            if bp_cords[p_bp] > self.pose_threshold:
                                ...
                                if ("centroid" in x_bp.lower()) or (
                                    "center" in x_bp.lower()
                                ):
                                    self.id_flag_cords[animal_name] = (
                                        int(bp_cords[x_bp]),
                                        int(bp_cords[y_bp]),
                                    )
                                    ID_flag = True
                    ...
                    if self.rotate:
                        self.frame = np.array(
                            Image.fromarray(self.frame).rotate(
                                90, Image.BICUBIC, expand=True
                            )
                        )
                    ...
                    for model_no, model_info in self.video_model_dict.items():
                        frame_results = self.data_df.loc[
                            row_n, model_info["model_name"]
                        ]
                        self.video_model_dict[model_no]["frame_results"] = frame_results
                        self.video_model_dict[model_no]["time"] += (
                            frame_results / self.fps
                        )
                        if self.print_timers:
                            cv2.putText(
                                self.frame,
                                ...
                                + " "
                                + str(round(self.video_model_dict[model_no]["time"], 2))
                                + str("s"),
                                (
                                    10,
                                    (
                    ...
                    if self.frame_setting:
                        frame_save_name = os.path.join(
                            self.video_frame_dir, str(row_n) + ".png"
                        )
                        cv2.imwrite(frame_save_name, self.frame)
                    print(
                        f'Frame: {row_n} / {self.video_meta_data["frame_count"]}. Video: {self.video_name} ({self.file_cnt + 1}/{len(self.files_found)})'
                    )
                    row_n += 1
