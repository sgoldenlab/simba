...
from simba.utils.checks import (check_file_exist_and_readable,
                                ...
                                check_valid_lst, check_video_and_data_frm_count_align)
from simba.utils.enums import TextOptions, Formats
...
def _roi_feature_visualizer_mp(data: pd.DataFrame,
                              text_locations: dict,
                              font_size: float,
                              circle_size: float,
                              save_temp_dir: str,
    ...
    while current_frm <= end_frm:
        ...
        if ret:
            ...
            if style_attr[POSE]:
                for animal_name, bp_data in animal_bp_dict.items():
                    for bp_cnt, bp in enumerate(zip(bp_data['X_bps'], bp_data['Y_bps'])):
                        bp_cords = data.loc[current_frm, list(bp)].values.astype(np.int64)
            ...
            img = PlottingMixin.roi_dict_onto_img(img=img,
                                                  roi_dict=roi_dict,
...
class ROIfeatureVisualizerMultiprocess(ConfigReader):
    ...
    def run(self):
        ...
        data_arr, frm_per_core = PlottingMixin.split_and_group_df(self, df=self.roi_feature_creator.out_df, splits=self.core_cnt, include_split_order=True)
