...
from simba.utils.checks import check_int, check_that_column_exist, check_all_file_names_are_represented_in_video_log
...
def val_clip_createror_mp(data: np.ndarray,
    ...
    def __insert_inter_frms(bg_color: Tuple[int, int, int] = (49, 32, 189),
...
class ClassifierValidationClipsMultiprocess(ConfigReader):
    ...
    def __init__(self,
        ...
        if len(self.data_paths) == 0:
            raise NoFilesFoundError(msg='No data found in the project_folder/csv/machine_results directory', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            with multiprocessing.Pool(self.core_cnt, maxtasksperchild=self.maxtasksperchild) as pool:
                ...
                for cnt, result in enumerate(pool.imap(constants, clip_data, chunksize=self.multiprocess_chunksize)):
            ...
            if self.concat_video:
                ...
                concatenate_videos_in_folder(in_folder=self.clf_validation_dir, save_path=concat_video_save_path, substring=None, file_paths=file_paths, remove_splits=False)
            if not self.clips:
