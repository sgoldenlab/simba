...
class InteractiveVideoPlotterWindow(object):
    def __init__(self,
        ...
        self.forward_last_frm_btn = Button(self.button_frame, text=">>", command=lambda: self.load_new_frame(frm_cnt=self.video_meta_data['frame_count']-1))
        self.forward_last_frm_btn.grid(row=1, column=4, sticky=E, padx=PADDING)
        self.back_one_frm_btn = Button(self.button_frame, text="<", command=lambda: self.load_new_frame(frm_cnt=self.current_frm_number-1))
        self.back_one_frm_btn.grid(row=1, column=1, sticky=W, padx=PADDING)
        self.back_first_frm = Button(self.button_frame, text="<<", command=lambda: self.load_new_frame(frm_cnt=0))
        ...
        self.select_frm_btn = Button(self.button_frame, text="Jump to selected frame", command=lambda: self.load_new_frame(frm_cnt=self.frame_entry_var.get()))
        ...
        self.jump_back_btn = Button(self.jump_frm, text="<<", command=lambda: self.load_new_frame(frm_cnt=self.current_frm_number-self.jump_size_scale.get()))
        self.jump_back_btn.grid(row=0, column=2, sticky=E)
        self.jump_forward_btn = Button(self.jump_frm, text=">>", command=lambda: self.load_new_frame(frm_cnt=self.current_frm_number+self.jump_size_scale.get()))
        ...
        move_to_highest_p_btn = Button(instructions_frm, text="SHOW HIGHEST \n PROBABILITY FRAME", command=lambda: self.load_new_frame(frm_cnt=self.max_frm))
