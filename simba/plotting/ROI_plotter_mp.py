...
def _roi_plotter_mp(
    ...
    while current_frm < end_frm:
        ...
        if ret:
            ...
            border_img = PlottingMixin.roi_dict_onto_img(
                img=border_img, roi_dict=roi_dict
            )
            ...
            for animal_cnt, animal_name in enumerate(animal_ids):
                ...
                for shape_name in video_shape_names:
                    timer = round(
                        data.loc[
                            current_frm, f"{animal_name}_{shape_name}_cum_sum_time"
                        ],
                        2,
                    )
                    entries = data.loc[
                        current_frm, f"{animal_name}_{shape_name}_cum_sum_entries"
                    ]
                    cv2.putText(
                        border_img,
                        str(timer),
...
class ROIPlotMultiprocess(ConfigReader):
    ...
    def __init__(
        ...
        check_float(
            name=f"{self.__class__.__name__} threshold",
            value=threshold,
            min_value=0.0,
            max_value=1.0,
        )
        check_int(
            name=f"{self.__class__.__name__} core_cnt", value=core_cnt, min_value=-1
        )
        if core_cnt == -1:
        ...
        self.video_save_path = os.path.join(
            self.out_parent_dir, f"{self.video_name}.mp4"
        )
        self.read_roi_data()
        self.shape_columns = []
        ...
        if len(self.shape_names) == 0:
            raise CountError(
                msg=f"No drawn ROIs detected for video {self.video_name}, please draw ROIs on this video before visualizing ROIs",
                source=self.__class__.__name__,
            )
        self.animal_names = [
            self.find_animal_name_from_body_part_name(
                bp_name=x, bp_dict=self.animal_bp_dict
            )
            for x in body_parts
        ]
        for x in itertools.product(self.animal_names, self.shape_names):
            self.data_df[f"{x[0]}_{x[1]}"] = 0
            self.shape_columns.append(f"{x[0]}_{x[1]}")

        self.bp_dict = self.roi_analyzer.bp_dict
        self.__insert_data()
        self.video_path, self.core_cnt, self.threshold, self.body_parts = (
            video_path,
            core_cnt,
            threshold,
            body_parts,
    ...
    def __insert_data(self):
        roi_entries_dict = self.roi_entries_df[
            ["ANIMAL", "SHAPE NAME", "START FRAME", "END FRAME"]
    ...
    def __calc_text_locs(self) -> dict:
        ...
        for animal_cnt, animal_name in enumerate(self.animal_names):
            ...
            for shape in self.shape_names:
                ...
                loc_dict[animal_name][shape][
                    "timer_text"
                ] = f"{shape} {animal_name} timer:"
                loc_dict[animal_name][shape][
                    "entries_text"
                ] = f"{shape} {animal_name} entries:"
                loc_dict[animal_name][shape]["timer_text_loc"] = (
                ...
                loc_dict[animal_name][shape]["timer_data_loc"] = (
                ...
                loc_dict[animal_name][shape]["entries_text_loc"] = (
                    (self.video_meta_data["width"] + TextOptions.BORDER_BUFFER_X.value),
                    (
                        self.video_meta_data["height"]
                        - (
                ...
                loc_dict[animal_name][shape]["entries_data_loc"] = (
                    int(self.border_img_w - (self.border_img_w / 8)),
                    (
                        self.video_meta_data["height"]
                        - (
    ...
    def __calculate_cumulative(self):
        for animal_name in self.animal_names:
            for shape in self.shape_names:
                self.data_df[f"{animal_name}_{shape}_cum_sum_time"] = (
                    self.data_df[f"{animal_name}_{shape}"].cumsum()
                    / self.video_meta_data["fps"]
                )
                roi_bouts = list(
                    detect_bouts(
                        data_df=self.data_df,
                        target_lst=[f"{animal_name}_{shape}"],
                ...
                self.data_df[f"{animal_name}_{shape}_cum_sum_entries"] = self.data_df[
                    f"{animal_name}_{shape}_entry"
                ].cumsum()
        ...
        for shape, df in self.roi_dict.items():
            if not df["Name"].is_unique:
                ...
                DuplicateNamesWarning(
                    f'Some of your ROIs with the same shape ({shape}) has the same names for video {self.video_name}. E.g., you have two rectangles named "My rectangle". SimBA prefers ROI shapes with unique names. SimBA will keep one of the unique shape names and drop the rest.',
                    source=self.__class__.__name__,
                )
    ...
    def run(self):
        ...
        color_lst = create_color_palettes(
            self.roi_analyzer.animal_cnt, len(self.body_parts)
        )[0]
        ...
        with multiprocessing.Pool(
            ...
            for cnt, result in enumerate(
                pool.imap(constants, data_lst, chunksize=self.multiprocess_chunksize)
            ):
            ...
            pool.join()
            stdout_success(
                msg=f"Video {self.video_name} created. ROI video saved at {self.video_save_path}",
