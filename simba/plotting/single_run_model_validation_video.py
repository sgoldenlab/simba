...
class ValidateModelOneVideo(ConfigReader, PlottingMixin, TrainModelMixin):
    ...
    def __create_video(self):
        ...
        while (cap.isOpened()):
            ...
            cv2.putText(frame, 'Timer', (10, int(self.settings['styles']['space_scale'])), self.font, self.settings['styles']['font size'], (0, 255, 0), 2)
            addSpacer = 2
            cv2.putText(frame, (f'{self.clf_name} {target_timer}s'), (10, self.settings['styles']['space_scale']*addSpacer), self.font, self.settings['styles']['font size'], (0, 0, 255), 2)
            addSpacer+=1
    ...
    def run(self):
        self.__run_clf()
        if self.shortest_bout > 1: self.__plug_bouts()
        self.__save()
        self.__create_video()
