...
from simba.utils.enums import TagNames, TextOptions, Formats
from simba.utils.printing import log_event, stdout_success
from simba.utils.read_write import (get_fn_ext, get_video_meta_data, read_df, write_df, read_pickle)
from simba.utils.checks import check_int, check_float, check_file_exist_and_readable, check_if_keys_exist_in_dict
...
class ValidateModelOneVideo(ConfigReader, PlottingMixin, TrainModelMixin):
    ...
    def run(self):
        ...
        while (cap.isOpened()) and (frm_cnt<len(self.data_df)):
            ...
            cv2.putText(frame, "Timer", (TextOptions.BORDER_BUFFER_Y.value, int(self.settings["styles"]["space_scale"])), self.font, self.settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer = 2
            cv2.putText(frame, (f"{self.clf_name} {target_timer}s"),(TextOptions.BORDER_BUFFER_Y.value, self.settings["styles"]["space_scale"] * addSpacer),self.font,self.settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer += 1
            cv2.putText(frame, "Ensemble prediction", (TextOptions.BORDER_BUFFER_Y.value, self.settings["styles"]["space_scale"] * addSpacer), self.font,self.settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer += 2
            if clf_val == 1:
                cv2.putText(frame,self.clf_name,(TextOptions.BORDER_BUFFER_Y.value, +self.settings["styles"]["space_scale"] * addSpacer),self.font,self.settings["styles"]["font size"],TextOptions.COLOR.value,2)
                addSpacer += 1
