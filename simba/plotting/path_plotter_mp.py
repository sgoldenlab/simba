    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if self.video_setting or self.frame_setting:
                ...
                if type(self.style_attr['bg color']) == dict:
                    if self.style_attr['bg color']['type'] == 'static':
                        self.video_path = find_video_of_file(video_dir=self.video_dir, filename=self.video_name, raise_error=True)
                    ...
                    elif self.style_attr['bg color']['type'] == 'moving':
                        self.video_path = find_video_of_file(video_dir=self.video_dir, filename=self.video_name, raise_error=True)
        ...
        stdout_success(msg=f'Path plot visualizations for {str(len(self.files_found))} videos created in project_folder/frames/output/path_plots directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
