...
from simba.utils.checks import check_file_exist_and_readable, check_if_keys_exist_in_dict, check_valid_lst, check_video_and_data_frm_count_align, check_valid_array, check_valid_dataframe, check_int
...
from simba.utils.errors import ROICoordinatesNotFoundError, NoFilesFoundError, BodypartColumnNotFoundError
...
class ROIfeatureVisualizer(ConfigReader):
    ...
    def __init__(self,
        ...
        self.circle_size = int(TextOptions.RADIUS_SCALER.value / (TextOptions.RESOLUTION_SCALER.value / self.max_dim))
        ...
        for animal_cnt, animal_data in self.bp_lk.items():
            ...
            for shape in self.shape_names:
                ...
                if self.direct_viable and self.style_attr[DIRECTIONALITY]:
                    ...
                    self.loc_dict[animal][shape]["directing_text_loc"] = ((self.video_meta_data["width"] + 5), (self.video_meta_data["height"] - (self.video_meta_data["height"] + 10) + self.spacing_scale * add_spacer))
                    self.loc_dict[animal][shape]["directing_data_loc"] = (int(self.img_w_border_w - (self.img_w_border_w / 8)), (self.video_meta_data["height"] - (self.video_meta_data["height"] + 10) + self.spacing_scale * add_spacer))
                    add_spacer += 1
        ...
        for cnt, animal_data in self.bp_lk.items():
            ...
            for _, shape in shape_df.iterrows():
                ...
                if self.direct_viable:
                    cv2.putText(self.img_w_border, self.loc_dict[animal][shape_name]["directing_text"], self.loc_dict[animal][shape_name]["directing_text_loc"], self.font, self.font_size, shape_color, 1,)
    ...
    def run(self):
            ...
            if ret:
                ...
                if self.style_attr[POSE]:
                    for animal_name, bp_data in self.animal_bp_dict.items():
                        for bp_cnt, bp in enumerate(zip(bp_data['X_bps'], bp_data['Y_bps'])):
                            ...
                            cv2.circle(self.img_w_border, (bp_cords[0], bp_cords[1]), 0, self.animal_bp_dict[animal_name]["colors"][bp_cnt], self.circle_size)
                if self.style_attr[ANIMAL_NAMES]:
                    for animal_name, bp_data in self.animal_bp_dict.items():
                        ...
                        cv2.putText(self.img_w_border, animal_name, (bp_cords[0], bp_cords[1]), self.font, self.font_size, self.animal_bp_dict[animal_name]["colors"][0], 1)

                self.img_w_border = PlottingMixin.roi_dict_onto_img(img=self.img_w_border,
        ...
        self.cap.release()
        self.writer.release()
        stdout_success(msg=f"Feature video {self.video_name} saved in {self.save_path} directory ...", elapsed_time=self.timer.elapsed_time_str)
