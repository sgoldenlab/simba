...
class ROIfeatureVisualizer(ConfigReader):
    ...
    def __init__(self, config_path: str, video_name: str, style_attr: dict):
        ...
        for animal_name in self.multi_animal_id_list:
            for _, shape in shape_df.iterrows():
                ...
                if self.roi_directing_viable:
                    cv2.putText(
                        self.img_w_border,
                        self.loc_dict[animal_name][shape_name]["directing_text"],
                        self.loc_dict[animal_name][shape_name]["directing_text_loc"],
                        self.font,
                        self.font_size,
                        shape_color,
                        1,
            ...
            try:
                if ret:
                    ...
                    for animal_name, shape_name in itertools.product(
                        ...
                        in_zone_col_name = "{} {} {}".format(
                            shape_name, animal_name, "in zone"
                        )
                        ...
                        if (
                            self.roi_directing_viable
                            and self.style_attr["Directionality"]
                        ):
                    ...
                    self.writer.release()
                    print(
                        "Feature video {} saved in {} directory ...(elapsed time: {}s)".format(
                            self.video_name, self.save_path, self.timer.elapsed_time_str
                        )
                    )
