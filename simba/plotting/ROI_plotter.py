...
class ROIPlot(ConfigReader):
    ...
    def __get_bordered_img_size(self) -> Tuple[int, int]:
        ...
        cap.set(1, 1)
        _, img = self.cap.read()
        self.base_img = cv2.copyMakeBorder(
            img,
            0,
            0,
            0,
        ...
        self.base_img_h, self.base_img_w = (
            self.base_img.shape[0],
            self.base_img.shape[1],
        )
        cap.release()
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if ret:
                ...
                self.img_w_border = PlottingMixin.roi_dict_onto_img(
                    img=self.border_img, roi_dict=self.roi_dict
                )
                for animal_cnt, animal_name in enumerate(self.animal_names):
                    bp_data = (
                ...
                for animal_cnt, animal_name in enumerate(self.animal_names):
                    for shape in self.shape_names:
                        time = str(
                            round(
                                self.data_df.loc[
                                    frame_cnt, f"{animal_name}_{shape}_cum_sum_time"
                                ],
                                2,
                            )
                        )
                        entries = str(
                            int(
                                self.data_df.loc[
                                    frame_cnt, f"{animal_name}_{shape}_cum_sum_entries"
                                ]
                            )
                        )
                        cv2.putText(
                            ...
                            self.font,
                            self.scalers["font_size"],
                            self.shape_dicts[shape]["Color BGR"],
                            TextOptions.TEXT_THICKNESS.value,
                        )
                        cv2.putText(
                            self.border_img,
                ...
                print(
                    f"Frame: {frame_cnt+1} / {self.video_meta_data['frame_count']}, Video: {self.video_name}."
                )
                frame_cnt += 1
            else:
                break
        ...
        stdout_success(
            msg=f"Video {self.video_name} created. Video saved at {self.video_save_path}",
