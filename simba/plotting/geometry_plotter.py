    ...
    for frm_cnt, frm_id in enumerate(range(start_frm, end_frm-1)):
        ...
        for shape_cnt, shape in enumerate(data[frm_cnt][0:-2]):
            if isinstance(shape, Polygon):
                cv2.polylines(img, [np.array(shape.exterior.coords).astype(np.int)], True, (colors[shape_cnt][::-1]) , thickness=TextOptions.LINE_THICKNESS.value)
                ...
                for interior in interior_coords:
                    cv2.polylines(img, [interior], isClosed=True, color=(colors[shape_cnt][::-1]), thickness=TextOptions.LINE_THICKNESS.value)
            if isinstance(shape, LineString):
                cv2.polylines(img, [np.array(shape.coords, dtype=np.int32)], False, (colors[shape_cnt][::-1]), thickness=TextOptions.LINE_THICKNESS.value)
            if isinstance(shape, MultiPolygon):
                for polygon_cnt, polygon in enumerate(shape.geoms):
                    cv2.polylines(img, [np.array((polygon.convex_hull.exterior.coords), dtype=np.int32)], True, (colors[shape_cnt+ polygon_cnt + 1][::-1]),thickness=TextOptions.LINE_THICKNESS.value)
            if isinstance(shape, MultiLineString):
                for line_cnt, line in enumerate(shape.geoms):
                    cv2.polylines(img, [np.array(shape[line_cnt].coords, dtype=np.int32)], False, (colors[shape_cnt][::-1]), thickness=TextOptions.LINE_THICKNESS.value)
            if isinstance(shape, Point):
                cv2.circle(img, (int(np.array(shape.centroid)[0]), int(np.array(shape.centroid)[1])), 0, colors[shape_cnt][::-1], TextOptions.LINE_THICKNESS.value)
...
class GeometryPlotter(ConfigReader, PlottingMixin):
    ...
    .. image:: _static/img/GeometryPlotter.png
       :width: 600
       :align: center
