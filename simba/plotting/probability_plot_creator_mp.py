...
from simba.utils.checks import check_that_column_exist, check_valid_lst, check_int, check_str, check_all_file_names_are_represented_in_video_log
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if self.last_frame:
                _ = self.make_probability_plot(
                    ...
                    fps=self.fps,
                    save_path=os.path.join(self.probability_plot_dir, f"{self.video_name}_{self.clf_name}_final_image.png"))
            ...
            if self.video_setting or self.frame_setting:
                ...
                print(f"Creating probability images, multiprocessing (determined chunksize: {self.multiprocess_chunksize}, cores: {self.cores})...")
                with multiprocessing.Pool(self.cores, maxtasksperchild=self.maxtasksperchild) as pool:
                    constants = functools.partial(probability_plot_mp,
                                                  clf_name=self.clf_name,
                    ...
                    for cnt, result in enumerate(
                        pool.imap(constants, data_split, chunksize=self.multiprocess_chunksize)):
                        print(f"Image {int(frm_per_core * (result + 1))}/{len(data_df)}, Video {file_cnt + 1}/{len(self.files_found)}...")
