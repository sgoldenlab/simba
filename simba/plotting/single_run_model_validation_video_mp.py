...
from simba.utils.read_write import (concatenate_videos_in_folder, get_fn_ext, get_video_meta_data, read_df, write_df, find_core_cnt, read_pickle)
from simba.utils.checks import check_int, check_float, check_file_exist_and_readable, check_valid_extension, check_if_keys_exist_in_dict
from simba.utils.enums import TextOptions


def validation_video_mp(data: pd.DataFrame,
                        bp_dict: dict, video_save_dir: str,
                        settings: dict,
                        video_path: str,
    ...
    while (current_frm <= end_frm) & (current_frm <= video_meta_data['frame_count']):
        ...
        if ret:
            ...
            cv2.putText(img, "Timer", (TextOptions.BORDER_BUFFER_Y.value, settings["styles"]["space_scale"]), font, settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer = 2
            cv2.putText(img, (f"{clf_name} {target_timer}s"), (10, settings["styles"]["space_scale"] * addSpacer), font, settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer += 1
            cv2.putText(img, "Ensemble prediction", (TextOptions.BORDER_BUFFER_Y.value, settings["styles"]["space_scale"] * addSpacer), font, settings["styles"]["font size"], (0, 255, 0), 2)
            addSpacer += 2
            if clf_data[current_frm] == 1:
                cv2.putText(img, clf_name, (10, +settings["styles"]["space_scale"] * addSpacer), font, settings["styles"]["font size"], TextOptions.COLOR.value, 2)
            addSpacer += 1
            if gantt_setting == 1:
...
class ValidateModelOneVideoMultiprocess(ConfigReader, PlottingMixin, TrainModelMixin):
    ...
    def __init__(self,
        ...
        if (cores  > find_core_cnt()[0]) or (cores == -1): cores = find_core_cnt()[0]
        ...
        if not os.path.exists(self.clf_data_validation_dir): os.makedirs(self.clf_data_validation_dir)
    ...
    def run(self):
        ...
        if self.settings["styles"] is None:
            ...
            self.settings["styles"]["circle size"] = int(TextOptions.RADIUS_SCALER.value / (TextOptions.RESOLUTION_SCALER.value / max_dim))
            self.settings["styles"]["font size"] = float(TextOptions.FONT_SCALER.value / (TextOptions.RESOLUTION_SCALER.value  / max_dim))
            self.settings["styles"]["space_scale"] = int(TextOptions.SPACE_SCALER.value / (TextOptions.RESOLUTION_SCALER.value / max_dim))
