...
def check_int(name, value, max_value=None, min_value=None):
    ...
    if (min_value != None):
        if int(value) < min_value:
            print(('VALUE ERROR: {} should be more than, or equal {}. It is set to {}'.format(name, str(min_value), str(value))))
            raise ValueError('VALUE ERROR: {} should be more than, or equal {}. It is set to {}'.format(name, str(min_value), str(value)))
    if (max_value != None):
        if int(value) > max_value:
            print(('VALUE ERROR: {} should be less than, or equal {}. It is set to {}'.format(name, str(max_value), str(value))))
            raise ValueError('VALUE ERROR: {} should be less than, or equal {}. It is set to {}'.format(name, str(max_value), str(value)))
...
def check_str(name, value, options=(), allow_blank=False):
    ...
    if len(options) > 0:
        if value not in options:
            print(('VALUE ERROR: {} is set to {} in SimBA, but this is not a valid option {}'.format(name, str(value), options)))
...
def check_float(name, value, max_value=None, min_value=None):
    try:
        t.Float().check(value)
    except t.DataError as e:
        print('{} {} {}'.format(name, str(value), e.as_dict()))
        print('VALUE ERROR: {} should be a float in SimBA, but is set to {}'.format(name, str(value)))
        raise ValueError
    if (min_value != None):
        if value < min_value:
            print('VALUE ERROR: {} should be more than, or equal {}. It is set to {}'.format(name, str(min_value), str(value)))
...
def read_simba_meta_files(folder_path=None):
    ...
    if len(meta_file_lst) == 0:
        print('SIMBA WARNING: The training meta-files folder (project_folder/configs) does not have any meta files inside it (no files in this folder has the "meta" substring in the filename')
...
def check_file_exist_and_readable(file_path=None):
...
def insert_default_headers_for_feature_extraction(df=None, headers=None, pose_config=None, filename=None):
    if len(headers) != len(df.columns):
        raise ValueError('SIMBA ERROR: Your SimBA project is set to using the default {} pose-configuration. '
                         ...
                         'within file {} file, SimBA found {} columns.'.format(pose_config, str(len(headers)), filename, str(len(df.columns))))
    else:
        df.columns = headers
        return df

def check_that_column_exist(df=None, column_name=None, file_name=None):
    if column_name not in df.columns:
        print('SIMBA ERROR: Could not find column named `{}` within the file representing {}'.format(column_name, file_name))
        raise ValueError('SIMBA ERROR: Could not find column named `{}` within the file representing {}'.format(column_name, file_name))
    else:
        pass
