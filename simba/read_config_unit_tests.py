...
def check_int(name, value, max_value=None, min_value=None):
    ...
    if (min_value != None):
        if int(value) < min_value:
            print(('VALUE ERROR: {} should be more than, or equal {}. It is set to {}'.format(name, str(min_value),
                                                                                              str(value))))
            raise ValueError(
                'VALUE ERROR: {} should be more than, or equal {}. It is set to {}'.format(name, str(min_value),
                                                                                           str(value)))
    if (max_value != None):
        if int(value) > max_value:
            print(('VALUE ERROR: {} should be less than, or equal {}. It is set to {}'.format(name, str(max_value),
                                                                                              str(value))))
            raise ValueError(
                'VALUE ERROR: {} should be less than, or equal {}. It is set to {}'.format(name, str(max_value),
                                                                                           str(value)))
...
def check_str(name, value, options=(), allow_blank=False):
    ...
    if len(options) > 0:
        if value not in options:
            print(('VALUE ERROR: {} is set to {} in SimBA, but this is not a valid option {}'.format(name, str(value),
                                                                                                     options)))
            raise ValueError(
                'VALUE ERROR: {} is set to {} in SimBA, but this is not a valid option {}'.format(name, str(value),
                                                                                                  options))
        else:
            pass
    else:
...
def read_config_entry(config: ConfigParser,
    ...
    try:
        if config.has_option(section, option):
            ...
            if options != None:
                if value not in options:
                    print(('VALUE ERROR: {} is set to {} in SimBA, but this is not among the valid options ({})'.format(
                        option, str(value), options)))
                    raise ValueError
                else:
                    return value
            return value
            ...
            print(
                'VALUE ERROR: SimBA could not find an entry for option {} under section {} in the project_config.ini. Please specify the settings in the settings menu.'.format(
                    option, section))
            raise ValueError
    except ValueError:
        print((
