        ...
        if config.has_option(section, option):
            if data_type == 'float':
                ...
                if not os.path.isdir(value):
                    ('VALUE ERROR: SimBA is looking for the folder {} but it does not exist.'.format(value))
                    raise ValueError
            if options != None:
                if value not in options:
            ...
            print('VALUE ERROR: SimBA could not find an entry for option {} under section {} in the project_config.ini. Please specify the settings in the machine models settings menu.'.format(section, option))
            raise ValueError
    except ValueError:
        print(('VALUE ERROR: SimBA could not find an entry for option {} under section {} in the project_config.ini. Please specify the settings in the machine models settings menu.'.format(section, option)))
        raise ValueError
...
def check_file_exist_and_readable(file_path=None):
    if not os.path.isfile(file_path):
        ...
        raise FileNotFoundError('SIMBAFILE CORRUPTED ERROR: {} is not readable'.format(file_path))
