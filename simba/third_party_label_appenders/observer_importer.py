...
class NoldusObserverImporter(ConfigReader):
    ...
    def __init__(self, config_path: str, data_dir: str):
        ...
        self.observer_files_found = glob.glob(data_dir + "/*.xlsx") + glob.glob(
            data_dir + "/*.xls"
        )
        self.observer_files_found = [
            x for x in self.observer_files_found if "~$" not in x
        ]
    ...
    def __read_data(self):
        print(
            f"Reading Noldus Observer annotation files ({str(len(self.observer_files_found))} files)..."
        )
        self.annotation = {}
        for file_path in self.observer_files_found:
            ...
            for video_name in df[VIDEO_NAME_FIELD].unique():
                ...
                video_df[TIME_FIELD] = self.check_timestamps(
                ...
                if video_name in list(self.annotation.keys()):
                    self.annotation[video_name] = pd.concat(
                        [self.annotation[video_name], video_df], axis=0
        ...
        print(
            f"Annotations for {str(len(list(self.annotation.keys())))} video names found in Ethovision files..."
        )
    ...
    def run(self):
        for file_path in self.feature_file_paths:
            ...
            for clf_name in self.clf_names:
                clf_df = self.annotation[file_name][[EVENT_TYPE_FIELD, "FRAME"]][
                    self.annotation[file_name][BEHAVIOR_FIELD] == clf_name
                ].reset_index(drop=True)
                start_events, stop_events = len(
                    clf_df[clf_df[EVENT_TYPE_FIELD] == "START"]
                ), len(clf_df[clf_df[EVENT_TYPE_FIELD] == "STOP"])
                if start_events != stop_events:
                    raise ThirdPartyAnnotationEventCountError(
                        video_name=file_name,
                        clf_name=clf_name,
                        start_event_cnt=start_events,
                        stop_event_cnt=stop_events,
                    )
                start_df, stop_df = clf_df[
                    clf_df[EVENT_TYPE_FIELD] == "START"
                ].reset_index(drop=True), clf_df[
                    clf_df[EVENT_TYPE_FIELD] == "STOP"
                ].reset_index(
                    drop=True
                )
                start_df, stop_df = start_df["FRAME"].rename(
                    columns={"FRAME": "START"}
                ), stop_df["FRAME"].rename(columns={"FRAME": "STOP"})
                ...
                if len(clf_df.query("START > STOP")) > 0:
                    raise ThirdPartyAnnotationOverlapError(
                        video_name=file_name, clf_name=clf_name
                    )
                if len(clf_df) == 0:
                    ThirdPartyAnnotationsClfMissingWarning(
                        video_name=file_name, clf_name=clf_name
            ...
            print(
                f"Imported Noldus Observer annotations for video {file_name} (elapsed time {video_timer.elapsed_time_str}s)..."
            )
