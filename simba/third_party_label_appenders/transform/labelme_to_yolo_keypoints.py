...
class LabelmeKeypoints2YoloKeypoints:
    ...
    def __init__(self,
        ...
        :example:
        >>> x = LabelmeKeypoints2YoloKeypoints(data_path=r"D:\platea\ts_annotations", save_dir=r'D:\platea\yolo_071525')
        >>> x.run()
        """
        ...
        self.save_dir, self.flip_idx = save_dir, flip_idx
    ...
    def run(self):
        ...
        if self.flip_idx is None:
            self.flip_idx = get_yolo_keypoint_flip_idx(x=kp_names)
        ...
        for lbl_file_cnt, (lbl_name, lbl_path) in enumerate(self.lbls.items()):
            ...
            bp_arr = np.full(shape=(len(kp_names), 3), fill_value=0, dtype=np.int32)
...
# x = LabelmeKeypoints2YoloKeypoints(data_path=r"D:\platea\ts_annotations",
