...
SETTINGS = [{SUBJECT_COL: 'focal', BEHAVIOR_COL: 'invest_no', 'New_behavior_name': 'focal_invest_no'},
            {SUBJECT_COL: 'stimulus', BEHAVIOR_COL: 'invest_no', 'New_behavior_name': 'stimulus_invest_no'}]
...
class BorisSourceCleaner(object):
    ...
    def __init__(self,
        ...
        check_if_dir_exists(in_dir=data_dir); check_if_dir_exists(in_dir=save_dir)
        self.data_paths = find_files_of_filetypes_in_directory(directory=data_dir, extensions=['.csv'], raise_warning=False, raise_error=True)
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            check_that_column_exist(df=df, column_name=BEHAVIOR_COL, file_name=file_path)

            for rule in self.settings:
                rule_rows = df.loc[(df[SUBJECT_COL] == rule[SUBJECT_COL]) & (df[BEHAVIOR_COL] == rule[BEHAVIOR_COL])]
                rule_rows = rule_rows[rule_rows['Behavior type'] != 'POINT']

                if len(rule_rows) == 0:
                    stdout_warning(msg=f'WARNING: No rows for subject name {rule[SUBJECT_COL]} with behavior name {rule[BEHAVIOR_COL]} found in file {file_path}')
                    continue

                elif (len(rule_rows) % 2) != 0:
                    stdout_warning(msg=f'WARNING: Found an UNEVEN number of rows for subject name {rule[SUBJECT_COL]} and behavior name {rule[BEHAVIOR_COL]} found in file {file_path}')

                rule_rows[BEHAVIOR_COL] = rule['New_behavior_name']
                df.update(rule_rows)

            write_df(df=df, file_type='csv', save_path=save_path)
            stdout_success(msg=f'SAVED DATA VIDEO {save_path}. Video {file_cnt+1}/{len(self.data_paths)}')

        stdout_success(f'All data files saved in {self.save_dir}.')


if __name__ == '__main__':
    boris_cleaner = BorisSourceCleaner(data_dir=DATA_DIR,
                                       save_dir=SAVE_DIR,
                                       settings=SETTINGS)
    boris_cleaner.run()