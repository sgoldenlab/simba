...
class SleapCsvImporter(object):
    ...
    def __init__(self,
        ...
        if len(self.files_found) == 0:
            print('SIMBA ERROR: No CSV files found in the {} directory'.format(str(len(self.files_found))))
            raise ValueError('SIMBA ERROR: No CSV files found in input directory')
    ...
    def __insert_all_animal_names(self):
        for animal_cnt, animal_data in self.ID_cords.items():
            cv2.putText(self.new_frame, animal_data['name'], animal_data['cord'], cv2.FONT_HERSHEY_SIMPLEX, self.vid_font_scale, self.animal_bp_dict[animal_data['name']]['colors'][0], 3)
    ...
    @staticmethod
    @jit(nopython=True)
    def __transpose_table(data: np.array, idx: np.array, animal_cnt: int):
    ...
    def initate_import_slp(self):
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            self.data_df = pd.DataFrame(self.__transpose_table(data=data_df.values, idx=idx.values, animal_cnt=self.animal_cnt))
