...
class ROI_menu:
    ...
    def roi_table_menu(self):
        ...
        if len(self.filesFound) == 0:
            raise NoFilesFoundError('No videos found the SimBA project (no avi, mp4, mov, flv files in the project_folder/videos directory)', source=self.__class__.__name__)
        ...
        tableframe = CreateLabelFrameWithIcon(parent=self.scroll_window, header='Video Name', icon_name=Keys.DOCUMENTATION.value, icon_link=Links.ROI.value)
