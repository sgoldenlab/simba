...
class ROIClfCalculator(ConfigReader):
    ...
    def __print_missing_roi_warning(self,
        ...
        """
        names = 'None'
        ROIWarning(msg=f'ROI named "{roi_name}" of shape type "{roi_type}" not found for video {self.video_name}. Skipping shape...')
        ...
        ROIWarning(msg=f'NOTE: Video {self.video_name} has the following {roi_type} shape names: {names}')
    ...
    def run(self,
        ...
        self.ROI_dict_lists, self.behavior_list, self.measurements = ROI_dict_lists, self.clf_names, measurements
