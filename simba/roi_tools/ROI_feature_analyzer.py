...
from simba.utils.printing import stdout_success, log_event, SimbaTimer
...
from simba.utils.warnings import NoFileFoundWarning, NoDataFoundWarning
...
class ROIFeatureCreator(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        for animal in range(self.animal_cnt):
            if self.config.has_option(ConfigKey.ROI_SETTINGS.value, f'animal_{animal + 1}_bp'):
                bp = self.config.get('ROI settings', 'animal_{}_bp'.format(str(animal + 1)))
                if len(bp) == 0:
                    raise NoROIDataError(msg='Please analyze ROI data for all animals before appending ROI features . No body-part setting found in config file [ROI settings][animal_{}_bp]', source=self.__class__.__name__)
                else:
                    self.tracked_animal_bps.append([bp + '_x', bp + '_y'])

        self.files_found = glob.glob(self.outlier_corrected_dir + '/*.' + self.file_type)
        if len(self.files_found) == 0:
            raise NoFilesFoundError(msg=f'SIMBA ERROR: No data files found in {self.outlier_corrected_dir}', source=self.__class__.__name__)
        self.features_files = glob.glob(self.features_dir + '/*.' + self.file_type)
        if len(self.features_files) == 0:
            NoFileFoundWarning(msg=f'No data files found in {self.features_dir}', source=self.__class__.__name__)
        ...
        if not self.settings:
            for animal_name, shape_name in itertools.product(self.multi_animal_id_list, self.all_shape_names):
                ...
                if animal_name in list(video_distances.keys()):
                    if shape_name in list(video_distances[animal_name].keys()):
                        ...
                        self.out_df[column_name] = video_distances[animal_name][shape_name]
                    else:
                        raise NoROIDataError( msg=f'The ROI named {shape_name} could not be find in video {self.video_name}. Draw the shape or remove the file from the SimBA project',source=self.__class__.__name__)
                else:
                    pass
            ...
            for animal_cnt, shape_name in itertools.product(list(self.settings['body_parts'].keys()), self.all_shape_names):
                ...
                try:
                    ...
                    raise NoROIDataError(msg=f'The ROI named {shape_name} could not be find in video {self.video_name}', source=self.__class__.__name__)
    ...
    def __summarize_roi_results(self):
        ...
        for roi_feature in self.roi_distance_columns:
            self.summary[self.video_name][f'{roi_feature} mean (mm)'] = self.out_df[roi_feature].mean().round(4)
        if self.roi_directing_viable:
            for roi_feature in self.roi_directing_columns:
                self.summary[self.video_name][f'{roi_feature} sum (s)'] = round((self.out_df[roi_feature].sum() / self.fps), 4)
    ...
    def save(self):
        ...
        stdout_success(msg='Created additional ROI features for files within the project_folder/csv/features_extracted directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
        ...
        if len(list(self.summary.keys())) > 0:
            ...
            for video_name, video_data in self.summary.items():
                for video_measure, val in video_data.items():
                    summary_out_df.loc[len(summary_out_df)] = [video_name, video_measure, val]
            summary_out_df.to_csv(os.path.join(self.logs_path, f'ROI_features_summary_{self.datetime}.csv'))
            summary_timer.stop_timer()
            stdout_success(msg=f'ROI feature summary data saved at {save_path}', elapsed_time=summary_timer.elapsed_time_str)
