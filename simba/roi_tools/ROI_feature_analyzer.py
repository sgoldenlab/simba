...
from simba.utils.enums import TagNames, Keys
...
from simba.utils.read_write import get_fn_ext, read_df, write_df, find_files_of_filetypes_in_directory, read_data_paths
from simba.utils.checks import check_valid_lst, check_that_column_exist, check_all_file_names_are_represented_in_video_log, check_file_exist_and_readable
...
class ROIFeatureCreator(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if self.roi_directing_viable:
            ...
            self.dr = None
        if len(self.outlier_corrected_paths) == 0: raise NoFilesFoundError(msg=f'No data found in the {self.outlier_corrected_dir} directory', source=self.__class__.__name__)
        if len(self.feature_file_paths) == 0: raise NoFilesFoundError(msg=f'No data found in the {self.features_dir} directory', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        if self.append_data:
            ...
            if len(o_paths) != 0 or len(f_paths) != 0:
                raise NoFilesFoundError(msg=f'Before appending ROI features, make sure each video is represented in both the {self.outlier_corrected_dir} and {self.features_dir} directory, you have data files that are only represented in one folder {o_paths} {f_paths}', source=self.__class__.__name__)
        for file_cnt, file_path in enumerate(self.data_paths):
            video_timer = SimbaTimer(start=True)
            _, self.video_name, _ = get_fn_ext(file_path)
            features_file_path = os.path.join(self.features_dir, f'{self.video_name}.{self.file_type}')
            self.out_df = read_df(file_path=features_file_path, file_type=self.file_type)
            ...
            for animal_cnt, animal_data in self.bp_lk.items():
                ...
                for _, row in self.video_roi_dict[Keys.ROI_RECTANGLES.value].iterrows():
                    ...
                    self.summary.loc[len(self.summary)] = [self.video_name, animal_name, roi_name, 'Average distance (mm)', self.out_df[c].mean().round(4)]
                ...
                for _, row in self.roi_dict[Keys.ROI_POLYGONS.value].iterrows():
                    ...
                    self.summary.loc[len(self.summary)] = [self.video_name, animal_name, roi_name, 'Average distance (mm)', self.out_df[c].mean().round(4)]
                    c = f"{roi_name} {animal_name} {body_part_name} in zone"
                    self.out_df[c] = FeatureExtractionMixin.framewise_inside_polygon_roi(bp_location=animal_df.values[:, 0:2], roi_coords=roi_coords)
                if self.roi_directing_viable:
                    ...
                    for shape_name in self.shape_names:
                        ...
                        self.summary.loc[len(self.summary)] = [self.video_name, animal_name, shape_name, 'Total direction time (s)', round((self.out_df[c].sum() / self.fps), 4)]
            video_timer.stop_timer()
            if self.append_data:
                write_df(df=self.out_df, file_type=self.file_type, save_path=features_file_path)
                print(f'New file with ROI features created at  {features_file_path} saved (File {file_cnt+1}/{len(self.outlier_corrected_paths)}), elapsed time: {video_timer.elapsed_time_str}s')
