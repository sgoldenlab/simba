...
class ROIFeatureCreator(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        for animal in range(self.animal_cnt):
            ...
            if len(bp) == 0:
                raise NoROIDataError(msg='Please analyze ROI data for all animals before appending ROI features . No body-part setting found in config file [ROI settings][animal_{}_bp]')
            else:
