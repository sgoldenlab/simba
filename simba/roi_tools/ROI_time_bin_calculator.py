...
class ROITimebinCalculator(ConfigReader):
    ...
    def __init__(self,
        ...
        self.roi_analyzer = ROIAnalyzer(ini_path=self.config_path, data_path=DirNames.OUTLIER_MOVEMENT_LOCATION.value, calculate_distances=False, settings=self.settings)
    ...
    def run(self):
        ...
        check_if_filepath_list_is_empty(filepaths=self.outlier_corrected_paths, error_msg=f'No data files found in {self.outlier_corrected_dir}')
