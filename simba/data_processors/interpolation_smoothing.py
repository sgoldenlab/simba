...
from simba.utils.read_write import read_df, write_df, get_video_meta_data, get_fn_ext, find_video_of_file, find_files_of_filetypes_in_directory
        ...
        for file_cnt, file_path in enumerate(self.file_paths):
            ...
            if self.initial_import_multi_index:
                if len(df.columns) != len(self.bp_col_names):
                    raise DataHeaderError(msg=f'The SimBA project suggest the data should have {len(self.bp_col_names)} columns, but the input data has {len(df.columns)} columns')
                df.columns = self.bp_headers
            df[df < 0] = 0
            ...
            write_df(df=df, file_type=self.file_type, save_path=file_path, multi_idx_header=self.initial_import_multi_index)
        self.timer.stop_timer()
        stdout_success(msg='Interpolation complete!', elapsed_time=self.timer.elapsed_time_str)
...
class AdvancedSmoother(ConfigReader):
    ...
    def run(self):
        ...
        self.timer.stop_timer()
        stdout_success(msg='Smoothing complete complete!', elapsed_time=self.timer.elapsed_time_str)
