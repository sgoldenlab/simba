...
from simba.utils.checks import check_valid_lst, check_file_exist_and_readable, check_if_dir_exists, check_int, check_float, check_video_has_rois, check_str
from simba.utils.errors import NoFilesFoundError, AnimalNumberError, InvalidInputError
...
class SpontaneousAlternationCalculator(ConfigReader):
    ...
    def run(self):
        ...
        for file_path in self.file_paths:
            ...
            print(px_per_mm)
            self.animal_polygons = GeometryMixin().multiframe_bodyparts_to_polygon(data=bp_arr, parallel_offset=self.buffer, pixels_per_mm=px_per_mm, video_name=self.video_name, animal_name='Animal', verbose=self.verbose)
