...
from simba.utils.checks import check_if_filepath_list_is_empty, check_that_dir_has_list_of_filenames
...
from simba.utils.errors import AnimalNumberError, InvalidInputError, CountError
...
class DirectingOtherAnimalsAnalyzer(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if self.animal_cnt < 2: raise AnimalNumberError('Cannot analyze directionality between animals in a 1 animal project.', source=self.__class__.__name__)
        check_if_filepath_list_is_empty(filepaths=self.outlier_corrected_paths, error_msg=f'SIMBA ERROR: No data found in the {self.outlier_corrected_dir} directory')
    ...
    def summary_statistics(self):
        ...
        stdout_success(msg=f'Summary directional statistics saved at {self.save_path}', source=self.__class__.__name__)
