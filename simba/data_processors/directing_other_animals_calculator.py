...
from simba.utils.checks import (check_all_file_names_are_represented_in_video_log,
                                check_that_dir_has_list_of_filenames, check_file_exist_and_readable)
from simba.utils.enums import TagNames
from simba.utils.errors import AnimalNumberError, CountError, InvalidInputError
from simba.utils.printing import SimbaTimer, log_event, stdout_success
from simba.utils.read_write import get_fn_ext, read_df, write_df, read_data_paths
...
class DirectingOtherAnimalsAnalyzer(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        data_paths = read_data_paths(path=data_paths, default=self.outlier_corrected_paths, default_name=self.outlier_corrected_dir, file_type=self.file_type)
        ...
        if not self.check_directionality_viable()[0]:
            raise InvalidInputError(msg='You are not tracking the necessary body-parts to calculate direction.', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            for animal_permutation in self.animal_permutations:
                ...
                first_ear_left_arr = data_df[[first_animal_bps["Ear_left"]["X_bps"],first_animal_bps["Ear_left"]["Y_bps"]]].values
                first_ear_right_arr = data_df[[first_animal_bps["Ear_right"]["X_bps"], first_animal_bps["Ear_right"]["Y_bps"]]].values
                first_nose_arr = data_df[[first_animal_bps["Nose"]["X_bps"], first_animal_bps["Nose"]["Y_bps"]]].values
                ...
                for x_bp, y_bp in zip(second_animal_x_bps, second_animal_y_bps):
                    ...
                    direction_data = self.jitted_line_crosses_to_nonstatic_targets(left_ear_array=first_ear_left_arr,
        ...
        if self.bool_tables:
            ...
            for video_cnt, (video_name, video_data) in enumerate(self.results.items()):
                print(f"Saving boolean directing tables for video {video_name} (Video {video_cnt + 1}/{len(self.results.keys())})...")
            ...
            stdout_success(msg=f"All boolean tables saved in {save_dir}!",source=self.__class__.__name__)
        ...
        if self.aggregate_statistics_tables:
            ...
            stdout_success(msg=f"Summary directional statistics saved at {save_path}", source=self.__class__.__name__)
        ...
        if self.summary_tables:
            ...
            stdout_success(f"All detailed directional data saved in the {save_dir} directory!", source=self.__class__.__name__)
        self.timer.stop_timer()
        stdout_success(msg="All directional data saved in SimBA project", elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def transpose_results(self):
        ...
        for video_name, video_data in self.results.items():
            ...
            for animal_permutation, permutation_data in video_data.items():
                for bp_name, bp_data in permutation_data.items():
                    directing_df = (bp_data[bp_data["Directing_BOOL"] == 1].reset_index().rename(
                        columns={"index": "Frame_#", bp_name + "_x": "Animal_2_bodypart_x",
