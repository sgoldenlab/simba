...
class TimeBinsMovementCalculator(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        self.save_path = os.path.join(
            self.project_path, "logs", f"Time_bins_movement_results_{self.datetime}.csv"
        )
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            for animal_data in self.bp_dict.values():
                ...
                for bin, movement_df in enumerate(movement_df_lists):
                    time_bin_sliced = [
                        movement_df[i : i + fps]
                        for i in range(0, movement_df.shape[0], fps)
                    ]
                    bin_velocities = []
                    ...
                    results.append(
                        {
                            ...
                            "VALUE": round(movement_df.sum(), 4)[0],
                        }
                    )
                    results.append(
                        {
                            ...
                            "VALUE": round(np.mean(bin_velocities), 4),
                        }
                    )
