...
class AggregateClfCalculator(ConfigReader):
    ...
    def __init__(
        ...
        self.file_save_name = os.path.join(
            self.project_path, "logs", "data_summary_" + str(self.datetime) + ".csv"
        )
        check_if_filepath_list_is_empty(
    ...
    def save(self):
        ...
        self.results_df = (
            self.results_df[self.results_df["Measure"].isin(self.chosen_measures)]
        ...
        stdout_success(
            msg=f"Data log saved at {self.file_save_name}",
