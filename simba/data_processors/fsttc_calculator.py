...
class FSTTCCalculator(ConfigReader):
    ...
    def find_sequences(self):
        ...
        for file_cnt, file_path in enumerate(self.machine_results_paths):
            ...
            print(
                "Analyzing behavioral sequences: {}. Video {}/{}".format(
                    self.video_name,
                    str(file_cnt + 1),
                    str(len(self.machine_results_paths)),
                )
            )
            ...
            self.video_sequence_dict[self.video_name]["session_length_frames"] = len(
                self.data_df
            )
            bouts_df = detect_bouts(
                data_df=self.data_df, target_lst=self.behavior_lst, fps=self.fps
            )
            bouts_df["Start_frame"] = (bouts_df["Start_time"] * self.fps).astype(
                int
            ) - 1
            ...
            for first_clf, second_clf in self.clf_permutations:
                ...
                first_clf_df = (
                    bouts_df[bouts_df["Event"] == first_clf]
                    .sort_values(by=["Start_frame"])
                    .reset_index(drop=True)
                )
                second_clf_df = (
                    bouts_df[bouts_df["Event"] == second_clf]
                    .sort_values(by=["Start_frame"])
                    .reset_index(drop=True)
                )
    ...
    def plot_FSTTC(self):
        ...
        figure_FSCTT = sns.violinplot(
            x="BEHAVIOR COMBINATION", y="FSTTC", data=data_df, cut=0
        )
        figure_FSCTT.set_xticklabels(
            figure_FSCTT.get_xticklabels(), rotation=45, size=7
        )
    ...
    def save(self):
        ...
        stdout_success(
            msg=f"FSTTC data saved at {file_save_path}",
