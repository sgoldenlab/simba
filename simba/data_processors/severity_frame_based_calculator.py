...
class SeverityCalculator(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        if self.settings["normalization"] == "ALL VIDEOS":
            ...
            for video_name, video_movements in self.movements.items():
                ...
                movements_cuts[video_name] = np.hstack(
                    (
                        video_movements.reshape(-1, 1),
                        data.astype(np.int8).reshape(-1, 1),
                    )
                )
        else:
            for video_name, video_movements in self.movements.items():
        ...
        for file_cnt, file_path in enumerate(self.machine_results_paths):
            ...
            df = pd.concat(
                [
                    df,
                    pd.DataFrame(
                        movements_cuts[video_name], columns=["MOVEMENT", "BIN"]
                    ),
                ],
                axis=1,
            )
    ...
    def save(self):
        ...
        if self.settings["save_bin_definitions"]:
            ...
            for i in range(0, self.settings["brackets"]):
                brackets_data_cols.append(f"GRADE {i+1} START")
                brackets_data_cols.append(f"GRADE {i + 1} STOP")
            ...
            stdout_success(
                msg=f"Severity brackets definitions saved at {self.definitions_path}",
                elapsed_time=self.timer.elapsed_time_str,
            )
