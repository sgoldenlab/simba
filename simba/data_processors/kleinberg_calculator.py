...
class KleinbergCalculator(ConfigReader):
    ...
    def __init__(
        ...
        self.hierarchical_search, sigma, gamma, hierarchy = (
            hierarchical_search,
            float(sigma),
            float(gamma),
            int(hierarchy),
        ...
        self.sigma, self.gamma, self.hierarchy = (
            float(sigma),
            float(gamma),
            float(hierarchy),
        )
        ...
        original_data_files_folder = os.path.join(
            ...
            "Pre_Kleinberg_{}".format(self.datetime),
        )
        ...
        print(
            f"Processing Kleinberg burst detection for {str(len(self.machine_results_paths))} file(s)..."
        )
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.machine_results_paths):
            ...
            print(
                f"Kleinberg analysis video {video_name}. Video { str(file_cnt+1)}/{str(len(self.machine_results_paths))}..."
            )
            data_df = read_df(file_path, self.file_type).reset_index(drop=True)
            video_out_df = deepcopy(data_df)
            for clf in self.clfs:
                check_that_column_exist(
                    df=data_df, column_name=clf, file_name=video_name
                )
        ...
        if len(detailed_df_lst) > 0:
            ...
            stdout_success(
                msg="Kleinberg analysis complete. See {detailed_save_path} for details of detected bouts of all classifiers in all hierarchies",
