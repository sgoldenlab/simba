...
class GibbsSampler(ConfigReader):
    ...
    def __init__(self,
        ...
        self.data, self.pseudo_num, self.sequence_len = data, pseudo_number, sequence_length
        ...
        self.save_path = os.path.join(self.logs_path, f'gibbs_sampling_results_{self.datetime}.csv')

    def __get_start_positions(self):
        start_positions = []
        for val in range(len(self.data)):
            start_positions.append(random.randint(0, len(self.data.columns) - self.sequence_len))
    ...
    def __make_probability_table(self, df: pd.DataFrame, unique_vals: np.ndarray):
        ...
        for column in df:
            ...
            for value in unique_vals:
                occurance_lst.append((len(df[df[column] == value]) + self.pseudo_num) / len(df[column]) + self.pseudo_num)
    ...
    def __iterate_through_possible_seq_in_hold_out(self,
        ...
        for startv in range(0, len(hoObs.columns) - self.sequence_len + 1):
            cols = ['F_{}'.format(ind) for ind in range(startv, startv + self.sequence_len)]
            seq = hoObs.loc[:, cols]
            ptot = 1
    ...
    def __sum_results(self,
                    full_sequence_set,
                    summary_df):

        for i in range(self.sequence_len):
            full_sequence_set = full_sequence_set.rename(columns={f'C_{i}': self.output_cols[i]})
            ...
            for iteration in range(0, iterations):
                ...
                if iteration==0:
                    ...
                    for index, row in df_it.iterrows():
                        seq_lst.append(list(row[start_positions[index]:start_positions[index] + self.sequence_len]))
                        idx_lst.append(index)
                    non_holdout_df = pd.DataFrame(seq_lst, columns=self.non_holdout_cols, index=idx_lst)
                else:
                    non_holdout_df = full_sequence_set.loc[full_sequence_set.index != holdout_idx]

                probability_df = self.__make_probability_table(non_holdout_df, self.unique_vals)
                df_holdout_seqs = self.__iterate_through_possible_seq_in_hold_out(hold_out_observation_df, bg_df, probability_df, holdout_idx)
                ...
                print(f'Current error: {error}, iteration: {iteration}, epoch: {iteration_counter}')
                ...
                if holdout_idx < (len(self.data) - 1):
                    ...
                    holdout_idx = 0
            self.summary_df, output = self.__sum_results(full_sequence_set, self.summary_df)


        output.to_csv(self.save_path)
        self.timer.stop_timer()
        stdout_success(msg=f'Gibbs sampling results saved @{self.save_path}', elapsed_time=self.timer.elapsed_time_str)
