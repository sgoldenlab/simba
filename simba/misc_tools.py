...
def find_video_of_file(video_dir: str,
    ...
    if return_path is None:
        print('SIMBA WARNING: SimBA could not find a video file representing {} in the project video directory'.format(
            str(filename)))
    return return_path
...
def smooth_data_gaussian(config: configparser.ConfigParser,
    ...
    try:
        ...
        print(
            'Gaussian smoothing failed for video {}. Time window parameter {} can not be interpreted as an integer.'.format(
...
def copy_single_video_to_project(simba_ini_path: str,
    ...
    if os.path.isfile(destination):
        ...
        print(
            'SIMBA COMPLETE: Video {} imported to SimBA project (project_folder/videos directory, elapsed time: {}s).'.format(
                file_name, timer.elapsed_time_str))
...
def copy_multiple_videos_to_project(config_path: str,
    ...
    if file_type.lower().strip() not in allowed_video_formats:
        print(
            'SIMBA ERROR: SimBA only works with avi and mp4 videofiles (Please enter mp4 or avi in entrybox). Or please convert your videos to mp4 or avi to continue. ')
        raise ValueError()
...
def find_files_of_filetypes_in_directory(directory: str,
    ...
    try:
        ...
        print('SIMBA WARNING: No files found in the {} directory with accepted extensions {}'.format(directory,
                                                                                                     str(extensions)))
    ...
    if not accepted_file_paths and raise_warning:
        print('SIMBA WARNING: SimBA could not find any files with accepted extensions {} in the {} directory'.format(
            extensions, str(directory)))
