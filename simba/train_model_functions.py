...
def read_all_files_in_folder(file_paths: list,
    ...
    Helper function to read in all data files in a folder for downstream ML algorithm.
...
def read_in_all_model_names_to_remove(config, model_cnt, clf_name):
    """
    Helper function find all fields that contain annotations but are not the target.
...
def delete_other_annotation_columns(df: pd.DataFrame,
    ...
    """
    Helper function delete all fields that contain annotations but are not the target.
...
def print_machine_model_information(model_dict: dict):
    ...
    ----------
    model_dict: dict
        Python dictionary holding model meta data
