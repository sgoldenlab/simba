...
class SLEAPImporterCSV(ConfigReader, PoseImporterMixin):
    ...
    def run(self):
        for file_cnt, (video_name, video_data) in enumerate(
            ...
            if "instance.score" in data_df.columns:
                data_df = data_df.drop(["instance.score"], axis=1)
            idx = data_df.iloc[:, :2]
            ...
            if len(self.bp_headers) != len(self.data_df.columns):
                raise CountError(
                    msg=f"SimBA project expects {len(self.bp_headers)} data columns, but your SLEAP data file {video_name} contains {len(self.data_df.columns)} columns",
