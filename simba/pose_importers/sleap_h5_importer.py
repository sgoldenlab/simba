        ...
        for file_cnt, (video_name, video_data) in enumerate(
            ...
            try:
                with h5py.File(video_data["DATA"], "r") as sleap_dict:
                    ...
                    data["node_names"] = [
                        s.decode() for s in data["node_names"].tolist()
                    ]
                    data["track_names"] = [
                        s.decode() for s in data["track_names"].tolist()
                    ]
                    data["tracks"] = np.transpose(data["tracks"])
                    data["track_occupancy"] = data["track_occupancy"].astype(bool)
            except OSError:
                stdout_warning(
                    msg=f'{video_data["DATA"]} is not a valid SLEAP H5 file. Skipping {video_name}...'
                )
                continue
            valid_frame_idxs = np.argwhere(
                data["track_occupancy"].any(axis=1)
            ).flatten()
            tracks = []
