...
class SimBABlobImporter(ConfigReader):
    ...
    and converts them into SimBA's standard pose-estimation format. Because blob tracking is geometric (derived from a background-subtracted
    silhouette) and carries no pose-confidence score, a constant likelihood column (``<bp>_p = 1.0``) is appended for each body part,
    yielding a 15-column SimBA pose file. The imported data can optionally be interpolated and smoothed.

    .. image:: _static/img/simba.pose_importers.simba_blob_importer.SimBABlobImporter.webp
       :alt: Blob tracking yields 5 geometric body parts (nose, left, center, right, tail); SimBABlobImporter adds a constant likelihood and writes a 15-column SimBA CSV
       :width: 800
       :align: center
