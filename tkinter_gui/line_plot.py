...
def line_plot_config(configini):
    ...
    for i in filesFound:
        ...
        csv_df_shifted = csv_df_shifted.rename(
            columns={columnNames[0]: str(columnNames[0] + '_shifted'), columnNames[1]: str(columnNames[1] + '_shifted'),
                     columnNames[2]: str(columnNames[2] + '_shifted'),
                     columnNames[3]: str(columnNames[3] + '_shifted')})
        ...
        csv_df_combined["distanceOI"] = np.sqrt((csv_df[columnNames[0]] - csv_df[columnNames[2]]) ** 2 + (
                    csv_df[columnNames[1]] - csv_df[columnNames[3]]) ** 2)
